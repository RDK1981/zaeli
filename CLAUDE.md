...
*Last updated: 31 July 2026 — Session 32 v2 Round A ✅ · **⭐⭐⭐ MAJOR VISION PIVOT — Zaeli is now a "family productivity trio"** (Calendar + Shopping + Budget + Reminders as the 4th pillar), not "AI companion that also manages family life". Home is the front door (Dashboard renamed); Chat is one swipe right. Kids Hub / Tutor / Travel / My Space / Meals all moved to `app/_hidden/` folder — not deleted, just off the map for v2. Focused product surface for Anna beta round 3 · **Home tile design**: big-text headlines (26px 800) — "2 events on today.", "4 items to grab.", "1 due at 3pm.", "On track for the month." — with detail rows below + per-tile mic in add pill + "Full ×→" hint. Universal chat bar identical Home↔Chat for zero-flicker swipe · **Reminders subsystem** (4th pillar) — new `supabase-reminders.sql` (three shapes: timed/date-only/undated + recurring 12-month horizon), `lib/reminders.ts`, gold Home tile + 92% sheet with unified input pill, `add_reminder` Sonnet tool. Family-shared visibility, creator-only push notifications · **Server-side brief scheduler** — Phase 07 shipped. `supabase/functions/brief-scheduler` (Deno) + pg_cron every 15 min. Cron fires at each family's morning/evening brief time (widened window: 0-45 min after target for cron latency), generates brief via Sonnet, upserts to zaeli_briefs cache, and pushes RICH content to lockscreen via Expo push. Local notifications KILLED — Phase 07 is sole notification source now · **Round A bug fixes** — (a) Notify chip on calendar adds fixed (chip render was in wrong branch), (b) Reminder timezone bug fixed (Hermes parses no-tz ISO as UTC; new `parseLocalIsoAsDate` + CURRENT_TIME injected into Sonnet prompt), (c) Pantry no longer pre-blocks shopping adds — always adds first, mentions pantry status softly for user to remove if not needed · **Sheet-over-Home** — tapping a tile no longer scrolls to Chat first. Sheets pop over Home directly (Modal is RN portal). Closing a sheet keeps user on Home · **MoreSheet redesign** — 6 full-width shorter tiles stacked (was 3×2 grid), Reminders added, cleaner scan · **Flat Expenses model in Our Budget** — Categories tab renamed to Expenses, new flat list (fixed/variable badge + monthly amount) alongside legacy nested categories during transition · Prior Session 30 work still current (Phase 5 API keys server-side, Stripe LIVE, Family push, Anna round 2 fixes, multi-photo chat, expo-notifications plugin, verbose debug diagnostics) — remaining: production EAS build + `eas submit --platform ios --latest` for Anna beta round 3, Round B stale-tile bug fix (Home tile doesn't refresh after sheet edits), Shopping sheet input pill unification, website v6 shipped (mirror section + big-text tiles + lockscreen brief + Reminders + trio deep-dive replaced by mirror + A$6.99 pricing), Sonnet 5 migration queued. **Pricing dropped 31 July: A$9.99 → A$6.99 family plan tax-inclusive** — reflects narrower v2 product surface. Tutor add-on hidden with the rest until trio validates. Stripe live products need updating (Rich creates new Price + Payment Link with 14-day trial, updates `lib/stripe.ts` constants + webhook `PRICE_TO_PLAN`).*
