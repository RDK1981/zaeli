...
*Last updated: 17 August 2026 late — Session 37 ✅ · **CHAT BRIEF KILLED · HOME CHATTILE SHIPPED · CHAT EMPTY-STATE GREETING · MARKETING KICK-OFF · COST INVESTIGATION → BUILD 60 BACKLOG.** Rich pivoted brief architecture after a chain of realisations: (a) evening brief still showing "clear day" at 10pm Monday when Tuesday was busy — stale-cache issue, (b) at 3:30pm the morning brief is still there problem inherent to persistent tile displaying a moment-in-time push, (c) the brief was DESIGNED as a lockscreen push moment, not a widget. Chat is for INPUT, not reading briefs. Full pivot in one session — kill client brief pipeline, replace Home BriefTile with time-of-day ChatTile, add chat empty-state greeting so users never land on blank screen · **⭐⭐⭐ Chat brief killed (Build 57, commit `b52c5c0`)** — removed all 4 `tryFireBrief` call sites in `app/(tabs)/index.tsx` (mount, isActive, focus, voice/photo callback) + removed 5-min held-brief-check interval. `tryFireBrief`, `buildBriefContext`, `generateBrief` left in place as dead code (harmless). Server-side `brief-scheduler` unchanged — still fires lockscreen pushes at each user's scheduled brief times · **⭐⭐⭐ Home ChatTile Variation B (Build 58, commit `83da3bf`)** — replaced my initial Session 37 attempt at a BriefTile (had all the stale-cache problems Rich diagnosed). New tile is a time-of-day greeting card at top of Home. 5 windows: Morning 5am–11:59am ☀️ peach "Morning 👋 What's on your mind?" · Midday 12pm–3:59pm 💬 sky "Hey — anything I can help with?" · Afternoon 4pm–6:59pm 👋 mint "Afternoon 👋 What can I sort?" · Evening 7pm–11:59pm 🌙 lavender "Evening — anything to knock off?" · Late 12am–4:59am 💤 slate "Still up? I'm here." Tap → routes to Chat. Zero AI cost per render. All tokens already in app palette. Killed `latestBrief` + `briefExpanded` state, removed `briefRes` fetch from `loadData` Promise.all, removed `cachedBrief` hydration, removed `briefLatest` cache key from `lib/home-cache.ts`. Mockup: NEW `zaeli-chat-tile-mockups.html` (3 variations shown — Rich picked B) · **⭐ Old brief messages stripped from chat persistence (Build 59, commit `8ea41dc`)** — Rich installed Build 58 and briefs were still visible in chat feed. Root cause: `useChatPersistence` hydrates from AsyncStorage file with 24h TTL — old brief messages saved BEFORE Session 37 kill were still on disk. File TTL only fires after 24h of NO chat activity; any chat use refreshes savedAt. NEW `stripBriefs()` helper in `lib/use-chat-persistence.ts` filters `m.isBrief || m.id?.startsWith('brief-')` on hydrate AND save. One-shot clears stale briefs from prior builds on first chat open post-install · **⭐ Chat empty-state greeting (Build 59, commits `bb2bb8e` + `c81a25e`)** — Rich: "chat can't have a blank screen, must have the greeting." `renderMessages()` now returns time-of-day greeting card when messages array is empty. Same 5-window palette as Home ChatTile → visual continuity whether user tapped tile or swiped right. Font sizes bumped to 17/26 Regular/Bold (matching chat message spec — first pass was 16/22 which is Home tile size, wrong context) · **⭐ Session 37 hotfix — brief-scheduler drift regen (created + reverted, commits `3ad3a96` + `48182fa`)** — first attempted `data_signature` check on `brief-scheduler` so tomorrow's late-landing events would trigger silent regen. Shipped when we thought client would still read the brief row via Home tile. After ChatTile pivot, no client reads the row live → drift regen serves no purpose → REVERTED. Scheduler back to fire-once-per-day idempotent · **⭐⭐ Cost investigation — Andy usage → imgCtx cache breakage identified → Build 60 backlog** — Andy re-engaged 17 Aug 9pm-10pm Brisbane, one session: 3 user messages with photos → 6 Sonnet API calls → A$0.25 AUD total (~A$0.08/message). Investigation via api_logs token-level query with real column names (schema missing `cache_read_tokens` + `cache_creation_tokens` — blind spot). Back-solved from cost + input/output tokens: cache IS working WITHIN message pairs (initial + followup) but BREAKS BETWEEN messages when photos change. Root cause at `app/(tabs)/index.tsx:6064` — `const staticCached = ${staticSystem}${imgCtx}\n\n${CAPABILITY_RULES}` puts `imgCtx` (image description, ~2000 tokens per photo, populated from `lastImageDesc.current`) INSIDE the cached prefix. New photo → different `imageDescription` → different `imgCtx` → different `staticCached` bytes → Anthropic cache miss → pays to write ~8700-token prefix again. **BUILD 60 BACKLOG (deferred by Rich — cost-only, invisible to users, no urgency at beta scale)**: (1) move `imgCtx` OUT of `staticCached` INTO `dynamicUncached` (~30% reduction on photo-heavy sessions, wash on text-only), (2) Sonnet 4.6 → Sonnet 5 ($3/$15 → $2/$10 intro through Aug 2026 = ~33% always) — free win, orthogonal, (3) NEW `supabase-api-logs-cache-cols.sql` migration — add `cache_read_tokens` + `cache_creation_tokens` columns for observability + update `lib/api-logger.ts` to persist them, (4) trim `CAPABILITY_RULES` (~5000 tokens → target ~3000, some ABSOLUTE HONESTY blocks are redundant), (5) OPTIONAL Sonnet follow-up → Haiku 4.5 for the reflection turn only (~25-30% overall reduction, persona quality trade-off). Combined: A$5.86/mo Rich testing baseline → target A$2.10/mo (70% margin at A$6.99) achievable · **⭐ Honest math correction** — earlier claimed 10× reduction from cache fix. WRONG. Real math: ~30% on photo-heavy sessions, 0% on text-only. Was mixing up "cache never hits" (false — hits on followups) with "cache never hits between messages when photos change" (true). Documented for future — verify cache behavior with real data before extrapolating · **⭐ Marketing kick-off — Instagram + Facebook pages, FB cover generator, 4-week content calendar** — Rich set up @zaeli.app IG + Facebook Page. NEW `zaeli-fb-cover-generator.html` (browser Canvas PNG generator, 820×312 + @2x, tagline + explainer variants — Rich generated + downloaded the tagline PNGs). NEW `zaeli-4week-content-calendar.html` — 12 posts × 3 platforms (IG/TT/FB Groups) × 4 weeks, **Mum POV throughout** (mums are the buyers — Rich course-corrected from initial "family POV" pivot), Australian-generic references (not Brisbane), mental-load themes. Weeks: Recognition → Curiosity → Reveal → Invite. Starts Aug 25 (Mon/Thu/Sun cadence). FB Groups strategy for stranger reach (bypass friends being weird about it). Bonus tabs: timing guide, hashtag library, FB groups list · **⭐ Andy engagement — briefly back, Nadia still not joined** — Andy came back 17 Aug 9pm-10pm Brisbane for one solid session (3 messages + 3 photo uploads = school newsletter multi-event add pattern). Server evening brief fired for him. Nadia invite from 14 Aug (named "Family" — probably fat-fingered) still pending 3 days later. Rich needs to nudge personally: "great to see you back — did you get Nadia set up?" — soft, actionable · **Prior Session 36 work still current** — Build 54 six-fix bug bash (reminders regression, iCal flicker, "?" avatar, welcome email misfire, per-user briefs, manual share iCal toggle). Session 35 iCal two-way sync + Home tile instant-load. **Pricing A$6.99/mo family plan tax-inclusive** (unchanged). **REMAINING**: Rich runs `eas build` + `eas submit` for Build 59 (currently pending) → Anna/Kara test → Andy re-engagement follow-up. Build 60 (cost fixes) fresh-session-tomorrow work. Marketing calendar posts start Aug 25.*

*Legacy Session 36 header (kept for context — Session 36 shipped Build 54 bug bash 17 Aug morning-afternoon): 17 August 2026 — Session 36 ✅ · **BUILD 54 BUG BASH — 6 REAL BETA-USER-FACING FIXES BUNDLED (widgets deferred).** Rich pivoted Build 54 from widgets to bug rectification after Session 35's iCal ship uncovered a cluster of real UX bugs. Six fixes shipped end-to-end: (1) Reminders regression, (2) iCal flicker + concurrent-sync trampling, (3) "?" avatar on Zaeli events + imported iCal events, (4) welcome email `resend_welcome` sending wrong template (Kara received "You're on beta list" AGAIN instead of "You're in — welcome"), (5) per-user brief architecture (Phase 85 promoted from queued — briefs now include personal iCal events), (6) manual share iCal event toggle. Investigations by 3 parallel Explore agents nailed root causes before touching any code · **⭐ Bug 1 — Reminders regression** — "Can you remind me to order hair product at 9am?" landed in the To-dos tab with the entire raw prompt as the title. THREE root causes: (a) `add_reminder` tool description at `app/(tabs)/index.tsx:1804` never taught Sonnet to strip conversational prefaces or interpret "at 9am" as implicit-today; (b) stale line 2636 still told Sonnet self-reminders go to `add_todo`; (c) tool_choice forced `add_reminder` but with untimed input → landed in reminders table with null remindAt/remindOn → rendered in To-dos tab (isTimed=false). Fix in 4 edits: expanded `add_reminder` description with preface-stripping rules + implicit-today example; expanded `add_todo` description to differentiate from reminders; swapped the stale "use add_todo" line to "use add_reminder"; defence-in-depth in `executeTool('add_reminder')` — strips prefaces client-side + auto-populates `remind_at` from "at Xam/pm" pattern when Sonnet omitted it. Belt-and-braces so this can NEVER regress silently again · **⭐⭐ Bug 2 — iCal event flicker** — events showed then disappeared then reappeared on every open. Root cause TWO stacked issues: (1) mount fired `syncNow` AND every AppState `background → active` transition ALSO fired `syncNow` with zero debounce or in-flight guard — concurrent syncs raced through the DELETE-stale step; (2) when `Calendar.getEventsAsync()` threw for any single calendar (recurring events like "Albert Lumines - Monthly 1:1" trigger transient EventKit read failures), the loop `continue`d and those events' `external_id`s never made it into `allExternalIds` — the DELETE step then wiped those rows from Supabase → next sync re-inserted → flicker. Fixes: (a) `lib/calendar-sync.ts` module-level `_syncInFlight = new Map<userId, Promise>` — coalesces concurrent calls per user, second caller reuses same promise; (b) `lib/calendar-sync.ts` per-calendar `readSucceeded` tracking — if ANY calendar read fails, SKIP the delete-stale step entirely (better to have one stale row for one sync than yo-yo flicker); (c) `app/_layout.tsx` 5-second debounce on AppState foreground syncs — collapses rapid Control Centre / Notification Centre / camera dismiss bounces to one sync · **⭐⭐ Bug 3 — "?" avatar** — TWO merged causes: (a) imported iCal events set `assignees=[]` by design (Rich's rule — don't stamp "R" on every work meeting) but Home tile avatar renderer at `dashboard.tsx:504-513` had no branch for `source==='apple-ical'` → fell through to `.charAt(0)` of undefined → "?"; (b) `loadRoster()` is async and fires from `_layout.tsx` in the background — between splash-hide and roster hydration, `getRoster()` returns `[]` (Round B removed DEFAULT_ROSTER) → every event's avatar renders "?" during the roster race. Additionally, duplicate-family_members cases (Session 35's ensureOwnMembership name-match guard leaves the legacy row's mismatched id in place) mean historical events reference stale UUIDs that NEVER resolve → permanent "?". Fixes: (a) dashboard.tsx avatar render branches — `isExternal` → iPhone icon in avatar slot; roster empty → subtle placeholder circle; roster loaded but no match → default-tinted circle (never paint "?"); (b) NEW `supabase-repair-stale-assignees.sql` — dry-run diagnostic first (per-family count + 20-event sample with valid+stale ID split), then commented UPDATE that rewrites affected events' assignees arrays to keep only valid UUIDs. Idempotent, safe to run multiple times · **⭐⭐ Bug 4 — Welcome email `resend_welcome` misnamed action** — Kara installed the app 16 Aug, went from awaiting-install to active. Instead of receiving the auto-drip "You're in — welcome to Zaeli" (day-0), she received "You're on the Zaeli beta list" AGAIN at 7:31pm. Rich manually sent the correct template at 8:36pm via admin-announce. Root cause: the `resend_welcome` action in `supabase/functions/admin-actions/index.ts:440-465` called `beta-notify` (which is HARDCODED to send the beta-list confirmation email — the WRONG template). Admin console button label said "Resend welcome email" but the wiring was a lie. Also `beta-notify` doesn't write to `email_log` so the misfire was invisible in the audit trail. Fixes: (a) rewired `resend_welcome` to call `sendEmailInline({templateId:'welcome', triggeredBy:'resend', force:true})` — writes to email_log; (b) added `force` param to `sendEmailInline` that bypasses dedup (normal grant/invite keeps dedup, but manual resend must be able to fire a template already sent); (c) NEW `resend_beta_list_confirmation` action for the RARE case of actually wanting to re-send the pre-install confirmation email; (d) admin console UI relabelled with two distinct buttons — "Resend welcome" (post-install) + "Resend beta-list" (pre-install), each with clear description text; (e) NEW `supabase-kara-email-backfill.sql` — inserts a `template_id='welcome'` row into Kara's email_log with `triggered_by='manual'` + Rich's 8:36pm send time. Prevents any future auto-retry from double-sending. Day-3 checkin will fire correctly regardless (keys off beta_start_date + separate template_id dedup) · **⭐⭐⭐ Bug 5 — Per-user briefs (Phase 85 promoted from queued)** — brief-scheduler generated ONE brief per family per window per day. RLS-scoped personal iCal events (`privacy_scope='personal'` + `imported_by_user_id=X`) were INVISIBLE to the shared brief — Andy sees "clear day" while his iPhone Calendar is full of work meetings. Trust-breaking. Phase 85 (per-user brief cache) was queued for scale-past-100-families but promoted to Build 54 because iCal sync makes this immediately visible to real users. NEW `supabase-zaeli-briefs-per-user.sql` — adds `user_id` column (nullable → backfill from family owner → NOT NULL), drops old unique constraint `(family_id, date_key, time_window)`, adds new `(family_id, user_id, date_key, time_window)`. `supabase/functions/brief-scheduler/index.ts` REWRITTEN: iterates adult profiles (not families), each user gets their own brief candidate based on their `user_preferences` brief times, `gatherLiveData` uses `.or()` to query family-shared events + THIS user's personal iCal events in one call, upserts with user_id, `sendUserPush` sends to just that user's token. Also `lib/brief-generator.ts` (client-side chat brief) updated to accept `userId` param + filter cache SELECT/UPSERT by it. Caller in `app/(tabs)/index.tsx` passes `getProfile()?.id`. Also fixed original Edge Function bug where upsert used `text:` column but schema is `brief_text` (silent server-side upsert failures for weeks — client-side was the only path actually writing rows). Cost: 2× briefs per family with 2 adults; negligible at beta scale · **⭐ Bug 6 — Manual share iCal event toggle** — In CalSheetEventCard's expanded view for imported iCal events (`source === 'apple-ical'`): new mint toggle "Share with family" (default off = personal, tap = shared). Flips `privacy_scope` between 'personal' and 'shared' via one Supabase UPDATE. Optimistic UI + revert on error. `lib/calendar-sync.ts` also updated to PRESERVE `privacy_scope` on subsequent iCal syncs (strip it from the UPDATE row, only INSERT sets default 'personal' — otherwise the next sync would reset the user's manual share) · **Prior Session 35 work still current** — Full iCal two-way sync SHIPPED (Builds 49→52) + Home tiles instant-load (Build 53) + api_logs RLS unblocked + website calendar sync section + family_members duplicate cleanup + name-match guard. Session 34 (same day) shipped Builds 47 + 48. Session 32 v2 vision pivot to trio + Reminders as 4th pillar. **Pricing A$6.99/mo family plan tax-inclusive** (unchanged). **REMAINING**: Rich runs `supabase-zaeli-briefs-per-user.sql` (per-user briefs schema) + reviews + applies `supabase-repair-stale-assignees.sql` (dry-run first) + runs `supabase-kara-email-backfill.sql` + `supabase functions deploy admin-actions` + `supabase functions deploy brief-scheduler` + admin console push to Netlify + EAS production build for Build 54 + `eas submit`. Widget work back in queue for Build 55. Sonnet 5 migration still queued.*

*Legacy Session 35 header (kept for context — Session 35 shipped Builds 49→53 same-week predecessor): 15 August 2026 late — Session 35 ✅ · **ICAL TWO-WAY SYNC LIVE (Builds 49→52) + HOME TILES INSTANT-LOAD (Build 53) + API_LOGS RLS UNBLOCKED (3 days silent) + WEBSITE CALENDAR SYNC SECTION SHIPPED + BUILD 54 SCOPED (widgets + long-press shortcuts + share personal event, DEFERRED to fresh session).** Sprint day — 6 EAS builds shipped through 47→48→49→50→51→52→53. Rich holding Build 54 for tomorrow to avoid tired-legs native iOS work. Andy dormant since 14 Aug 21:14 (last api_log — but api_logs had been broken from ~12 Aug so real usage may be higher; Rich sent re-engagement text with iCal sync screenshot). Damien Android never installed the sideload APK (never signed up). Nadia invite pending 2 days unaccepted · **⭐⭐⭐ Full iCal two-way sync SHIPPED (Builds 49→52)** — Andy asked, we shipped it end-to-end in one day. Scope locked Session 34 (per-user external sync, opt-in per calendar, full two-way not phased). Build 49 = foundation + read-in: `supabase-calendar-sync.sql` added `events.source/external_id/external_calendar_id/imported_by_user_id/privacy_scope/synced_at` columns + new `calendar_sync_config` table + RLS enforces `privacy_scope='personal'` only visible to `imported_by_user_id`. NEW `lib/calendar-sync.ts` — permission gate, per-calendar picker, syncNow. NEW Settings CalendarSyncView — full opt-in-per-calendar picker + Sync now + disconnect. Build 50 = write-back: `supabase-calendar-writeback.sql` added `events.mirrored_apple_id`, NEW `ensureZaeliCalendar()` creates a "Zaeli" EventKit calendar (loose iCloud source matching — strict CALDAV+name failed silently), NEW `mirrorZaeliEventsToEventKit()` full reconciliation INSERT/UPDATE/DELETE. Build 51 = timezone HOTFIX — Postgres +00 UTC timestamps were being parsed literally by iOS, mirroring events +10 hours off (7am Brisbane → 5pm iPhone). Fix in `parseIsoAsDate` (mirror of reminders' `parseLocalIsoAsDate`): strip tz suffix, construct Date from local components. Build 52 = onboarding step + update/delete propagation + white-on-white DateTimePicker fix (3 places missing `themeVariant="light"` — added to `onboarding/index.tsx:862`, `index.tsx:8958`, `our-budget.tsx:1602`) + auto-remediation via `MIRROR_SCHEMA_VERSION` (bumps → forces one-time full re-mirror on app open). Real cross-family leak fix along the way: old duplicate events RLS policies from Session 21 (`events_select_family` etc) plus my Build 48 additions were BOTH permissive → OR wins → Anna would have seen Rich's personal events. Dropped the 4 old policies · **⭐⭐ api_logs RLS INSERT policy MISSING — 3 days of blind flying FIXED** — Session 33's admin console SQL enabled RLS on api_logs but only created SELECT policy for admins@zaeli.app. Since ~12 Aug, EVERY `logApiCall()` insert has been silently rejected. Discovered when checking "did Andy use app today?" and admin console showed zero rows. Fix: `CREATE POLICY "api_logs insert authenticated" ON public.api_logs FOR INSERT WITH CHECK (family_id = public.current_family_id())`. Lesson: enabling RLS without policies = deny-everything (same Session 21 lesson). Whenever adding RLS to a table with existing writes, VERIFY policies exist before enabling · **⭐ Home tiles instant-load — cache-then-refresh pattern (Build 53)** — Rich reported 2-3 second blank tile flash on every app open (poor UX, felt broken). Root cause: dashboard `useEffect` fired network fetch → tiles rendered empty state during fetch → then jumped to real data. NEW `lib/home-cache.ts` — per-family per-tile AsyncStorage cache (`loadTile`/`saveTile`/`clearFamilyCache`/`clearAllHomeCache`/`saveLastFamilyId`/`getLastFamilyId`). Dashboard now: (1) mount → `getLastFamilyId()` → hydrate every tile from cache in parallel → paint in ~50ms. (2) background fetch fresh from Supabase → `saveTile()` → silently update state if changed. `saveLastFamilyId(fid)` written after every successful fetch so NEXT cold-start knows which family's cache to read (profile isn't loaded yet during Session 30 splash-latency fix flow). Cache cleared on sign-out via `onAuthChange` in `_layout.tsx` (`clearAllHomeCache()` — safer than needing to know which family was signed in). Also added AppState listener foreground sync so returning from background triggers iCal + tile refresh · **⭐ Family_members duplicate cleanup + name-match guard** — After Build 47's `ensureOwnMembership()` shipped, some accounts ended up with DUPLICATE roster rows (Richard+Richard, Anna+Anna). Root cause: `ensureOwnMembership` INSERTed when it couldn't find a row keyed by auth.uid, but the earlier row may have been a name-match with a different id. Fix in `lib/family-roster.ts`: before INSERT, also check for existing row with same NAME in family — if found, patch its id to auth.uid instead of inserting new. Idempotent, prevents future dupes · **⭐ Website — iPhone Calendar sync section added** — `zaeli-app-links-template/public/index.html` — NEW Section 3.5 "iPhone Calendar sync" between Lockscreen and Reminders (mint bg, phone mockup showing calendar picker + Sync now button + "Zaeli events appear in iPhone. iPhone events appear in Zaeli."). Positioned as headline feature. Privacy page updated with iPhone Calendar events disclosure bullet. Deployed via Cloudflare Pages auto-deploy (Rich forgot last enter on cmd first time; pushed successfully on second try) · **⭐ Widgets + long-press icon shortcuts + share personal event manually — SCOPED for Build 54, DEFERRED to fresh session** — Rich revisited widgets tonight. Locked scope: (1) Small Home widget = mic launcher (tap → opens chat with mic active). (2) Medium Home widget = today's events + next reminder. (3) Lock Screen circular = Zaeli logo (tap → opens app). (4) Lock Screen inline = one-line today summary text. Also: long-press Zaeli app icon → 4-action menu (Start with voice / Today / Shopping / Quick add) via Info.plist `UIApplicationShortcutItems` + `zaeli://` URL scheme routing. Also: share personal event manually — toggle `privacy_scope` from `personal` → `shared` in event edit sheet (pure JS, one Supabase UPDATE). Foundation packages installed tonight: `@bacons/apple-targets` (Expo config plugin for native iOS widget target) + `react-native-shared-group-preferences` (writes to iOS Shared App Group UserDefaults — swapped from `expo-shared-preferences` which was Android-only). Widget work involves proper native Swift/SwiftUI/WidgetKit — high risk of first-try iteration cycles. Rich chose to defer to fresh session tomorrow · **Andy engagement diagnosis** — Andy's last api_log was 14 Aug 21:14 (though api_logs was broken from ~12 Aug so real last use may be later). Never sent Nadia her invite. Silence for 24+ hours. Rich sent direct text with iPhone calendar sync screenshot to spark re-engagement. If Andy comes back and uses iCal sync, that's the killer feature that keeps him. If not, next re-engagement is a personal check-in from Rich · **Prior Session 34 work still current** — Build 47 (Andy Add Event RLS fix, Option C member editing, ensureOwnMembership, cross-family name leak fix moved from staticSystem to dynamicSystem, anti-snowball rules in CALENDAR HONESTY). Session 32 v2 vision pivot to family productivity trio + Reminders as 4th pillar + Home is front door + Kids Hub/Tutor/Travel/My Space/Meals in `_hidden` + server-side brief scheduler + sheet-over-Home + MoreSheet 6 full-width tiles + flat Expenses model. **Pricing A$6.99/mo family plan tax-inclusive** (unchanged). **REMAINING**: Build 54 widget + long-press + share event work (fresh session tomorrow). Sonnet 5 migration still queued. Andy engagement watch.*

*Legacy Session 34 banner folded — Build 47 + 48 work covered above. Full session detail below in SESSION 34 section.*

*Legacy Session 34 summary (15 August 2026 — first sprint) — Same-day predecessor to Session 35. Session 34 shipped Builds 47 + 48: (Build 47) Andy Add Event RLS bug at `app/(tabs)/index.tsx:2979` inline DUMMY_FAMILY_ID literal → `awaitFamilyId()` async helper; Option C Our Family member profile fully editable (`supabase-family-editing.sql` + `EditNameSheet` + `ColorPickerSheet` + real Remove-from-family); admin-announce SMTP false-failure fix (`EdgeRuntime.waitUntil` background send, return 200 immediately); brief-scheduler dormant-gate hole (fresh-signup exemption for profiles <7 days old). (Build 48) Andy missing from own Family list (`ensureOwnMembership()` + proactive `_layout.tsx` auto-heal); cross-family name leak in Sonnet prompt (Poppy/Anna appeared in Andy's chat — moved FAMILY line from Anthropic-cached `staticSystem` to `dynamicSystem` built at prompt-build time from `getRoster()` + `getProfile()`); anti-snowball rules added to CALENDAR HONESTY (NO META-COMMENTARY ON FAILURES / call find_calendar_events FIRST / recurring defaults). Recurring event code verified clean — Andy's Netball bug was prompt drift not code. iCal sync roadmap scoped (per-user, opt-in-per-calendar, full two-way, 3 builds). Android path scoped (fast-track APK for Damien via 2 admin console email templates — Waitlist + Back door). Session 35 (immediately after) shipped the entire iCal roadmap.* Andy (nortonata@hotmail.com) is second real beta user after Anna, hitting real bugs. Andrew Fraser awaiting install 3+ days. Damien Pagnin (Rich's cousin) signed up but is Android — Rich decided to fast-track him via direct-install APK while proper Google Play Store path builds in background · **⭐ Admin-announce false-failure FIXED (Batch 9)** — `HTTP 546` with `CPU Time exceeded` on Zoho SMTP path meant client saw "failed" while email actually delivered. Root cause: `await client.send()` burned enough CPU (TLS handshake + DATA transmission) to trip Supabase's ~200ms budget before response could ship. Fix: return 200 immediately, run SMTP in background via `EdgeRuntime.waitUntil`. Trade-off: lose ability to report SMTP failure to client (rare — Zoho is stable). Real audit trail belongs in `email_log` — deferred · **⭐ Brief-scheduler dormant-gate hole FIXED** — Session 33's Batch 6 added "skip families with no api_logs in last 7 days" gate to save Sonnet cycles on abandoned accounts. But brand-new users have zero api_logs BY DEFINITION → their first-ever morning brief never fires → the exact re-engagement hook we wanted for fresh signups is blocked. Fix: any family with a profile created <7 days ago is "fresh" and exempt from the dormant gate. Andy will now get his first morning brief tomorrow · **⭐⭐ Andy Add Event RLS bug FIXED (Build 47)** — Session 21's `FAMILY_ID` constant sweep replaced explicit references, but MISSED an inline string literal at `app/(tabs)/index.tsx:2979` in the calendar sheet Save handler. Every sheet-added event has been trying to INSERT with `family_id: '00000000-0000-0000-0000-000000000001'` (DUMMY_FAMILY_ID). Rich's chat-tool adds worked (they use `getFamilyId()`) — nobody noticed. Andy hit it first Add Event → RLS violation surfaced ("new row violates row-level security policy for table 'events'"). Fix: `awaitFamilyId()` async helper in `lib/family.ts` — blocks briefly (polls every 60ms, max 4s) waiting for profile load, throws with friendly error instead of falling back to DUMMY · **⭐⭐⭐ Option C — Our Family member profile fully editable (Build 47)** — Andy discovered "nothing tappable" on member profile screens (name/colour rows read-only, "Remove" fired placeholder Alert). Full backend + client fix: NEW `supabase-family-editing.sql` (profiles.budget_access column for kid Budget toggle persistence + tighter family_members UPDATE RLS to owner-OR-self + tighter DELETE RLS to owner-only + `remove_family_member(target_id)` SECURITY DEFINER RPC that deletes family_members row + nulls profiles.family_id to revoke RLS access). NEW helpers in `lib/family-roster.ts`: `updateMemberName` / `updateMemberColor` / `removeMember` / `getBudgetAccess` / `setBudgetAccess`. `family.tsx` UI: EditNameSheet (text input modal) + ColorPickerSheet (8-swatch palette Blue/Coral/Purple/Green/Amber/Magenta/Teal/Slate) + tap-to-edit Name/Colour rows + persisted kid Budget toggle + real Remove-from-family with confirmation · **⭐⭐⭐ Andy missing from own Family list — HOTFIX (Build 48)** — Andy sees his 3 family members (Nadia adult, Neave Y5, Louie Y3) but himself absent. Root cause at `app/onboarding/index.tsx:391`: `family.filter(m => m.id !== 'me')` explicitly excludes the owner from family_members insert. Comment justified as "family_members is for OTHER family members" — WRONG. Every screen (Our Family, Calendar avatars, Meal cook picker) treats family_members as THE authoritative roster. Fix in three parts: (1) NEW `ensureOwnMembership()` helper in `lib/family-roster.ts` — checks if signed-in user has a family_members row keyed by auth.uid, INSERTs one from profile data if missing, idempotent. (2) `family.tsx` load auto-heal fires when user opens Our Family. (3) `_layout.tsx` proactive heal fires on every app open — before user even navigates. (4) Onboarding fix: removed the filter, owner inserted with `id = profile.id` matching auth.uid. (5) `invite/[token].tsx` finishAdult + finishKid both call `ensureOwnMembership()` after signup. Invariant now enforced: `family_members.id === profiles.id === auth.uid()` for the person themselves · **⭐⭐⭐ Cross-family name leak in Sonnet system prompt FIXED (Build 48)** — Andy's Zaeli asked "who's it for (Poppy? Anna?)" for his Netball event — RICH'S family names appeared in Andy's conversation. Root cause at `app/(tabs)/index.tsx:4676`: hardcoded `FAMILY: Rich (logged in), Anna, Poppy (Yr6, age 12), Gab (Yr4, age 10, boy), Duke (Yr1, age 8, boy).` line INSIDE the cached static system prompt. Every user of the app got Rich's family names injected into Sonnet's context. Fix: moved FAMILY line from `staticSystem` (Anthropic-cached at 90% discount) into `dynamicSystem` (uncached), built at prompt-build time from `getRoster()` + `getProfile()`. Also genericized hardcoded example names ("Poppy's pasta" → "The bolognese"), removed hardcoded "Rich" reference ("Never tell Rich to do things" → "Never tell the user..."). Same fixes in `lib/brief-generator.ts` (6 hardcoded name references replaced with placeholders + explicit "USE ONLY REAL FAMILY NAMES FROM PROVIDED FAMILY LIST" rule) and `supabase/functions/brief-scheduler/index.ts` (hardcoded "Duke's soccer" → "<kid>'s soccer") · **⭐ Anti-snowball rules added to CALENDAR HONESTY (Build 48)** — Andy saw Zaeli say "If it still doesn't show up, that's the calendar doing its little vanishing act again" — Sonnet snowballing on past failed turns, inventing external causes to avoid responsibility. Three new rules: (a) NO META-COMMENTARY ON FAILURES — explicit ban on "vanishing act", "seems flaky", "sounds like it never landed" — each turn starts CLEAN, ignore past-turn drama. (b) WHEN USER ASKS "DID YOU ADD X?" — call find_calendar_events FIRST, don't guess. (c) RECURRING DEFAULTS — title + weekday + time is enough, don't over-ask clarifying questions · **⭐ Recurring event code verified clean (Build 48)** — traced `generateRecurrenceDates` + batch insert path in `app/(tabs)/index.tsx`. Enum-locked schema forces Sonnet to pick from `daily/weekdays/weekly/fortnightly/monthly`. Silent-RLS block detection via `.select('id')` returns TOOL_FAILED if 0 rows land. 12-month horizon, 400-row cap, batched inserts. Andy's Netball bug was PROMPT drift (Sonnet over-asking then meta-commenting) not CODE — the anti-snowball rules address it · **iCal sync — full two-way roadmap SCOPED, foundation SQL READY, code deferred to Build 49+** — Andy asked for two-way sync between iPhone Calendar and Zaeli. Decisions locked: per-user external sync (Andy's iPhone events show only for Andy in Zaeli, Anna doesn't see his work meetings), opt-in per calendar (picker for his 8+ iOS calendars), full two-way (not phased). Roadmap 3 builds: Build 49 (foundation + read-in), Build 50 (write-back), Build 51 (reconciliation + Android parity). NEW `supabase-calendar-sync.sql` DRAFTED (not yet run) — adds events.source/external_id/external_calendar_id/imported_by_user_id/privacy_scope/synced_at columns, new calendar_sync_config table (per-user JSONB of selected iOS calendars + Zaeli-owned calendar id), updated events RLS to enforce privacy_scope=personal only visible to imported_by_user_id · **Android path SCOPED** — Damien (Rich's cousin) signed up but has Android. Decision: fast-track via direct-install APK (EAS build → download link → sideload) for close family only. Public Android waitlist waits 6-8 weeks for proper Google Play Store internal testing track. NEW 2 admin console email templates: 🤖 Android waitlist (generic, 6-8 weeks holding line) + 🚪 Android back door (fun conspiratorial invite for close family/friends, honest about scary "install from unknown sources" warning + no push notifications yet + manual reinstall for updates + guinea pig role). Both templates match existing lifecycle email branding (Zaeli wordmark, Rich sign-off) · **Prior Session 32 v2 work still current** — v2 vision pivot to family productivity trio + Reminders as 4th pillar, Home is front door, Kids Hub/Tutor/Travel/My Space/Meals in _hidden folder, server-side brief scheduler, sheet-over-Home pattern, MoreSheet 6 full-width tiles, flat Expenses model in Budget. **Pricing A$6.99/mo family plan tax-inclusive** (from Session 32 v2, unchanged). **REMAINING**: verify Andy sees himself in family list post-Build-48 install, verify Andy no longer sees Rich's family names in chat, verify recurring events work now that anti-snowball rules landed. Then iCal sync Build 49+ work resumes. Sonnet 5 migration still queued.*

*Legacy session summary (17 July 2026 — Session 30 ✅ (extended) · PHASE 5 API KEYS SERVER-SIDE + STRIPE LIVE MODE + OUR BUDGET BACKEND + ANNA ROUND 2 · **⭐⭐⭐ PHASE 5 SHIPPED** (commit `e84a091`) — Anthropic/OpenAI/Whisper keys were bundled in the app via `EXPO_PUBLIC_*` vars (extractable from compiled binary). 3 NEW Supabase Edge Functions (`anthropic-proxy`/`openai-proxy`/`whisper-proxy`, all JWT-verified). NEW `lib/ai-proxy.ts` with `callAnthropic`/`callOpenAI`/`callWhisper`. Migrated every active call site in `lib/api-logger.ts` (huge leverage — covers brief-generator + tutor-summaries transitively), `index.tsx`, `dashboard.tsx`, `tutor-session/practice/reading.tsx`, `kids.tsx`, `our-budget.tsx`, `calendar.tsx`, `lib/zaeli-memory.ts`. Rich removed keys from EAS + local `.env`; `ANTHROPIC_API_KEY` + `OPENAI_API_KEY` now live server-side as Supabase secrets · **⭐ STRIPE LIVE MODE ACTIVATED** (commits `58285e91`/`3101f65`/`582091f`) — Rich completed KYC (ABN, banking, ID). Live products created (Zaeli Family Plan A$9.99/mo tax-inclusive + Tutor Add-on A$7.99/mo tax-inclusive), live Payment Link with 14-day free trial (card required upfront) `https://buy.stripe.com/fZubJ36EM1QR2z6eBl83C01`, live webhook registered (6 events, new whsec_), live Customer Portal (return `https://zaeli.app`). `lib/stripe.ts` + webhook `PRICE_TO_PLAN` map updated with live Price IDs; sandbox kept as backwards-compat fallback. Settings Subscribe button "Subscribe · A$9.99/mo" → "Start free 14-day trial". `STRIPE-LIVE-SETUP.md` 244-line walkthrough shipped · **⭐ OUR BUDGET SUPABASE-BACKED** (commit `f2974e4`) — NEW `supabase-budget-tables.sql` (4 tables, Session 21 RLS pattern, ON DELETE CASCADE line items). NEW `lib/budget.ts` (loadBudget + 8 CRUD helpers + uuidv4 export). `our-budget.tsx` — SEED_ constants removed, useState([]) + hydration effect, all saves/removes fire-and-forget DB persist. Session 17 built as UI-only, deferral never picked up — every restart wiped data + Anna couldn't see Rich's edits. Bonus: AI averaging bug fixed — Sonnet was fumbling multi-image math, now schema requires `total_seen` + `months_analysed`, app computes average client-side · **⭐ ANNA ROUND 2 FIXES** (commit `c8ef80c`) — Anna uploaded 3 parent-teacher interview photos, only Gab's event landed but Zaeli confidently fabricated "Poppy's + Duke's are in the system, neatly stacked like a very efficient school morning." Three root causes: (a) vision-describe prompt asked for "2-3 sentences" — catastrophic for multi-event photos, rewrote to demand numbered enumeration + max_tokens 800→1500, (b) MULTI-EVENT PHOTO UPLOADS rule added to CAPABILITY_RULES (emit N tool calls, honestly report success/fail counts), (c) CALENDAR HONESTY — ABSOLUTE rule mirroring send_family_message pattern (NEVER claim added/booked without ✅ tool response). Separate fix: chat text render strips `**bold**` markdown (RN Text has no bold renderer for `**`, was showing literally) · **Whisper English lock + send_family_message honesty** (commit `ff5bfd6`) — Anna beta 1 mystery test: Zaeli responded in WELSH to English input because Whisper hallucinated Welsh transcription. Fix: append `language: 'en'` to all 9 Whisper call sites. Second bug: Zaeli said "sent it" for a family message when Edge Function showed zero invocations — Sonnet fabricated. Fix: ABSOLUTE HONESTY RULES added to send_family_message · **Removed stale "CANNOT send messages" capability rule** (commit `c8db27f`) — leftover Session 15-ish rule contradicted send_family_message tool. Rule now distinguishes: no SMS/iMessage/email (true) but she CAN and SHOULD send family push via send_family_message · **Dev row gate + GDPR/Terms pages + welcome email doc** (commit `96e485d`) — Settings Developer section gated to `p.profile?.email === 'richarddekretser@gmail.com'`. NEW `zaeli-app-links-template/public/privacy.html` + `terms.html` deployed to zaeli.app (Australian Privacy Act + AI processor disclosure). Settings Privacy/Terms rows Linking.openURL. NEW `SUPABASE-EMAIL-SETUP.md` (Zaeli-branded auth email templates) · **Verbose debugPushToken diagnostic fix** (commit `250aa87`) — per-step try/catch + fixed MISSING `await getCurrentUserId()` (was called without await — since getCurrentUserId is async, was returning Promise object, not string). Added typeof-check on `Notifications.getPermissionsAsync`/`getExpoPushTokenAsync` at step 0 so we know immediately if expo-notifications native module didn't link · Prior 14 July Session 30 (Family push infra + Multi-photo chat + Trivia guard + Splash latency + expo-notifications plugin discovery) still current — remaining: production EAS build with all today's work, `eas submit --platform ios --latest` to TestFlight, Anna beta round 3 with everything live, live Stripe testing (REAL MONEY warning — test with own card + refund), Sonnet 5 migration (queued), per-user brief cache (queued past 100 families), website v5 (parked), Outlook/Gmail sync (parked) · **Family push infra shipped** (commit `8bbf7e3`) — `supabase-push-tokens.sql` adds `profiles.expo_push_token`, Edge Function `supabase/functions/family-notify/index.ts` (JWT-verified, family-boundary-enforced, batches to Expo push API), `lib/notifications.ts` `registerPushToken`/`unregisterPushToken`/`notifyFamily`, `_layout.tsx` fire-and-forget register after auth, `index.tsx` Notify chip on calendar confirm cards + `send_family_message` tool with fuzzy name matching + "family"/"everyone" shortcuts. Plus Spend Processing overlay (full-screen dark backdrop + ActivityIndicator + "Processing..." status label, silent success, Alert on error) + tutor Play icon removed (matches home chat) · **Multi-photo chat + Stripe testability + foreground refresh** (commit `e6c8ea6`) — `pendingImage: string | null` → `pendingImages: string[]` with `MAX_CHAT_PHOTOS = 4` cap, additive Camera/Photos pickers, horizontal thumbnail strip with per-photo ✕, `send(text?, overrideImages?: string[])`, ONE Sonnet call for ALL images with multi-photo aggregation prompt, `Msg.imageUris?: string[]` alongside legacy `imageUri`, horizontal grid render for 2+ photos (110×110). Stripe: Developer row "💳 Test Stripe checkout" force-opens Payment Link even when beta grant hides Subscribe button. Foreground refresh: `_layout.tsx` AppState listener reloads profile on background→active transition (Stripe checkout return path — user completes payment in Safari, returns to app, subscription card auto-updates) · **Trivia answer-in-question guard + cold-start splash latency fix** (commit `34b57eb`) — Duke saw "Which sport is played with a round ball and goals, and is called soccer in many countries?" answer "Soccer". Three-layer defence: prompt rule with BAD/GOOD examples + client-side word-boundary regex filter (4+ char answers, drops self-revealing). Splash: 3s splash on foreground return after iOS jetsammed app. Root cause: `await loadProfile()` blocking `setAuthed` in cold-start useEffect (Supabase network 1-2s decent, 3s+ flaky). Fix: setAuthed immediately once session verified (fast AsyncStorage), kick loadProfile background. Route guard already tolerates null profile. Cuts splash ~2s · **⭐ CRITICAL DISCOVERY: `expo-notifications` plugin was missing from `app.json` plugins array** (commit `6eab45a`) — without it, iOS never gets `aps-environment` entitlement → `getExpoPushTokenAsync` throws at boot → the `.catch(() => {})` in `_layout.tsx` swallowed silently → `profiles.expo_push_token` stayed null forever. Local scheduled notifications (briefs) still worked because they don't need APNs — that's what confused diagnosis. Fixes: added expo-notifications plugin, replaced silent catch with `.catch(e => console.log(...))`, added Developer row "🔔 Register push token now" that manually triggers + shows result via Alert · **Verbose push diagnostic** (commit `dcb263b`) — `debugPushToken()` returns structured state (step name, detail, userId, permission, projectId, token snippet, dbWrite outcome) so on-device Alert shows exactly where the failure is · **EAS Starter plan upgrade** (14 iOS builds/month) — free tier build cap hit mid-session. `eas build` and `eas submit` are separate commands (confused Rich twice) · **GIPHY key discovery** — Kids Hub rewards GIFs failing in standalone because `EXPO_PUBLIC_GIPHY_API_KEY` in local `.env` never added to EAS Environment Variables (same Session 27 pattern) · Prior Session 29 (Anna beta round 1 + API cost fix + meal pivot) + Session 28 (Stripe end-to-end) + Session 25 (Universal Links LIVE) all still current — remaining: verbose diagnostic build to nail push failure step, GIPHY EAS env var, website v5 rewrite (parked), Anna beta round 2 (awaiting successful push build), Sonnet 5 migration (still queued), per-user brief cache (still queued), **Phase 5 (BEFORE PUBLIC LAUNCH) — move Anthropic/OpenAI keys server-side (client-bundled keys extractable)*** (commit `92a35d4`) — Tutor math accuracy via three-layer defence (anti-sycophancy prompt rules + hidden `<expected>N</expected>` marker Sonnet emits + LAST_EXPECTED_ANSWER injection on next call — fixes "12 is wrong for 84÷7" and "360 is wrong for 15×24"), pantry data injected into chat context + PANTRY RULES prompt block (Zaeli stops saying "not in pantry" for items IN pantry), pantry "yesterday" date bug fixed (midnight-normalise both dates before diffing), multi-photo receipt scan (up to 4 photos batched into ONE Sonnet call), in-sheet scan UX (progress banner + result toast — no more nav-to-Chat handoff), Dashboard reshuffled Calendar → Shopping → Bento → Radar → Meals (honest usage priority), "Welcome In" → "Welcome!" · **Onboarding gate + polish** — route guard bounces new owner accounts to /onboarding via AsyncStorage `onboarding_complete` flag, grandfather clause skips profiles >24h old (Rich pre-gate signup safe), sign-in defaults to sign-up mode (new users vastly outnumber returning), Stripe "Coming soon" alert cleared on Manage subscription (commit `916f2ab`), dynamic user name in brief prompts via `${primaryUser}` template replacing 7 hardcoded "Rich" spots, computeSignature includes primaryUser so cached briefs don't leak names across family members · **API cost fix** (commit `0b65420`) — negative-cost bug in `lib/api-logger.ts` (was subtracting cache_read + cache_write from input_tokens which is ALREADY the uncached portion), Haiku 4.5 pricing corrected $0.25/$1.25 → $1.00/$5.00 (4× too low), Sonnet 5 + Opus 4.8 pricing added for future migrations, prompt caching wired onto Sonnet tool-calling path in `send()` (both initial call AND previously-unlogged follow-up call — real cost was 2× what api_logs showed), expected impact home_chat A$0.02-0.04 → A$0.005-0.01 per repeat-turn call — at ~250 chats/mo/family that's A$3.30 → ~A$0.50/family/mo (loss-making → 60% margin) · **Meal planner pivot (uncommitted)** — Anna feedback: meal planning daily is friction no busy family sustains; recipes + regulars is where the real value lives. DinnerCard label "MEAL PLANNER" → "TONIGHT'S MEALS", empty-state CTA "See family regulars →" (not "Plan it →"), tab order Meals/Recipes/Favourites → Recipes/Regulars/Plan (default opens Recipes). buildContext queries recipes WHERE 'favourite' IN tags → adds "Family regulars" line to chat LIVE DATA (12 hearted recipes). New MEAL IDEAS RULES prompt block: Zaeli reaches for regulars FIRST on "quick dinner?" queries · **Calendar max_tokens bump (uncommitted)** — Anna's 10-event paste dropped the last event; api_logs showed `output_tokens: 800` exactly (the ceiling), each add_calendar_event tool_use block ~80 tokens, 10 × 80 + preamble ≈ 800 → truncated mid-block → malformed JSON silently dropped. Fix: max_tokens 800→2000 (initial) and 500→1500 (followup) · **Admin console Session 29 refresh** — moved source-of-truth from Downloads to `app/(tabs)/zaeli-admin/index.html`. Wordmark accent yellow → sky blue #A8D8F0, model ID claude-sonnet-4-20250514 → claude-sonnet-4-6, price sliders A$9.99/A$7.99, per-family margin card replaces nonsensical break-even, real subscription state in Accounts table (beta_end_date + subscription_status), memory system health card, double-$ display bug fixed (13 rows) · **Stripe beta grant infrastructure** — profiles.beta_end_date column, lib/stripe.ts helpers (getSubscription with beta override, isFamilyInBeta), family-roster.ts beta override (kids get tutor_active=true while family in beta), TutorSidebar uses isFamilyInBeta to unlock locked children · Prior Session 28 (Stripe end-to-end + calendar phantom fix + splash saga) + Session 27 (app icon 2B, first preview build, brief bucket-check + dedup fixes) + Session 25 (Universal Links LIVE) all still current — remaining: build #15 (holding to bundle more Anna feedback), Sonnet 5 migration (intro pricing $2/$10 through Aug 2026 makes it CHEAPER than 4.6), per-user brief cache when scaling past 100 families, TestFlight submission, **Phase 5 (BEFORE PUBLIC LAUNCH) — move Anthropic/OpenAI keys server-side (client-bundled keys extractable)***

---

## ══════════════════════════════════
## SCREEN ARCHITECTURE — CRITICAL (REVISED Session 32 v2 ✅)
## ══════════════════════════════════

**Two-page swipe world — Home-first (v2 pivot):**
```
Home (0) ← OPENS HERE          Chat (1)
          swipe left with finger → for Chat
```
App opens on Home (page 0 — DOM leftmost). Chat is one swipe left with finger (arrow points → toward content). "Dashboard" was renamed "Home" in Session 32 v2 Round A; "Chat" was renamed from "Home" (yes, confusing history — the LABEL on the current chat screen used to say "Home", meaning the Zaeli-chat home base; now the Bento Dashboard IS home). **Kids Hub / Tutor / Travel / My Space all moved to `app/_hidden/`** — off the map for v2, not deleted, no MoreSheet tiles for them.

**PHILOSOPHY (revised Session 32 v2):**
Zaeli is a family productivity trio — Calendar + Shopping + Budget — with Reminders as the 4th pillar. Chat remains the AI conversation surface for anything not one-tap on Home. AI is a companion to the productivity core, not the whole product. This is a scope reduction from the "AI-first companion" framing (Session 9 Philosophy B) — the ambition remains, the beachhead is tighter. Rich made this pivot after Anna beta round 2 feedback: real families want reliable productivity glue first, AI polish second.

**LEGACY philosophy note (pre-Session 32):** *Zaeli was framed as "AI companion that also manages family life — NOT a family dashboard with AI bolted on. Chat is the product's beating heart."* Kept for context — that framing drove the Session 9-30 architecture. The v2 pivot preserves the AI (chat, tools, briefs, notifications, memory) but reframes Home as the front door.
...
- **Family plan: A$6.99/month inc GST** with 14-day free trial · Tutor add-on **hidden for v2** (returns when trio validates, likely A$7.99/child) · 100% web sales · **Reduced 31 July 2026** from A$9.99 (Session 25) alongside the v2 pivot — narrower product surface (Kids/Tutor/Travel/MySpace/Meals hidden) warrants the price reflection. Cozi Gold nearest competitor at ~A$4/mo (no AI, no lockscreen brief, no cross-device push); A$6.99 sits in the "clearly more than Cozi, still under $10" sweet spot. "Less than a coffee" copy hook. Session 26 pricing history preserved below.
...
### L. Verbose debugPushToken diagnostic fix (commit `250aa87`)

Session 30's initial `dcb263b` diagnostic was still returning "returned null" without pinpointing the failure step on-device. Diagnosis: `getCurrentUserId()` is async but was being called WITHOUT `await` in both `registerPushToken` and `debugPushToken` — was returning a Promise object, coerced to a string, always falsy. The whole diagnostic was walking past auth without noticing.

**Fixes:**
- `await getCurrentUserId()` in both call sites (obvious in retrospect — auto-suggestion or a lint rule would have caught it)
- Per-step try/catch wrapping each of the 5 phases (auth / permissions / getExpoPushTokenAsync / db-write / done) so structured failure lands with the specific exception text
- New step 0 pre-check via `typeof Notifications.getPermissionsAsync === 'function'` + `typeof Notifications.getExpoPushTokenAsync === 'function'` — surfaces immediately if the `expo-notifications` native module didn't link (belt-and-braces on Session 30's plugin discovery)
- Developer row Alert surfaces `notifTypes` field first so Rich sees the module-availability check upfront

Result: on-device Alert now shows exactly where the failure is, with the actual JS error string. Enabled the Phase 5 push-token verification path to work cleanly for the first time.

### M. Our Budget backend Supabase migration + AI averaging fix ⭐ (commit `f2974e4`)

Session 17 built Our Budget as UI-only with `SEED_INCOME`/`SEED_CATEGORIES`/`SEED_GOALS` constants — the "backend pass will pick it up" deferral was never actioned. Consequence: every app restart wiped user data. Anna couldn't see Rich's budget edits and vice versa. Silently broken for weeks, only surfaced when Rich actually tried to use it.

**NEW SQL: `supabase-budget-tables.sql`**
- 4 tables: `income_streams`, `budget_categories`, `category_line_items` (child of budget_categories), `savings_goals`
- Session 21 RLS pattern throughout — 4 policies per table via `family_id = public.current_family_id()`, session-aware for line_items via parent category subquery
- `ON DELETE CASCADE` on `category_line_items.category_id` — deleting a fixed category cleans up its line items automatically
- Idempotent DO-block for policy creation (drop-if-exists + create)

**NEW `lib/budget.ts`**
- `loadBudget(familyId)` returns `{ incomeStreams, categories, goals }` — hydrates from all 4 tables in parallel via Promise.all
- 8 CRUD helpers: `saveIncomeStream` / `removeIncomeStream` / `saveCategory` / `removeCategory` / `saveLineItem` / `removeLineItem` / `saveGoal` / `removeGoal`
- All helpers upsert (INSERT with `.upsert` + `.onConflict('id')`) so save-then-edit-then-save works transparently
- Exports `uuidv4()` helper — used everywhere IDs need generation

**`app/(tabs)/our-budget.tsx` migration**
- SEED_ constants removed
- `useState<X[]>([])` for each of income/categories/goals + hydration `useEffect` on mount calling `loadBudget(getFamilyId())`
- All save/remove helpers now: (1) update local state immediately (optimistic UI), (2) fire-and-forget the DB persist via lib helper — no await, no loading spinner. Errors log to console but don't block UI
- `applyAISuggestions` (from statement analysis) routed through the same helpers so persisted state stays consistent
- All ID generation switched from `Math.random().toString(36)...` to `uuidv4()` (matches DB uuid column type)

**AI averaging fix (same commit)** — Sonnet was fumbling multi-image math when averaging monthly spend across a statement pack (would return totals in `avg_monthly_amount`, or divide by wrong month count). Fix: schema now demands `total_seen` + top-level `months_analysed`, app computes the average client-side. Bulletproofs against any future Sonnet arithmetic drift.

**Verified end-to-end on device**: added income stream on Rich's device → showed on Anna's device on next open. Restart persistence works. Multi-image statement analysis now correctly averages.

### N. Whisper English lock + send_family_message honesty rules (commit `ff5bfd6`)

Two Anna feedback bugs from beta round 1 mystery test session.

**Bug 1 — Zaeli responded in WELSH to English input.** Root cause: Whisper hallucinates non-English transcription surprisingly often on unclear English audio (Welsh / Spanish / Portuguese are the common ones). No language hint was being passed. Sonnet then dutifully responded in the transcribed language.

Fix: append `language: 'en'` to all 9 Whisper API call sites:
- `app/(tabs)/index.tsx` (chat mic × 2 — voice overlay + inline chat bar)
- `app/(tabs)/tutor-session.tsx` (tutor voice)
- `app/(tabs)/tutor-practice.tsx` (tutor practice voice)
- `app/(tabs)/tutor-reading.tsx` (read-aloud transcription)
- `app/(tabs)/calendar.tsx` (calendar voice input)
- `app/(tabs)/index.tsx` (todos + shopping + meal planner add-via-voice paths)

**Bug 2 — Zaeli fabricated "sent it" for a family message.** Rich asked "did you send it to Anna?" — Zaeli said "yes, sent" but the family-notify Edge Function invocation log showed zero hits. Sonnet had drafted the message, echoed it back, and then when asked confidently claimed success.

Fix: ABSOLUTE HONESTY RULES added to the send_family_message capability block in CAPABILITY_RULES. Explicit ban on claiming "sent" without an actual "✅ Sent" tool response in the same turn. If ambiguous ("did you send it?" after a draft) she must ASK first ("Want me to text Anna now — 'X'?") rather than guess. Pattern: any tool that causes external-visible side effects and can be fabricated by Sonnet needs an ABSOLUTE HONESTY block.

### O. Removed stale "CANNOT send messages" capability rule (commit `c8db27f`)

Even with `ff5bfd6`'s honesty rules, Zaeli was still refusing to send family messages. Screenshot proof: user typed "Send Anna a text to say I'll be home in 20 minutes." Zaeli drafted; when asked "Did you send it?" she replied "I can draft the text, but I can't actually send messages from here."

Root cause: a leftover Session 15-ish capability rule said Zaeli CANNOT send any messages, which contradicted the send_family_message tool added Session 30. The old rule won because it appeared earlier in CAPABILITY_RULES and Sonnet gave it more weight.

Fix: rule rewritten to distinguish two cases:
- **Cannot**: SMS, iMessage, email, WhatsApp, or any messaging channel EXTERNAL to Zaeli (people outside the family)
- **Can and should**: family push notifications via `send_family_message` for people INSIDE the Zaeli family

Fallback stays: "I'll draft it and you paste" for people outside the family (e.g. "text the plumber").

### P. ⭐⭐⭐ Phase 5 API keys server-side (commit `e84a091`)

The critical pre-launch security migration flagged since Session 27. Anthropic + OpenAI keys were bundled in the app via `EXPO_PUBLIC_ANTHROPIC_API_KEY` / `EXPO_PUBLIC_OPENAI_API_KEY`. These are baked into the compiled JS bundle and extractable from the App Store binary by any determined user. Fine for TestFlight dogfood with trusted testers; NOT fine for public distribution — a leaked key = someone using Rich's Anthropic account on his bill.

**3 NEW Supabase Edge Functions** (all JWT-verified via `Authorization: Bearer <session.access_token>`):
- `supabase/functions/anthropic-proxy/index.ts` — proxies `/v1/messages`. Forwards optional `betaHeaders` array as the `anthropic-beta` request header (needed for prompt-caching-2024-07-31). Returns vendor-shape response verbatim so client code doesn't need to change beyond the endpoint URL.
- `supabase/functions/openai-proxy/index.ts` — proxies `/v1/chat/completions`. Simpler than Anthropic (no beta headers).
- `supabase/functions/whisper-proxy/index.ts` — proxies multipart `/v1/audio/transcriptions`. Re-emits FormData to get Content-Type header right, and auto-forces `language=en` server-side as a belt-and-braces on Session N's client-side fix (defence in depth on the Welsh problem).

**NEW `lib/ai-proxy.ts`** — `callAnthropic({ model, messages, system?, tools?, max_tokens?, betaHeaders?, temperature? })`, `callOpenAI({ model, messages, response_format?, temperature?, max_tokens? })`, `callWhisper(audioFile, { language? })`. Each helper resolves the current Supabase session, attaches JWT to Authorization header, POSTs to the corresponding Edge Function URL, and returns the vendor-shape response verbatim. No behavior change from a call-site perspective — just a URL swap + JWT attach.

**Migrated call sites (ACTIVE files only):**
- `lib/api-logger.ts` `callClaude` — huge leverage: covers `brief-generator.ts`, `tutor-summaries.ts`, and 5+ other callers transitively
- `app/(tabs)/index.tsx` — chat vision path, tool-calling initial + follow-up, Whisper × 2 (voice overlay + inline mic), LandingOverlay brief, generic `callGPT`, recipe upload vision, receipt scan vision
- `app/(tabs)/dashboard.tsx` — Zaeli Noticed pattern-detection + `generateDashBrief`
- `app/(tabs)/tutor-session.tsx` — Whisper
- `app/(tabs)/tutor-practice.tsx` — GPT tool-calling + Whisper
- `app/(tabs)/tutor-reading.tsx` — vision (read-aloud grading) + Whisper + feedback
- `app/(tabs)/kids.tsx` — trivia GPT generation
- `app/(tabs)/our-budget.tsx` — statement analysis
- `app/(tabs)/calendar.tsx` — chat tool + follow-up + Whisper
- `lib/zaeli-memory.ts` — pattern detection + insight extraction

**Legacy pre-Session-14 files** (e.g. `landing.tsx` deleted Session 25, some old helpers) still reference `EXPO_PUBLIC_*` env vars but they're dead code. Once vars removed from EAS at build time, Metro inlines `undefined` and those old references become no-ops. Bundle no longer contains the keys.

**External activation steps Rich completed:**
- `supabase secrets set "ANTHROPIC_API_KEY=..."` + `"OPENAI_API_KEY=..."` (values from local `.env`)
- `supabase functions deploy anthropic-proxy` + `openai-proxy` + `whisper-proxy` (all with default JWT verification, NOT `--no-verify-jwt`)
- Removed `EXPO_PUBLIC_ANTHROPIC_API_KEY` + `EXPO_PUBLIC_OPENAI_API_KEY` from expo.dev Environment Variables (Preview + Production scopes)
- Removed same keys from local `.env` (so `expo start` also proves the migration)

**PowerShell cmd.exe quoting gotcha** — Rich hit "'supabase' is not recognized" errors during secrets set. Root cause: `STRIPE_WEBHOOK_SECRET= whsec_...` with a space after `=` confused cmd.exe. Fix: wrap the whole `KEY=VALUE` in double quotes — `supabase secrets set "STRIPE_WEBHOOK_SECRET=whsec_..."`. Prevents cmd.exe from interpreting special characters (%, &, |, !) in secret values.

### Q. STRIPE-LIVE-SETUP.md walkthrough (commit `582091f`)

244-line doc covering the external activation path for going live:
- **Prerequisites**: ABN, banking, ID, phone number
- **Test-vs-live mode mental model** — Stripe's new UI has "Sandbox" vs "Live account" workspaces (not the old test-mode toggle). Top-right button toggles. Sandbox shows dark-blue banner top. Live account has no banner. Rich confused twice — worth documenting for future.
- Product setup with tax-inclusive gotcha (AU default is Exclusive, adds 10% on top of A$9.99 → charges A$10.99 — must explicitly set Inclusive)
- Payment Link creation (14-day free trial + card required upfront)
- Customer Portal config (return URL `https://zaeli.app`, cancellation at period end)
- Webhook registration (6 events, new whsec_ signing secret capture)
- Supabase secret update commands
- Real-money test flow — charge yourself with a real card + refund via Stripe dashboard immediately
- Rollback instructions if anything blows up

### R. Dev row gate + GDPR/Terms pages + welcome email doc (commit `96e485d`)

**Settings Developer section email-gated** — was showing scary buttons ("Reset to owner account", "Simulate invite accepted", "🔗 Open latest invite as receiver") to any user with a preview build. Anna would see these once she got the next build. Fix: guard entire Developer section behind `p.profile?.email === 'richarddekretser@gmail.com'`. Rich keeps on-device diagnostics in TestFlight/production; everyone else sees nothing scary.

**NEW `zaeli-app-links-template/public/privacy.html` + `terms.html`** — plain-English pages:
- Australian Privacy Act rights + how to exercise them
- AI processor disclosure (Anthropic + OpenAI in the US — CBPR handled via their respective compliance)
- Subscription terms (14-day trial, renewal, cancellation)
- Australian Consumer Law (ACL) preservation clause
- AI content disclaimer (Zaeli's suggestions are not medical/legal/financial advice)
- Contact `hello@zaeli.ai`

Both deployed to `zaeli.app/privacy.html` + `/terms.html` via the existing Netlify auto-deploy pipeline. Settings Privacy policy + Terms of service rows now `Linking.openURL` to the live URLs.

**NEW `SUPABASE-EMAIL-SETUP.md`** — copy-paste HTML templates for Rich to update Supabase Confirm signup + Reset password email templates. Zaeli-branded (sky-blue `a+i` wordmark, "Welcome in 👋" for signup, `hello@zaeli.ai` contact link). Rich updated both templates in the Supabase dashboard after the doc landed.

### S. ⭐ Stripe LIVE-mode Price IDs + Payment Link + 14-day trial (commit `3101f65`)

`lib/stripe.ts`:
- `STRIPE_PRICE_FAMILY = 'price_1Ttl54P6r3YzpJLXj3nySAYA'` (live Family Plan)
- `STRIPE_PRICE_TUTOR = 'price_1Ttl54P6r3YzpJLX4SUE4mnn'` (live Tutor Add-on)
- `STRIPE_PAYMENT_LINK_FAMILY = 'https://buy.stripe.com/fZubJ36EM1QR2z6eBl83C01'` (live Payment Link, 14-day trial)

`supabase/functions/stripe-webhook/index.ts`:
- `PRICE_TO_PLAN` map updated with live Price IDs
- Sandbox Price IDs kept as backwards-compat fallback (dev testing still works)

Settings Subscribe button label: "Subscribe · A$9.99/mo" → "Start free 14-day trial". Onboarding Ready step already said "Free for 14 days · A$9.99/month after" from Session 25 pricing pivot — NOW backed by real Stripe config.

**External activation Rich completed:**
- Stripe live mode ACTIVATED via KYC (ABN, business address, ID, bank, phone). Business website: `https://zaeli.app`. Business type: Individual/Sole trader. Description: "AI-powered family life management app for Australian families."
- Live products created — copied from sandbox via Stripe's "Choose what to copy" feature (tax settings + Payment Link + Products carried over cleanly)
- **Payment Link gotcha** — original copied Payment Link couldn't be edited (created via API — Stripe UI limitation). Solution: Rich Duplicated it, added 14-day trial, deactivated the original. New Duplicated link is the live one now embedded in the app.
- Live webhook "Zaeli subscription sync (live)" registered pointing at `https://rsvbzakyyrftezthlhtd.supabase.co/functions/v1/stripe-webhook`. 6 events subscribed. New whsec_ signing secret captured.
- Live Customer Portal activated (return URL `https://zaeli.app`, cancellation at period end, payment method updates, invoice history)
- `supabase secrets set "STRIPE_SECRET_KEY=sk_live_..."` + `"STRIPE_WEBHOOK_SECRET=whsec_..."` — LIVE keys now server-side

**⚠️ ANY LIVE STRIPE TESTING = REAL MONEY.** Test path: charge yourself with a real card via the Payment Link + immediate refund via Stripe dashboard. Never leave test charges hanging.

### T. ⭐ Anna round 2 fixes: multi-event photo add + calendar honesty + strip markdown (commit `c8ef80c`)

Anna uploaded 3 parent-teacher interview photos in one chat message (multi-photo pattern from Session 30 commit `e6c8ea6`). Only Gab's event actually landed in `events`. Zaeli then fabricated confident confirmation that Poppy's + Duke's were "in the system, neatly stacked like a very efficient school morning." Total hallucination — pure Sonnet trying to please the user without any tool response to back the claim.

**Three root causes fixed:**

**1. Vision-describe prompt asked for "2-3 sentences total"** — catastrophic for multi-event photos. Sonnet dutifully summarised: "Three parent-teacher interview slips for Gab, Poppy, Duke on various dates." No enumeration. When the tool path then tried to extract structured events from that summary, only ONE cleanly-formatted event was in scope (Gab's — first in the summary).

Fix: rewrote vision-describe prompt to demand NUMBERED enumeration ("List each distinct event with full detail — teacher name, date, time, location, child. Do not summarise. Do not skip. Number each event."). Bumped vision `max_tokens: 800 → 1500` to fit up to ~15 events without truncation (same pattern as Session 29's calendar bulk-paste bump).

**2. MULTI-EVENT PHOTO UPLOADS rule added to CAPABILITY_RULES** — explicit instruction to Sonnet: if images show N distinct events, emit N separate `add_calendar_event` tool calls. Do not batch into one. Honestly report success/fail counts in the reply ("Added 2 of 3 events; Duke's had no time listed — want to add manually?").

**3. CALENDAR HONESTY — ABSOLUTE rule** mirroring the send_family_message honesty pattern from `ff5bfd6`. NEVER claim added/booked/scheduled without a `✅ tool response` in the same turn. Do not paraphrase absence as presence. Do not invent event details from images without actually calling the tool. If tool fails, say so honestly ("Couldn't add Duke's — no time in the photo. Want me to try with a placeholder time?"). Same pattern as calendar phantom fix (Session 28) but at the language layer this time, not the DB layer.

**Separate fix — strip `**bold**` markdown in chat render.** Shopping tool return strings use `**Butter**` markdown for emphasis but RN `<Text>` renders `**` literally (no bold interpretation). User was seeing `Added **Butter** to your list` in chat feed. Fix: `cleanText` regex layer added in message render path (strips `**...**` around 1-40 chars, plus a `stripMd` helper for `inlineData.intro` + `followUp`). Also handles `[bracketed]` markers for consistency.

### U. External activations recap (14–17 July, non-code)

Beyond the per-commit externals noted above:
- **Supabase secrets set for Phase 5**: `ANTHROPIC_API_KEY` + `OPENAI_API_KEY` (values from Rich's local `.env`). Server-side only from here on.
- **EAS env vars removed**: `EXPO_PUBLIC_ANTHROPIC_API_KEY` + `EXPO_PUBLIC_OPENAI_API_KEY` deleted from expo.dev Environment Variables (Preview + Production scopes). Rich also removed from local `.env` — `expo start` now proves the migration by failing loudly if any code path still references them.
- **Netlify — privacy.html + terms.html deployed** to `https://zaeli.app/privacy.html` + `/terms.html`. Both URLs live and verified via browser.
- **Supabase auth email templates updated** with Zaeli-branded HTML (Confirm signup + Reset password). Sky-blue a+i wordmark, "Welcome in 👋", `hello@zaeli.ai` contact link.
- **Stripe LIVE**: mode activated, products created, Payment Link with trial live, webhook registered, Customer Portal activated, all live secrets in Supabase. See Section S for the full path.
...
- **Our Budget = Supabase-backed cross-device state.** 4 tables via `supabase-budget-tables.sql`. Every mutation persists via `lib/budget.ts` helpers (optimistic UI + fire-and-forget DB write). NEVER reintroduce `SEED_` constants — restart persistence + cross-device sync depend on the DB. When adding new budget-related state (recurring transfers, forecasts, whatever), extend the schema + helpers, don't inline seed arrays.
- **Budget AI averaging = client-side math, not Sonnet math.** Sonnet returns `total_seen` + top-level `months_analysed`; app computes `avg = total_seen / months_analysed`. Bulletproofs against multi-image arithmetic drift. Apply the same pattern to any AI extraction where Sonnet is tempted to do arithmetic — return raw counts + let the app do the math.
- **Whisper `language: 'en'` on every call.** Auto-detection routes unclear English audio to Welsh/Spanish/Portuguese surprisingly often. Every call site MUST include the language param. Belt-and-braces: `whisper-proxy` Edge Function force-injects `language=en` server-side too — but don't rely on that alone, always pass it client-side so intent is visible.
- **ABSOLUTE HONESTY RULES for tools that make external-visible changes.** `send_family_message` got this in `ff5bfd6`. Calendar got this in `c8ef80c`. Any future tool that (a) causes side effects and (b) Sonnet can fabricate success for needs an "ABSOLUTE" honesty block in CAPABILITY_RULES. Pattern: NEVER claim X unless the tool call was made this turn AND returned `✅`. If unsure, ASK first ("Want me to text Anna now — 'X'?"). Never paraphrase absence as presence.
- **Multi-event photo uploads = enumerate every event in vision, emit tool call per event.** Vision prompt MUST ask for numbered enumeration, NOT summary. `max_tokens` must fit the enumeration (1500 is safe for up to ~15 events). Sonnet then emits N separate tool calls, one per event, and honestly reports success/fail counts. Same pattern applies to any multi-item extraction (receipts, screenshots of task lists, calendar screenshots).
- **Phase 5: AI API keys live server-side ONLY.** Client uses `lib/ai-proxy.ts` helpers (`callAnthropic`/`callOpenAI`/`callWhisper`). Never re-introduce `EXPO_PUBLIC_ANTHROPIC_API_KEY` or `EXPO_PUBLIC_OPENAI_API_KEY` env vars. New AI call sites MUST use the proxy helpers. Edge Functions attach the real key server-side + verify JWT — client never sees the key. Bundle extraction attack surface eliminated.
- **PowerShell cmd.exe quoting rule**: any `supabase secrets set KEY=VALUE` command should wrap the whole `KEY=VALUE` in double quotes. Prevents cmd.exe from interpreting special characters (%, &, |, !, whitespace) in secret values. Example: `supabase secrets set "STRIPE_SECRET_KEY=sk_live_..."`.
- **Stripe live-mode tax setting**: prices MUST be "Tax behavior: Inclusive" for AU. Default Exclusive adds 10% GST on top (charges A$10.99 instead of A$9.99). Copy-from-sandbox usually preserves this but always verify each product's tax setting in live mode before flipping visibility.
- **Stripe live Payment Link with trial can't be edited via UI if created via API** (which the auto-copy uses). Solution: Duplicate the copied link → configure trial on the duplicate → deactivate the original → embed the duplicate's URL in the app. Non-obvious limitation, worth documenting.
- **Chat text render strips `**bold**` markdown** — RN `<Text>` has no bold renderer for `**`, would show `**word**` literally in the feed. Same `cleanText` regex layer handles `[bracketed]` markers. Applies to Msg.text render, inlineData.intro, and inlineData.followUp. Any future tool return string that uses markdown emphasis must expect the app to strip it.
- **Settings Developer section email-gated to Rich only** — `p.profile?.email === 'richarddekretser@gmail.com'`. Rich keeps diagnostics in TestFlight/production (push token debug, reset account, simulate invite, etc). Everyone else sees nothing scary. Never remove this gate — beta testers seeing "Reset to owner account" is a support-ticket generator.
- **Stripe UI 2026 uses "Sandbox" vs "Live account" workspaces** — NOT the old test-mode toggle. Top-right button now says "Switch to live account" or "Switch to sandboxes". Sandbox shows dark-blue banner top of every page; live account has no banner. When troubleshooting Stripe issues, always confirm which mode you're actually in first. Rich confused this twice — worth defaulting to a mental "am I in live or sandbox?" check on any Stripe operation.
- **Async helpers must be `await`ed** — `getCurrentUserId()` is async. Calling it without `await` returns a Promise object, coerced to string is `"[object Promise]"`, coerces to boolean as truthy but doesn't match any UUID. `250aa87` cost a diagnostic cycle on this. Any future helper that hits Supabase/AsyncStorage must be awaited at every call site; consider a lint rule if this bites again.
...
**NEW SQL (post 14 July):**
- `supabase-budget-tables.sql` — 4 tables (income_streams, budget_categories, category_line_items, savings_goals) with Session 21 RLS pattern + ON DELETE CASCADE on line items

**NEW Edge Functions (post 14 July):**
- `supabase/functions/anthropic-proxy/index.ts` — JWT-verified `/v1/messages` proxy, forwards `betaHeaders` as `anthropic-beta`
- `supabase/functions/openai-proxy/index.ts` — JWT-verified `/v1/chat/completions` proxy
- `supabase/functions/whisper-proxy/index.ts` — JWT-verified multipart `/v1/audio/transcriptions` proxy, auto-forces `language=en`

**NEW libs (post 14 July):**
- `lib/ai-proxy.ts` — `callAnthropic` / `callOpenAI` / `callWhisper` helpers with JWT attach
- `lib/budget.ts` — `loadBudget` + 8 CRUD helpers + `uuidv4()` export

**NEW docs (post 14 July):**
- `STRIPE-LIVE-SETUP.md` — 244-line walkthrough for going live (KYC → products → Payment Link → webhook → secret rotation)
- `SUPABASE-EMAIL-SETUP.md` — Zaeli-branded HTML templates for Confirm signup + Reset password auth emails

**NEW web assets deployed to zaeli.app (post 14 July):**
- `zaeli-app-links-template/public/privacy.html` — GDPR/Australian Privacy Act rights + AI processor disclosure
- `zaeli-app-links-template/public/terms.html` — subscription terms, ACL preservation, AI content disclaimer

**MODIFIED (post 14 July):**
- `lib/notifications.ts` — `debugPushToken` per-step try/catch + `await getCurrentUserId()` (was missing) + module-type pre-check at step 0
- `lib/api-logger.ts` — `callClaude` routed through `lib/ai-proxy.ts` (huge leverage: covers brief-generator + tutor-summaries + all Sonnet callers)
- `lib/zaeli-memory.ts` — pattern detection + insight extraction migrated to ai-proxy
- `lib/stripe.ts` — LIVE Price IDs (`STRIPE_PRICE_FAMILY` / `STRIPE_PRICE_TUTOR`) + `STRIPE_PAYMENT_LINK_FAMILY` constant
- `supabase/functions/stripe-webhook/index.ts` — `PRICE_TO_PLAN` map updated with LIVE Price IDs, sandbox kept as fallback
- `app/(tabs)/index.tsx` — Anthropic + OpenAI + Whisper migrated to ai-proxy (chat vision, tool + follow-up, mic × 2, LandingOverlay, generic GPT, recipe upload, receipt scan). `language: 'en'` on all Whisper calls. `send_family_message` CAPABILITY_RULES rewritten (honesty rules + removed stale "CANNOT send" contradiction). Vision-describe prompt rewritten to demand numbered enumeration + max_tokens 800→1500. MULTI-EVENT PHOTO UPLOADS rule added. CALENDAR HONESTY ABSOLUTE rule added. Chat render `cleanText` strips `**bold**` markdown + `[bracketed]` markers. `stripMd` helper for `inlineData.intro` + `followUp`.
- `app/(tabs)/dashboard.tsx` — Zaeli Noticed + `generateDashBrief` migrated to ai-proxy
- `app/(tabs)/tutor-session.tsx` — Whisper migrated to ai-proxy + `language: 'en'`
- `app/(tabs)/tutor-practice.tsx` — GPT + Whisper migrated to ai-proxy + `language: 'en'`
- `app/(tabs)/tutor-reading.tsx` — vision + Whisper + feedback migrated to ai-proxy + `language: 'en'`
- `app/(tabs)/kids.tsx` — trivia GPT migrated to ai-proxy
- `app/(tabs)/calendar.tsx` — chat tool + follow-up + Whisper migrated to ai-proxy + `language: 'en'`
- `app/(tabs)/our-budget.tsx` — SEED_ constants removed, useState([]) + hydration effect, all save/remove helpers write through `lib/budget.ts` (optimistic + fire-and-forget), `applyAISuggestions` routed through helpers, uuidv4 IDs, statement analysis AI averaging fix (schema now `total_seen` + `months_analysed`, app computes avg client-side)
- `app/(tabs)/settings.tsx` — Developer section email-gated to `richarddekretser@gmail.com`, Subscribe button label "Subscribe · A$9.99/mo" → "Start free 14-day trial", Privacy/Terms rows `Linking.openURL` to `zaeli.app/privacy.html` + `/terms.html`

**EXTERNAL (post 14 July):**
- Stripe live mode ACTIVATED (KYC completed by Rich — ABN, banking, ID, phone, business website `https://zaeli.app`)
- Live Stripe products created (Family Plan A$9.99/mo tax-inclusive, Tutor Add-on A$7.99/mo tax-inclusive)
- Live Payment Link with 14-day trial: `https://buy.stripe.com/fZubJ36EM1QR2z6eBl83C01`
- Live webhook registered pointing at `supabase.co/functions/v1/stripe-webhook` (6 events, new whsec_)
- Live Customer Portal activated (return `https://zaeli.app`)
- Supabase secrets updated: `STRIPE_SECRET_KEY` (sk_live_), `STRIPE_WEBHOOK_SECRET` (whsec_ live)
- Supabase secrets set (Phase 5): `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`
- EAS env vars REMOVED: `EXPO_PUBLIC_ANTHROPIC_API_KEY`, `EXPO_PUBLIC_OPENAI_API_KEY` (Preview + Production scopes)
- Local `.env` cleaned of Anthropic + OpenAI keys
- Netlify auto-deploy shipped `privacy.html` + `terms.html` to zaeli.app
- Supabase auth email templates updated (Confirm signup + Reset password, Zaeli-branded)

### What's next

- **⭐ One big EAS production build with everything from today** (last commit `c8ef80c`) — should be the last build for a while. All Session 30 work bundled: family push (with plugin + verbose diagnostic + missing await fix), multi-photo chat, Spend overlay, Stripe LIVE checkout (14-day trial + real card), Budget backend persistence + cross-device, Welsh gone from voice, calendar multi-event photo add with honesty rule, `**markdown**` stripped in chat, Phase 5 keys server-side, Developer section hidden for non-Rich.
- **`eas submit --platform ios --latest`** to push the build to TestFlight.
- **Test path when it lands on TestFlight**: push notifications finally working (Notify chip on calendar cards + `send_family_message` from chat), multi-photo chat send, Spend scan overlay, Stripe live checkout (Subscribe → 14-day trial → REAL CARD), Budget persistence + cross-device sync (Rich edits → Anna sees on next open), Welsh gone from voice input, calendar multi-event add via 3-photo school newsletter test, no `**bold**` markdown in chat feed, no Developer section visible for Anna's login.
- **⚠️ ANY LIVE STRIPE TESTING = REAL MONEY.** Charge yourself with a real card via the Payment Link, then immediately refund via Stripe dashboard → Payments → refund. Never leave test charges hanging.
- **Anna beta round 3** — with everything live. First-ever end-user Stripe checkout on real money. First cross-device Budget sync check. First voice tests with English lock.
- **GIPHY EAS env var still not added** — Kids Hub GIFs will keep showing 🎉 placeholder until Rich adds `EXPO_PUBLIC_GIPHY_API_KEY` at expo.dev → Environment Variables (Sensitive visibility, Preview + Production).
- **Website v5 rewrite** — still parked; 4-round plan drafted. Deploy to zaeli.app (replacing current orb splash). ~1 day total.
- **Outlook/Gmail calendar sync** — still parked. Revisit if Anna asks twice.
- **Sonnet 5 migration** — still queued for after Anna beta settles. Intro pricing $2/$10 through Aug 2026 makes it CHEAPER than 4.6.
- **Per-user brief cache** — still queued for scale past 100 families.
- **Phase 5 legacy dead-code cleanup** — old references to `EXPO_PUBLIC_ANTHROPIC_API_KEY` / `EXPO_PUBLIC_OPENAI_API_KEY` remain in a handful of pre-Session-14 files (landing.tsx already deleted Session 25 but similar legacy paths may exist). Now that vars are removed from EAS + local `.env`, those references are inert — Metro inlines `undefined`. Do a `grep -r EXPO_PUBLIC_ANTHROPIC` pass and clean when convenient.

---

## ══════════════════════════════════
## SESSION 31 — BENTO DASHBOARD v2 (early — 30 July 2026) ✅
## ══════════════════════════════════

Prep session for the v2 pivot. Introduced the Bento Dashboard as the future front door (Phase 01), then made it the actual front door (Phase 02) and hid old features (Phase 03).

- **Phase 01 — Bento Dashboard v2** — new dashboard.tsx laid out as a Bento tile grid: Calendar (slate, full-width), Meal Planner (mint), Weather + Zaeli Noticed (bento 35/65), Shopping (lavender), On the Radar (gold). Font bumps to match Chat's 17px baseline. Establishes the "tile-with-inline-add-pill" pattern later refined in v2.
- **Phase 02 — Dashboard is the front door** — swipe-world initial page flipped from Chat (1) → Dashboard (0). App opens on Dashboard. Chat becomes swipe-left. (Later revised in Round A when Rich clarified label: "Home" not "Dashboard".)
- **Phase 03 — Hide dead features** — Kids Hub, Tutor, Travel, My Space, Meals moved to `app/_hidden/` folder (Expo Router ignores underscore-prefixed folders). Not deleted. MoreSheet trimmed to trio.
- **Website v1 draft + v2 design brief HTML** — `zaeli-v2-design-brief.html` (11-chapter design brief) + first attempt at ActionBar mockup (superseded next session by the universal chat bar direction).

---

## ══════════════════════════════════
## SESSION 32 v2 — MAJOR VISION PIVOT (31 July 2026) ✅
## ══════════════════════════════════

The biggest architecture + product-vision shift since Session 14. Zaeli's Phase 1 roll-out ambition changed from "AI companion that also manages family life" to "family productivity trio (Calendar + Shopping + Budget + Reminders)". Ambition preserved; beachhead tightened for Anna beta round 3.

Rich's v2 workshop drivers:
- Anna's iPhone Notes was faster than Zaeli for shopping adds (6-10s vs 2-3s). Real speed problem.
- Meal planner adoption zero; broad AI framing was diffuse.
- Rich wanted "get this thing in real testing ASAP" — narrow feature set, clear identity.

### Phases shipped

- **Phase 04a — Bento refined + universal chat bar** ([`c59943e`](https://github.com/RDK1981/zaeli/commit/c59943e))
    * Brief/weather/Noticed/Chat tiles + coral mic FAB removed
    * Universal chat bar at bottom (mic/text/camera/coral send)
    * Budget tile redesigned minimal (no financial numbers on front door — Rich's rule: "$0 left is a downer every open")
    * Font bumps throughout (17px chat baseline)
- **Phase 04b — swipe-world DOM swap + dots killed** ([`370c0cb`](https://github.com/RDK1981/zaeli/commit/370c0cb))
    * PAGE_DASHBOARD=0 / PAGE_CHAT=1 constants swapped (Dashboard is now DOM leftmost, opens by default)
    * Page dots removed (looked awkward without chat bar underneath)
    * Hint arrow points → toward Chat (iOS convention — arrow shows content location, not finger direction)
- **Phase 04c — chat bar per-icon routing** (same commit)
    * NEW `ChatIntent` type in `lib/navigation-store` (mic/camera/focus/seed)
    * Dashboard chat bar: each control sets intent + navigates
    * Chat consumes intent on activation, jumps straight into the right mode
    * 250ms delay after swipe animation so it feels intentional
- **Phase 05 — Reminders subsystem (the 4th pillar)** ([`4383a22`](https://github.com/RDK1981/zaeli/commit/4383a22))
    * NEW `supabase-reminders.sql` — `reminders` table with 3 shapes: timed (remind_at) / date-only (remind_on) / undated. Recurring via repeat_group_id (12-month generated instances, same pattern as calendar events). created_by tracked so only creator gets phone notification.
    * NEW `lib/reminders.ts` — CRUD + local push scheduling. `saveReminder` schedules a local iOS notification for the creator only. `markReminderDone`/`unmarkReminderDone` cancels the push. `generateRecurrenceDates` + `saveReminderSeries` (daily/weekdays/weekly/fortnightly/monthly, 12-mo horizon).
    * Gold Home tile between Shopping and Budget
    * 92% Reminders sheet in Chat with add/list/tick/delete. Gold left-border on rows shows creator (own = gold, other = grey), "shared" tag on others' rows.
    * `add_reminder` Sonnet tool — three shapes supported (timed/date-only/undated), repeat support, honest tool response with when-label
    * SQL fixed for legacy pre-Session-14 `reminders` table ([`92ac2a2`](https://github.com/RDK1981/zaeli/commit/92ac2a2)) — CREATE TABLE IF NOT EXISTS was silently skipping; new SQL uses ADD COLUMN IF NOT EXISTS per-column, non-destructive to legacy rows
- **Phase 06 — Notify chip on all add tools** ([`58bc2e6`](https://github.com/RDK1981/zaeli/commit/58bc2e6))
    * Chat tool-completion path detects `add_shopping_item` and `add_reminder` tool_uses
    * Attaches Notify chip + notifyPayload (title = "🛒 Rich added to shopping" / "⏰ Rich added a reminder", body summarises what was added)
    * Reuses existing handleQuickReply notify pipeline (Session 29)
    * Calendar events already had richer per-event chips (Session 30 confirm card) — not touched here
- **Phase 07 — Server-side brief scheduler** (same commit)
    * NEW `supabase/functions/brief-scheduler/index.ts` (Deno Edge Function)
        - Scans `profiles.user_preferences` for briefMorningTime/briefEveningTime
        - Finds families whose scheduled time is within window
        - Skips if `zaeli_briefs` already has a row for family+date+window
        - Gathers LIVE DATA (events today/tomorrow, meal, shopping count, tasks)
        - Calls Anthropic Sonnet with compact brief prompt (Australia/Brisbane fixed TZ v1)
        - Upserts to `zaeli_briefs` cache
        - Sends RICH lockscreen push to every family adult with push token — brief prose in the notification body
        - Idempotent, no double-sends
        - `dry_run: true` body param for manual testing without hitting Anthropic/Expo
    * NEW `supabase-brief-scheduler.sql` — pg_cron every 15 min hitting the Edge Function. Requires `pg_cron` + `pg_net` extensions enabled in Supabase Studio. Requires service_role JWT in Authorization header (never commit that SQL block).
    * Rich deployed successfully end-to-end (Cron firing verified via `cron.job_run_details` + Edge Function logs).
- **Phase 08 — Budget flat Expenses model** ([`d03bb75`](https://github.com/RDK1981/zaeli/commit/d03bb75) + [`547148c`](https://github.com/RDK1981/zaeli/commit/547148c))
    * NEW `supabase-budget-expenses.sql` — `budget_expenses` table (flat, one row per monthly expense). `type: 'fixed' | 'variable'`. Session 21 RLS. Data migration query commented in-file for one-shot flatten of legacy categories + line_items.
    * `lib/budget.ts` gained `Expense` type + `loadExpenses` / `saveExpense` / `deleteExpense` helpers. Legacy Category + LineItem helpers KEPT for rollback.
    * `our-budget.tsx` — additive approach: flat Expenses section at TOP of Categories tab, legacy nested categories still render below labelled "Legacy categories". Zero risk to Anna's existing data.
    * NEW `ExpensesSection` component (grouped by fixed/variable, own totals, empty states, per-type Add CTA) + `EditExpenseSheet` (type toggle + 24-emoji picker + name + amount + delete-with-confirm)
- **Phase 09 — Calendar full-title-on-expand** (part of `4383a22`)
    * `CalSheetEventCard`: `numberOfLines={expanded ? undefined : 1}` on title. Long parent-teacher event names now readable when tapped open.
- **Phase 10 — Hidden features (`_hidden` folder)** (Session 31 Phase 03 landed early)
    * Kids Hub / Tutor / Travel / My Space / Meals routes moved to `app/_hidden/`. Expo Router ignores. MoreSheet trimmed to trio.
- **Phase 11 — Production build + submit** (pending Rich's run after Round A)

### Round A — Anna round 2 test feedback loop

Rich real-device tested the initial v2 build and surfaced a dozen issues. Shipped in 6 commits:

- **[`0923a71`](https://github.com/RDK1981/zaeli/commit/0923a71) — Bug fixes trio:**
    1. **Notify chip missing on calendar adds** — Chip render block lived in `hasOtherInline` branch only; calendar confirm cards trigger `hasCalendarInline` which had no chip render. Copied the chip block into the calendar branch. All Session 30 Notify chip functionality restored.
    2. **Reminder timezone bug** (1:27pm Brisbane saved as 11:27pm) — Hermes parses `"2026-07-31T13:27:00"` (no timezone) as UTC. Fix in `lib/reminders.ts`: NEW `normaliseLocalIso()` strips any tz suffix, NEW `parseLocalIsoAsDate()` builds Date via local constructor components (unambiguous). All 3 display sites fixed (dashboard tile, tool return message, reminders sheet). Sonnet tool description hardened + CURRENT_TIME injected into system prompt (fresh at each call).
    3. **Pantry pre-block on add** (Rich rule: pantry isn't 100% accurate) — `add_shopping_item` ALWAYS inserts now. If pantry contains the item, tool response includes soft "NOTE: … added anyway, tap to remove if not needed" that Sonnet relays. `force` flag removed from tool schema. SHOPPING RULES prompt rewritten.
- **[`8881e0f`](https://github.com/RDK1981/zaeli/commit/8881e0f) — Home rename + Home tile redesign + More sheet + Budget label:**
    * Dashboard header now shows "Home" label; Chat header changed "Home" → "Chat"
    * Hamburger on Home opens MoreSheet (was direct-to-Settings shortcut)
    * Home tile big-text headlines (26px 800): "2 events on today." / "4 items to grab." / "1 due at 3pm." / "On track for the month." Empty state falls back to competence line
    * Per-tile mic icon inside each add pill (Calendar/Shopping/Reminders) — routes to Chat with mic intent
    * "Full × →" hint top-right on every tile
    * Detail rows retain matching-size text (13-15px) below headline
    * MoreSheet — 6 full-width tiles at 68px each (was 3×2 grid), Reminders added between Shopping and Budget (gold `#FBF5D6`). NEW `IcoReminders` SVG. `channelContext` wires 'reminders' → `{ type: 'reminders_sheet' }`
    * Budget tab: "Categories" → "Expenses"
- **[`80762e8`](https://github.com/RDK1981/zaeli/commit/80762e8) — Sheet-over-Home + input pill + chat bar match:**
    * `openCalendarSheet` / `openShoppingSheet` / `openRemindersSheet` no longer call `onNavigateChat` — RN Modal is a portal, renders at root regardless of active page. Sheet appears over Home, closing keeps user on Home (return-to-origin is default because there's no nav to reverse)
    * Reminders sheet input replaced with unified pill: `[mic | text | camera | send]` (60px minHeight, gold-tinted mic button + border to match Reminders accent, camera opens Add-to-Chat picker)
    * Home chat bar style matches Chat's `barPillV2` exactly: border rgba(220,220,220,0.6), paddingV 10 paddingH 12, alignItems flex-end, minHeight 60, shadow (opacity 0.10, radius 18, offset 0/6), elevation 10. Bottom position matches Chat's barFloat: 24 iOS / 14 Android. Zero-flicker swipe.
- **[`05ace95`](https://github.com/RDK1981/zaeli/commit/05ace95) — Kill local notif + harden Phase 07 server push:**
    * Rich saw lockscreen notif with generic body "Tap to see how today shapes up." — that was the Phase 3a LOCAL notification (Session 23), not Phase 07's server push
    * `_layout.tsx`: removed `scheduleBriefNotifications` call. On next authed-open, `cancelBriefNotifications()` wipes any lingering scheduled local notifs from prior builds
    * `settings.tsx`: removed reschedule-on-prefs-change call (prefs still persist via `user_preferences` JSONB; server reads them)
    * `brief-scheduler` WIDER FIRING WINDOW: was `abs(now - target) < 15` (only ONE cron slot per brief time fired — cron drift = miss). NEW: fires if we're AT or PAST target by up to 45 min AND today's brief hasn't already been generated. Gives 3 cron slots per brief window
    * BETTER LOGGING at each stage — profile count, family count, candidate count, token count per family, push-send tickets
    * STRUCTURED PUSH RESULT — `sendBriefPush` returns `{ sent, failed, reason?, ticketErrors? }`. Response payload includes `briefLen`, `bodyLen`, full push result so we can see from a single curl why any family didn't get their push
    * Status `"generated_but_no_push"` when brief text lands in cache but push send returns 0 sent

### Locked decisions Session 32 v2 + Round A

- **Home is the front door. Chat is one swipe left.** Never swap back to Chat-first — Rich's v2 pivot is deliberate and grounded in Anna's usage data.
- **Trio + Reminders is the product surface** for Phase 1 launch. Kids Hub / Tutor / Travel / My Space / Meals stay in `app/_hidden/` until we have validated product-market fit on the trio.
- **Home tiles use big-text headlines (26px 800)** as the punch. Detail rows below at 13-15px. Empty state = competence line ("Nothing on today.", "List is empty."), NEVER a nudge ("You should plan…").
- **Per-tile mic in add pill** routes to Chat with mic intent — user says the thing, Zaeli tools it. Consistent across Calendar/Shopping/Reminders.
- **"Full × →" hint top-right on every tile** so tap-to-expand is obvious.
- **Universal chat bar identical Home↔Chat.** Same styles, same position, zero-flicker swipe. If a flicker ever appears, full component hoisting to swipe-world is the next step.
- **Sheet-over-Home is the pattern.** RN Modal is a portal — sheet renders over whichever page is active. Closing a sheet returns you to the page you were on. Never scroll-to-Chat-then-open-sheet.
- **Reminders subsystem = 4th pillar.** Family-shared visibility, creator-only phone notifications. Timed (push at instant) / date-only (shows on day) / undated (someday bucket). Recurring generates 12-month instances via `repeat_group_id` (same pattern as calendar events).
- **Server-side brief scheduler is the sole notification source.** Local notifications KILLED. Cron every 15 min, wide window (0-45 min after target) for cron latency, idempotent via `zaeli_briefs` cache row check. Brief body paragraph delivered via Expo push to lockscreen.
- **Reminder times are Brisbane local wall-clock.** Sonnet sends `"YYYY-MM-DDTHH:MM:SS"` with no tz suffix. Client uses `parseLocalIsoAsDate()` (constructs Date via local component constructor, not `new Date(string)` which Hermes parses as UTC). CURRENT_TIME injected fresh into Sonnet system prompt at each call.
- **Pantry never blocks a shopping add.** Always inserts. If in pantry, tool response includes a soft `NOTE:` that Sonnet relays as a gentle mention — user taps to remove if not needed. `force` flag gone. `PANTRY:` warning flow retired.
- **Categories → Expenses (Budget).** Flat model. Legacy nested categories still render below during transition. Data migration SQL (commented in-file) available for one-shot flatten.

### Files touched Session 31 + Session 32 v2 + Round A

**NEW SQL:**
- `supabase-reminders.sql` — reminders table + legacy-table-safe ADD COLUMN pattern
- `supabase-budget-expenses.sql` — flat expenses table + commented migration
- `supabase-brief-scheduler.sql` — pg_cron schedule (Rich fills service_role key, never commit)

**NEW libs / helpers:**
- `lib/reminders.ts` — CRUD + local push scheduling + `parseLocalIsoAsDate` + `normaliseLocalIso` + recurring
- `lib/budget.ts` — added `Expense` type + `loadExpenses` / `saveExpense` / `deleteExpense`

**NEW Edge Functions:**
- `supabase/functions/brief-scheduler/index.ts` — Deno, JWT-verified via service_role Bearer, Anthropic + Expo push

**NEW nav-store types:**
- `ChatIntent` (mic/camera/focus/seed) + `setChatIntent`/`consumeChatIntent`
- `ChatEntryContext.type` extended with `reminders_sheet` | `calendar_view` | `shopping_sheet`

**MOVED to `app/_hidden/`** (Session 31 Phase 03):
- `kids.tsx`, `kids-games-data.ts`, `wordle-data.ts`, `wordle-extra-words.ts`
- `tutor.tsx`, `tutor-child.tsx`, `tutor-curriculum.ts`, `tutor-practice.tsx`, `tutor-reading.tsx`, `tutor-session.tsx`
- `travel.tsx`
- `my-space.tsx`

**MODIFIED (major):**
- `app/(tabs)/dashboard.tsx` — full v2 rewrite: Bento tiles + big headlines + per-tile mic + Full hints + Home label + MoreSheet trigger + reminder tile
- `app/(tabs)/swipe-world.tsx` — DOM order swap, dots removed, hint arrow flipped
- `app/(tabs)/index.tsx` — Reminders sheet + `add_reminder` tool + `parseLocalIsoAsDate` display sites + calendar chip render fix + pantry rewrite + CURRENT_TIME in prompt + `chatbar` style match + sheet-over-Home flow + 3 view-query branches
- `app/(tabs)/our-budget.tsx` — Categories→Expenses rename + flat Expenses view + EditExpenseSheet + `loadExpenses` hydration
- `app/components/MoreSheet.tsx` — full-width stacked tiles, Reminders added, IcoReminders SVG, tileRow style bumped
- `app/_layout.tsx` — kill local notifs + cancel-on-open + registerPushToken retained
- `app/(tabs)/settings.tsx` — removed `scheduleBriefNotifications` reschedule call

**HTML mockups produced (in repo root):**
- `zaeli-v2-design-brief.html` — 11-chapter design brief (Session 31)
- `zaeli-v2-actionbar-mockup.html` — 9-chapter Bento + universal chat bar (Session 32 v2)
- `zaeli-v2-round-a-mockup.html` — 3-frame Round A mockup (big-text tiles + More sheet + sheet input pill)

### What's next after Round A

1. **Rich runs production EAS build** — `eas build --platform ios --profile production` then `eas submit --platform ios --latest`
2. **Anna beta round 3** — fresh feedback on the whole v2 pivot
3. **Round B queued fixes:**
    - Home tile stale after sheet edits (delete reminder → close sheet → Home Reminders tile still shows deleted item until force-close). Root cause: Home caches its own state; sheet mutates Chat's state; sheet close doesn't refire Home's `loadData` because Home never lost focus. Fix: shared `homeRefresh` trigger in nav-store, Chat bumps it after sheet mutations, Home watches in useEffect.
...
4. **Website v6 rewrite** — v2 pivot means the current site is stale. Reminders as 4th pillar visible, new brief-on-lockscreen story, Home not Dashboard.
5. **Sonnet 5 migration** — still queued for post-beta
...
Phase 101: Verbose debugPushToken diagnostic fix ✅ Session 30 (15 July) — commit 250aa87. Fixed missing `await getCurrentUserId()` in both registerPushToken + debugPushToken (async fn was returning Promise object coerced to string, always wrong). Per-step try/catch wrapping each of the 5 phases. New step 0 pre-check via typeof Notifications.getPermissionsAsync/getExpoPushTokenAsync — surfaces immediately if expo-notifications native module didn't link. On-device Alert now shows exactly where failure is with actual JS error string.
Phase 102: Our Budget Supabase backend ⭐ ✅ Session 30 (16 July) — commit f2974e4. Session 17 built as UI-only, deferral never picked up — every restart wiped data + Anna couldn't see Rich's edits. supabase-budget-tables.sql (4 tables, Session 21 RLS pattern, ON DELETE CASCADE on line items). NEW lib/budget.ts (loadBudget + 8 CRUD helpers + uuidv4 export, all upsert via .onConflict('id')). our-budget.tsx — SEED_ constants removed, useState([]) + hydration effect, all save/remove helpers write local then fire-and-forget DB persist, applyAISuggestions routed through helpers, ID generation switched to uuidv4. Verified cross-device end-to-end.
Phase 103: Budget AI averaging fix ✅ Session 30 (16 July) — part of commit f2974e4. Sonnet was fumbling multi-image math (returning totals in avg_monthly_amount, dividing by wrong month counts). Fix: schema requires total_seen + top-level months_analysed, app computes average client-side. Bulletproofs against future Sonnet arithmetic drift. Same pattern for any AI extraction where Sonnet is tempted to do arithmetic.
Phase 104: Whisper English language lock ⭐ ✅ Session 30 (16 July) — commit ff5bfd6 (part). Whisper hallucinated Welsh transcription on unclear English audio; Sonnet dutifully responded in Welsh. Fix: append `language: 'en'` to all 9 Whisper API call sites (chat mic × 2, tutor session/practice/reading, calendar, todos/shopping/mealplanner add-via-voice paths). Belt-and-braces server-side too in whisper-proxy.
Phase 105: send_family_message honesty rules ⭐ ✅ Session 30 (16 July) — commit ff5bfd6 (part). Zaeli said "sent it" for a family message but Edge Function showed zero invocations. Sonnet fabricated success. Fix: ABSOLUTE HONESTY RULES added to send_family_message capability block. Explicit ban on claiming "sent" without ✅ tool response same turn. If ambiguous, ASK first ("Want me to text Anna now — 'X'?"). Pattern: any tool causing external side effects + fabricatable success needs ABSOLUTE HONESTY block.
Phase 106: Removed stale "CANNOT send messages" rule ✅ Session 30 (16 July) — commit c8db27f. Leftover Session 15-ish capability rule contradicted send_family_message tool. Screenshot: user asked "Send Anna a text" → Zaeli refused ("I can't actually send messages from here") despite tool existing. Rule now distinguishes: no SMS/iMessage/email (true for people OUTSIDE family), but CAN and SHOULD send family push via send_family_message (INSIDE family). Fallback stays: "I'll draft it and you paste" for outside-family.
Phase 107: ⭐⭐⭐ Phase 5 — API keys server-side (PRE-LAUNCH CRITICAL) ✅ Session 30 (17 July) — commit e84a091. Anthropic + OpenAI + Whisper keys were bundled in app via EXPO_PUBLIC_* vars (extractable from compiled binary). 3 NEW Supabase Edge Functions: anthropic-proxy (JWT-verified /v1/messages, forwards betaHeaders as anthropic-beta), openai-proxy (JWT-verified /v1/chat/completions), whisper-proxy (JWT-verified multipart /v1/audio/transcriptions, re-emits FormData, auto-forces language=en server-side). NEW lib/ai-proxy.ts — callAnthropic/callOpenAI/callWhisper helpers, attaches user JWT, returns vendor-shape response verbatim. Migrated call sites in ACTIVE files: lib/api-logger.ts callClaude (huge leverage — covers brief-generator + tutor-summaries + 5+ callers transitively), index.tsx (chat vision + tool + follow-up + Whisper × 2 + LandingOverlay + generic callGPT + recipe upload + receipt scan), dashboard.tsx, tutor-session/practice/reading.tsx, kids.tsx, our-budget.tsx, calendar.tsx, lib/zaeli-memory.ts. Legacy pre-Session-14 files still reference env vars but are dead code. Supabase secrets set for ANTHROPIC_API_KEY + OPENAI_API_KEY; EAS env vars removed; local .env cleaned.
Phase 108: STRIPE-LIVE-SETUP.md walkthrough ✅ Session 30 (17 July) — commit 582091f. 244-line doc: prerequisites (ABN, banking, ID, phone), test-vs-live mode mental model (Sandbox vs Live workspaces in NEW Stripe UI), product setup with tax-inclusive gotcha, Payment Link creation, Customer Portal config, webhook registration (6 events, new signing secret), API key + webhook secret capture, Supabase secret update commands, real-money test flow, rollback instructions.
Phase 109: Dev row gate + GDPR/Terms + welcome email doc ✅ Session 30 (17 July) — commit 96e485d. Settings Developer section now gated to `p.profile?.email === 'richarddekretser@gmail.com'` — Anna and future testers/users don't see scary "Reset to owner account" buttons. Rich still sees them in TestFlight/production. NEW zaeli-app-links-template/public/privacy.html + terms.html deployed to zaeli.app (Australian Privacy Act rights, AI processor disclosure Anthropic+OpenAI in US, subscription terms, ACL preservation, AI content disclaimer, contact hello@zaeli.ai). Settings Privacy/Terms rows Linking.openURL to https://zaeli.app/privacy.html + /terms.html. NEW SUPABASE-EMAIL-SETUP.md (Zaeli-branded auth email templates for Confirm signup + Reset password, sky-blue a+i, "Welcome in 👋", hello@zaeli.ai).
Phase 110: ⭐ Stripe LIVE-mode Price IDs + Payment Link + 14-day trial ✅ Session 30 (17 July) — commit 3101f65. lib/stripe.ts: STRIPE_PRICE_FAMILY = live price_1Ttl54P6r3YzpJLXj3nySAYA, STRIPE_PRICE_TUTOR = live price_1Ttl54P6r3YzpJLX4SUE4mnn, STRIPE_PAYMENT_LINK_FAMILY = https://buy.stripe.com/fZubJ36EM1QR2z6eBl83C01. supabase/functions/stripe-webhook/index.ts: PRICE_TO_PLAN map updated with live IDs; sandbox IDs kept as backwards-compat fallback. Settings Subscribe button label "Subscribe · A$9.99/mo" → "Start free 14-day trial". Onboarding Ready step already said "Free for 14 days · A$9.99/month after" from Session 25 pricing pivot — NOW backed by real Stripe config.
Phase 111: Stripe LIVE mode ACTIVATED ✅ Session 30 (17 July) — external. Rich completed KYC (ABN, business address, ID, bank, phone). Business website: https://zaeli.app. Business type: Individual/Sole trader. Live products created (Zaeli Family Plan A$9.99/mo tax-inclusive + Tutor Add-on A$7.99/mo tax-inclusive) via Stripe's "Choose what to copy" from sandbox. Payment Link gotcha: original copied Payment Link couldn't be edited (created via API — Stripe UI limitation), Rich Duplicated it + added 14-day trial + deactivated original. Live webhook "Zaeli subscription sync (live)" registered pointing at supabase.co/functions/v1/stripe-webhook (6 events subscribed, new whsec_). Live Customer Portal activated (return https://zaeli.app, cancellation at period end). Supabase secrets updated: STRIPE_SECRET_KEY (sk_live_), STRIPE_WEBHOOK_SECRET (whsec_ from live endpoint).
Phase 112: ⭐ Anna round 2 fixes ✅ Session 30 (17 July) — commit c8ef80c. Anna uploaded 3 parent-teacher interview photos, only Gab's event landed but Zaeli confidently fabricated "Poppy's + Duke's are in the system, neatly stacked like a very efficient school morning." Three root causes fixed: (a) Vision-describe prompt asked for "2-3 sentences total" — catastrophic for multi-event photos. Rewrote to demand numbered enumeration + max_tokens 800→1500. (b) MULTI-EVENT PHOTO UPLOADS rule added to CAPABILITY_RULES (emit N tool calls, honestly report success/fail counts). (c) CALENDAR HONESTY — ABSOLUTE rule mirroring send_family_message pattern (NEVER claim added/booked without ✅ tool response same turn, don't paraphrase absence as presence, don't invent event details from images without tool-adding).
Phase 113: Chat text strips `**bold**` markdown ✅ Session 30 (17 July) — part of commit c8ef80c. Shopping tool return strings use `**Butter**` markdown for emphasis but RN Text renders literally. Users saw `Added **Butter** to your list` in chat feed. Fix: cleanText regex layer strips `**...**` (1-40 chars) in Msg.text render. stripMd helper does same for inlineData.intro + followUp. Also handles `[bracketed]` markers for consistency.
Phase 114: Big production EAS build + TestFlight submit 🔨 NEXT — bundle all Session 30 work (last commit c8ef80c). One production build → `eas submit --platform ios --latest` → Anna beta round 3 with everything live.
```
...
- **Our Budget = Supabase-backed cross-device state** (Session 30 extension). 4 tables via `supabase-budget-tables.sql`. Every mutation persists via `lib/budget.ts` helpers (optimistic local UI + fire-and-forget DB write). NEVER reintroduce SEED_ constants — restart persistence + cross-device sync depend on the DB. When adding new budget-related state (recurring transfers, forecasts, etc), extend the schema + helpers, don't inline seed arrays. Same pattern applicable to any future data domain that needs cross-device sync.
- **AI arithmetic = client-side math, Sonnet returns raw counts** (Session 30) — for any AI extraction that requires arithmetic (multi-image averaging, totals across statements, aggregates over lists), have Sonnet return the raw component counts (`total_seen`, `months_analysed`, `item_count`) and let the app compute the final value. Sonnet fumbles multi-step arithmetic reliably enough that math logic belongs in code. Applied to budget statement analysis; apply anywhere else the same shape emerges.
- **Whisper `language: 'en'` on every call** (Session 30) — auto-detection routes unclear English audio to Welsh/Spanish/Portuguese surprisingly often. Every client call site must pass the language param. `whisper-proxy` Edge Function also force-injects it server-side as belt-and-braces (defence in depth). Don't rely on server alone — always pass client-side too so intent is visible in the call site.
- **ABSOLUTE HONESTY RULES for side-effect tools** (Session 30) — `send_family_message` got this in `ff5bfd6`, calendar got it in `c8ef80c`. Any future tool that (a) causes external-visible side effects and (b) Sonnet can fabricate success for needs an "ABSOLUTE HONESTY" block in CAPABILITY_RULES. Pattern: NEVER claim X unless the tool call was made this turn AND returned `✅`. If ambiguous, ASK first ("Want me to text Anna now — 'X'?"). Never paraphrase absence as presence. Never invent details from images without actually calling the tool.
- **Multi-item photo uploads: enumerate every item in vision, emit tool call per item** (Session 30) — vision prompt MUST ask for numbered enumeration, NOT summary. Vision `max_tokens` must fit the enumeration (1500 is safe for ~15 events). Sonnet then emits N separate tool calls, one per item, and honestly reports success/fail counts. Applies to multi-event calendars, multi-item receipts, multi-child school newsletters, etc.
- **Phase 5: AI API keys live server-side ONLY** (Session 30, LOCKED PRE-LAUNCH) — client uses `lib/ai-proxy.ts` helpers (`callAnthropic` / `callOpenAI` / `callWhisper`). NEVER re-introduce `EXPO_PUBLIC_ANTHROPIC_API_KEY` or `EXPO_PUBLIC_OPENAI_API_KEY` env vars — they bake into the compiled bundle and are extractable. New AI call sites MUST use the proxy helpers. Edge Functions attach the real key server-side + verify JWT — client never sees the key. Bundle extraction attack surface eliminated. Supabase URL + anon key stay client-side (RLS enforces security).
- **PowerShell cmd.exe quoting rule** (Session 30) — any `supabase secrets set KEY=VALUE` command should wrap the whole `KEY=VALUE` in double quotes. Prevents cmd.exe from interpreting special characters (%, &, |, !, whitespace) in secret values. Example: `supabase secrets set "STRIPE_SECRET_KEY=sk_live_..."`. Applies to any command where the value contains special chars.
- **Stripe live-mode tax setting** (Session 30) — prices MUST be "Tax behavior: Inclusive" for AU. Default Exclusive adds 10% GST on top (charges A$10.99 instead of A$9.99). Copy-from-sandbox usually preserves this but always verify each product's tax setting in live mode BEFORE flipping to public.
- **Stripe live Payment Link with trial can't be edited via UI if created via API** (Session 30) — which the auto-copy uses. Solution: Duplicate the copied link → configure trial on the duplicate → deactivate the original → embed the duplicate's URL in the app. Non-obvious limitation, worth documenting.
- **Chat text render strips `**bold**` markdown** (Session 30) — RN `<Text>` has no bold renderer for `**`, would show `**word**` literally in the feed. Same `cleanText` regex layer handles `[bracketed]` markers. Applies to `Msg.text` render, `inlineData.intro`, and `inlineData.followUp`. Any future tool return string that uses markdown emphasis must expect the app to strip it. Consider building a proper markdown-to-JSX renderer if we ever want to keep the bold.
- **Settings Developer section email-gated to Rich only** (Session 30) — `p.profile?.email === 'richarddekretser@gmail.com'`. Rich keeps diagnostics in TestFlight/production (push token debug, reset account, simulate invite, test Stripe checkout, etc). Everyone else sees nothing scary. NEVER remove this gate — beta testers seeing "Reset to owner account" is a support-ticket generator. When adding new dev rows, always put them inside the gated section.
- **Stripe UI 2026 uses "Sandbox" vs "Live account" workspaces** (Session 30) — NOT the old test-mode toggle. Top-right button now says "Switch to live account" or "Switch to sandboxes". Sandbox shows dark-blue banner at top of every page; live account has no banner. When troubleshooting Stripe issues, always confirm which mode you're in first. Rich confused this twice — worth defaulting to a mental "am I in live or sandbox?" check on any Stripe operation.
- **Async helpers MUST be `await`ed at every call site** (Session 30) — `getCurrentUserId()` is async. Calling it without `await` returns a Promise object, coerced to string is `"[object Promise]"`, coerces to boolean as truthy. Cost a diagnostic cycle on `250aa87`. Any future helper that hits Supabase/AsyncStorage must be awaited everywhere; consider a lint rule if this bites again.
- **⚠️ LIVE Stripe testing = REAL MONEY** (Session 30) — test path: charge yourself with a real card via the Payment Link + immediate refund via Stripe dashboard → Payments → refund. Never leave test charges hanging. Never assume "it's still test mode because it looks the same" — always confirm workspace before subscribing.
- **AI provider migrations touch `lib/api-logger.ts` first** (Session 30, from Phase 5 experience) — `callClaude` in api-logger is the transitive dependency of `lib/brief-generator.ts`, `lib/tutor-summaries.ts`, and 5+ other callers. Migrate the logger's callClaude and huge swathes of the app move with it. Same principle applies to any future infra swap (rate limit wrapper, retry logic, request tracing) — hit the central caller first, ripple out to direct callers second.
