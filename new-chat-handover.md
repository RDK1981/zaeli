...
*Legacy Session 34 header (kept for context): 15 August 2026 — Session 34 ✅ · **BUILD 48 SHIPPED WITH REAL-USER BUG FIXES + ICAL SYNC SCOPED + ANDROID PATH DECIDED.** Second real beta user (Andy, `nortonata@hotmail.com`) hitting real bugs, Damien Pagnin (Rich's cousin) waitlisted for Android. **Build 48 shipping to TestFlight now**: (1) Andy's Add Event RLS bug fixed (hardcoded DUMMY_FAMILY_ID at index.tsx:2979 in cal-sheet-save + NEW `awaitFamilyId()` async helper). (2) **Option C — Our Family module fully editable** — NEW `supabase-family-editing.sql` (profiles.budget_access + tighter family_members RLS owner-or-self + `remove_family_member` SECURITY DEFINER RPC), NEW helpers in `lib/family-roster.ts` (updateMemberName/updateMemberColor/removeMember/getBudgetAccess/setBudgetAccess), NEW EditNameSheet + ColorPickerSheet (8-swatch palette) modals + tap-to-edit Name/Colour + persisted kid Budget toggle + real Remove-from-family. (3) **Owner missing from own family list HOTFIX** — root cause: onboarding filtered `id === 'me'` from family_members insert. Every downstream screen (Our Family, calendar avatars, meal cook picker) treats family_members as authoritative roster. NEW `ensureOwnMembership()` helper — auto-heal fires in `_layout.tsx` on every app open (proactive) + `family.tsx` on Our Family mount (safety net) + onboarding now inserts owner with `id = profile.id` + invitee flow does same. Invariant enforced: `family_members.id === profiles.id === auth.uid()`. (4) **⭐⭐⭐ Cross-family data leak fixed** — Andy's Zaeli asked "who's it for (Poppy? Anna?)" for his Netball event because `FAMILY: Rich, Anna, Poppy, Gab, Duke` was HARDCODED in the cached Sonnet system prompt at index.tsx:4676. Every user of the app got Rich's family injected. Fix: moved FAMILY line from staticSystem (Anthropic-cached at 90% discount) to dynamicSystem (uncached) built from `getRoster()` + `getProfile()`. Same fix in `lib/brief-generator.ts` and `supabase/functions/brief-scheduler/index.ts` (6+ hardcoded name references replaced with placeholders + explicit "USE ONLY REAL FAMILY NAMES" rule). (5) **Anti-snowball rules** added to CALENDAR HONESTY block — Andy saw Zaeli say "the calendar doing its little vanishing act again". Three new rules: NO META-COMMENTARY on failures (each turn starts CLEAN), WHEN USER ASKS "DID YOU ADD X?" call find_calendar_events FIRST (never guess), RECURRING DEFAULTS (title + weekday + time is enough, don't over-ask). (6) Recurring event code verified clean — enum-locked schema + silent-RLS block detection + 12-month horizon all working. Bug was prompt drift, not code · **Admin-announce false-failure FIXED (Batch 9, deployed)** — Zoho SMTP finished but `HTTP 546 CPU Time exceeded` killed worker before response returned. Fix: return 200 immediately, run SMTP via `EdgeRuntime.waitUntil`. Trade-off: rare Zoho fails now invisible (email_log audit trail deferred) · **Brief-scheduler dormant-gate fresh-signup exemption (deployed)** — Batch 6 dormant gate blocked brand-new users' first-ever morning brief. Fix: profile <7 days old = exempt · **iCal two-way sync — SCOPED, foundation SQL READY, code deferred to Build 49+** — Andy asked. Decisions: per-user external sync (Andy's iPhone events only for Andy in Zaeli), opt-in per calendar, full two-way not phased. 3-build plan: Build 49 foundation + read-in, Build 50 write-back, Build 51 reconciliation + Android parity. NEW `supabase-calendar-sync.sql` drafted (events.privacy_scope + external_id + imported_by_user_id, calendar_sync_config table, updated RLS enforcing privacy_scope=personal only visible to importing user). **NOT yet deployed** — waiting for client code · **Android path DECIDED** — Damien (Rich's cousin) waitlisted. Direct-install APK for close family/friends now (EAS build → sideload link), public Google Play Store proper in 6-8 weeks. Two new admin console email templates deployed: 🤖 Android waitlist + 🚪 Android back door (fun conspiratorial for cousins/friends, honest about scary warning + no push yet + manual reinstall + guinea pig role) · **Prior Session 32 v2 vision pivot** — Zaeli's Phase 1 identity as "family productivity trio (Calendar + Shopping + Budget)" with Reminders as 4th pillar. Home is front door, Chat one swipe left. Kids Hub / Tutor / Travel / My Space / Meals in `app/_hidden/`. Server-side brief scheduler LIVE. Universal chat bar identical Home↔Chat. Sheet-over-Home pattern. Pricing A$6.99/mo tax-inclusive.*

*Legacy pre-Session-34 header (kept for context): 31 July 2026 — Session 32 v2 Round A ✅ · MAJOR VISION PIVOT + ROUND A — Zaeli's Phase 1 identity has changed from "AI companion that also manages family life" to "family productivity trio (Calendar + Shopping + Budget)" with Reminders as the 4th pillar. Home is the front door (renamed from Dashboard); Chat is one swipe left. Kids Hub / Tutor / Travel / My Space / Meals moved to `app/_hidden/` — off the map for v2, brought back when trio is validated. Big-text Home tile headlines (26px 800: "2 events on today.", "4 items to grab.", "1 due at 3pm.", "On track for the month.") with per-tile mic in add pill + "Full × →" hint. Universal chat bar identical Home ↔ Chat, zero-flicker swipe. Reminders subsystem (4th pillar) — 3 shapes (timed/date-only/undated) + recurring 12-month horizon + gold Home tile + 92% sheet with unified input pill + `add_reminder` Sonnet tool. Server-side brief scheduler LIVE — Phase 07 shipped (pg_cron every 15 min, wide window for cron latency, Anthropic Sonnet + Expo push with rich body prose to lockscreen). Local notifications KILLED. Sheet-over-Home pattern (Modal is portal, sheet opens over Home without scroll-to-Chat, closes back to Home). Round A bug fixes: Notify chip on calendar adds restored (chip render was in wrong branch), reminder timezone fixed (Hermes UTC bug — `parseLocalIsoAsDate` + CURRENT_TIME in Sonnet prompt), pantry no longer pre-blocks shopping adds (always inserts + soft mention). MoreSheet redesigned (6 full-width tiles, Reminders added). Budget "Categories" → "Expenses" (flat model). **All Session 30 infrastructure still current** (Phase 5 API keys server-side, Stripe LIVE, Family push, Anna round 2 fixes, Privacy/Terms deployed). · **⭐ POST-ROUND-A: WEBSITE v6 SHIPPED + PRICING DROPPED TO A$6.99** — full site rewrite for v2 pivot (Bento Home mockup + big-text tiles + "One family. One view." mirror section with two-phone sync mockup + "Zaeli on your lockscreen" hero moment + Reminders as 4th pillar section + updated Chat section + simplified pricing card). Trio deep-dive scrapped, mirror section replaced it. Pricing dropped **A$9.99 → A$6.99 family plan tax-inclusive** (14-day free trial unchanged) — reflects narrower v2 product surface. Tutor add-on hidden with the rest. "Less than a coffee. Whole family sorted." copy hook. Cost + ROI math verified: per-family API ~A$3.10/mo → per-family margin A$2.41-2.63/mo (38-41% margin). Sub-A$5 tier rejected as knife-edge. **Stripe live products need updating** — Rich creates new Price at A$6.99 tax-inclusive + new Payment Link with 14-day trial, updates `lib/stripe.ts` `STRIPE_PRICE_FAMILY` + `STRIPE_PAYMENT_LINK_FAMILY` constants + webhook `PRICE_TO_PLAN` map (keep old A$9.99 Price ID in map as backwards-compat fallback). · Three-day continuation of Session 30 has landed the largest single ship in the project's history — 9 commits pushed on top of 14 July's Session 30 base (all on origin/main). Headlines: **`e84a091` Phase 5 Anthropic + OpenAI + Whisper keys moved server-side via 3 Edge Functions** (client-bundled keys physically removed from the app — closes the pre-public-launch security hole flagged Session 27); **`3101f65` Stripe LIVE mode activated end-to-end** (Rich completed KYC, live products created A$9.99 + A$7.99 tax-inclusive, Payment Link with 14-day free trial live at buy.stripe.com/fZubJ36EM1QR2z6eBl83C01, live webhook registered + secrets set); **`f2974e4` Our Budget Supabase persistence** (4-table migration + `lib/budget.ts` swap, plus AI averaging fix); **`ff5bfd6` Whisper English lock** on all 9 voice-transcribe call sites (fixes Anna's Welsh transcription bug); **`c8db27f` + `c8ef80c`** honesty rules on `send_family_message` + calendar tools so Zaeli stops fabricating "message sent" / "event added" confirmations, plus Anna round 2 fixes (multi-event photo add prompt rewrite, `**markdown**` stripped from chat display); **`96e485d` GDPR + Terms pages** (privacy.html + terms.html deployed live to zaeli.app + dev row gated to Rich's email so Anna doesn't see it) · **Everything untested until the final build tomorrow** — Rich is queueing ONE `eas build --platform ios --profile production --auto-submit` with all 9 commits in it. Test path: push tokens (finally), multi-photo chat, Spend overlay, Stripe LIVE checkout with 14-day trial (REAL card required — test card 4242 doesn't work in live mode), Budget persistence + cross-device with Anna, AI budget averaging, multi-event calendar photo add, no `**markdown**` in chat, no Welsh from Whisper, Dev section hidden from Anna, "Start free 14-day trial" Subscribe button label · **Parked / deferred**: Sonnet 5 migration (queued for post-beta, intro pricing $2/$10 through Aug 2026 makes it cheaper than 4.6), per-user brief cache (queued past 100 families), Website v5 refresh (4-round plan drafted, ~1 day), Outlook/Gmail calendar sync (revisit if Anna asks twice), ElevenLabs voice, splash blue-flash full elimination (WSL needed, cosmetic parked) · **Prior Session 30 (14 July)** — push notification infrastructure, multi-photo chat, Kids Trivia guard, cold-start splash latency fix, expo-notifications plugin discovery, EAS Starter upgrade — all context, still applicable*
...
**A. Bug 1 — Reminders routing regression fix ⭐** (4 edits in `app/(tabs)/index.tsx`).
- Rich reported "Can you remind me to order hair product at 9am?" landed in the To-dos tab with the entire raw prompt as the title. Three combined root causes:
  1. `add_reminder` tool description at line 1804 never taught Sonnet to strip conversational prefaces or interpret "at 9am" as implicit-today
  2. Stale line 2636 in send_family_message capability rules still told Sonnet self-reminders go to `add_todo` (contaminated mental model even when tool_choice forced add_reminder)
  3. `add_todo` tool description at line 1791 was one line ("Add a todo item") — no differentiation from reminders
- Fix — 4 edits:
  - Line 1804: expanded `add_reminder` description with CRITICAL PREFACE-STRIPPING RULE (explicit list of prefaces to strip) + IMPLICIT-TODAY RULE ("at 9am" without a day = today, or tomorrow if past) + concrete example matching Rich's exact bug
  - Line 1791: expanded `add_todo` description to explicitly say "for time-based reminders, use add_reminder"
  - Line 2636: swapped "use add_todo instead" → "use add_reminder instead (it has proper time/push handling)"
  - Line ~2400 in `executeTool('add_reminder')`: DEFENCE-IN-DEPTH — strip PREFACE_PATTERNS client-side, if title still contains "at Xam/pm" AND no remind_at set, auto-infer remind_at (today or tomorrow) and strip the fragment from title. Belt-and-braces so this can NEVER regress silently again.

**B. Bug 2 — iCal event flicker fix ⭐⭐** (`lib/calendar-sync.ts` + `app/_layout.tsx`).
- Rich reported events appearing then disappearing on every open — extremely poor UX, felt broken.
- Investigation found TWO stacked causes:
  1. `syncNow` fired from mount (line 111) AND every AppState `background → active` transition (line 205) with ZERO debounce or in-flight guard. Concurrent syncs raced through the delete-stale step.
  2. When `Calendar.getEventsAsync()` threw for any single calendar (recurring events like "Albert Lumines - Monthly 1:1" trigger transient EventKit failures), the loop `continue`d and those events' external_ids never entered `allExternalIds` — the DELETE step then wiped those rows from Supabase → next sync re-inserted → visible flicker.
- Three fixes:
  1. NEW `_syncInFlight = new Map<string, Promise<SyncNowResult>>` module-level. `syncNow(userId, ...)` now coalesces — if a sync is already in flight for the same user, returns the same promise. Public API unchanged.
  2. NEW `allReadsSucceeded` tracking + `failedCalendars: string[]`. If ANY calendar read fails, SKIP the delete-stale step entirely for that run. Better to have one stale row for one sync than yo-yo flicker.
  3. NEW `lastForegroundSyncRef` in `_layout.tsx` — 5-second debounce on AppState foreground syncs. Collapses rapid Control Centre / Notification Centre / Camera dismiss / share sheet dismiss bounces to one sync.

**C. Bug 3 — "?" avatar fix ⭐⭐** (`app/(tabs)/dashboard.tsx` render branches + NEW `supabase-repair-stale-assignees.sql`).
- Two merged bug sources:
  1. Imported iCal events have `assignees=[]` intentionally (Rich's rule — don't stamp "R" on every work meeting). But `dashboard.tsx:504-513` had no branch for `source === 'apple-ical'` — fell through to `.charAt(0)` of undefined → "?".
  2. `loadRoster()` is async — between splash-hide and roster hydration, `getRoster()` returns `[]` (Round B removed DEFAULT_ROSTER). During that ~50ms-1s window, `memberById()` returns undefined for every event → avatars render "?".
  3. Bonus persistent-"?" case: duplicate-family_members leftover from Session 35's ensureOwnMembership name-match guard — historical events reference the OLD (mismatched) id, roster lookup fails forever.
- Fixes:
  - `dashboard.tsx:504-513` render logic split into 4 branches:
    - `isExternal` → render 📱 icon in avatar slot (matches CalSheetEventCard iPhone badge pattern)
    - `mem` resolved → normal letter avatar (existing path)
    - roster empty (`!rosterLoaded`) → subtle white-14% placeholder circle
    - roster loaded but no match → white-18% placeholder circle (never paint "?", never leak identity concerns)
  - NEW `supabase-repair-stale-assignees.sql` — well-commented dry-run + apply pattern:
    - STEP 1 (dry-run): per-family count of affected events + 20-event sample showing `current_assignees` / `stale_ids_to_drop` / `valid_ids_to_keep`
    - STEP 2 (apply — commented out, uncomment after reviewing dry-run): UPDATE rewrites each event's assignees array to keep only UUIDs that match a real family_members row in that family
    - Idempotent + safe to run multiple times

**D. Bug 4 — Welcome email `resend_welcome` misnamed action ⭐⭐** (`supabase/functions/admin-actions/index.ts` + admin console HTML + NEW `supabase-kara-email-backfill.sql`).
- Kara installed 16 Aug, went from awaiting-install to active. Instead of receiving the auto-drip "You're in — welcome to Zaeli" (day-0), she received "You're on the Zaeli beta list" AGAIN at 7:31pm. Rich manually sent the correct template at 8:36pm via admin-announce.
- Root cause: `resend_welcome` action at line 440-465 called `beta-notify` — which is HARDCODED to send the beta-list confirmation email. Admin console button label said "Resend welcome email" but the wiring was a lie. Also `beta-notify` doesn't write to `email_log` so the misfire was invisible in the audit trail.
- Fixes:
  1. Rewired `resend_welcome` to call `sendEmailInline({templateId:'welcome', triggeredBy:'resend', force:true})` — writes to email_log for audit trail
  2. Added `force` param to `sendEmailInline` — bypasses dedup (normal grant/invite paths still dedupe, but manual resend must be able to re-send a template already sent)
  3. NEW `resend_beta_list_confirmation` action for the RARE case someone actually wants to re-send the pre-install confirmation
  4. Admin console UI split into TWO clearly-labelled buttons with distinct descriptions:
     - "Resend welcome" — for post-install welcome
     - "Resend beta-list" — for pre-install confirmation (rare)
  5. NEW `supabase-kara-email-backfill.sql` — DO $$ block: looks up Kara by email, inserts a `template_id='welcome'` row into `email_log` with `triggered_by='manual'` + Rich's 8:36pm send time. Prevents any future auto-retry from double-sending. Safe to replay.
- Kara's day-3 checkin email will fire correctly regardless (keys off `beta_start_date` + separate `template_id='checkin-3day'` dedup — untouched by any of the above).

**E. Bug 5 — Per-user briefs (Phase 85 promoted from queued) ⭐⭐⭐** (NEW `supabase-zaeli-briefs-per-user.sql` + `supabase/functions/brief-scheduler/index.ts` rewrite + `lib/brief-generator.ts` updates + `app/(tabs)/index.tsx` caller).
- Server-side brief-scheduler generated ONE brief per family per window per day. Andy's personal iCal events (`privacy_scope='personal'` + `imported_by_user_id=Andy`) are RLS-scoped — invisible to any query that isn't Andy's. Result: shared family brief says "clear day" while Andy's iPhone Calendar is full of work meetings. Trust-breaking.
- Migration `supabase-zaeli-briefs-per-user.sql`:
  1. `ALTER TABLE zaeli_briefs ADD COLUMN user_id uuid REFERENCES auth.users(id) ON DELETE CASCADE` (nullable initially)
  2. Backfill: SET user_id = family owner via LATERAL join
  3. Delete any rows still null (orphans — shouldn't happen)
  4. NOT NULL constraint
  5. DROP old unique constraint `(family_id, date_key, time_window)` — tried both known auto-names
  6. CREATE UNIQUE INDEX new `(family_id, user_id, date_key, time_window)`
  7. Update lookup index to include user_id
  - Wrapped in BEGIN/COMMIT so nothing lands unless every step succeeds
- Brief-scheduler REWRITTEN:
  - `UserToBrief` type (userId + familyId + firstName + window + scheduledAt + pushToken)
  - Iterates adult profiles directly (not families) — each user's brief times come from their own `user_preferences`
  - `gatherLiveData(userId, familyId, dateKey)` uses `.or()` to fetch family-shared events (source IS NULL OR privacy_scope='shared') + THIS user's personal iCal events (source='apple-ical' + imported_by_user_id=userId + privacy_scope='personal') in one query
  - Events tagged `_source: 'family' | 'iphone'` in the shape so Sonnet knows the origin (though the prompt tells her to mix them into "your day")
  - Upsert keyed by `(family_id, user_id, date_key, time_window)`
  - NEW `sendUserPush(pushToken, ...)` sends to single user's token (was `sendBriefPush(familyId, ...)` fanning out to all family adults)
  - Dormant gate still fires per-family (any activity whitelists all adults); fresh-signup exemption per-family
- Client-side `lib/brief-generator.ts`:
  - `generateBrief({ familyId, userId, window, dateKey, context })` — userId now required
  - Cache SELECT/UPSERT filtered by user_id, onConflict updated
- `app/(tabs)/index.tsx` caller — passes `getProfile()?.id` as userId; skips generate + drops placeholder if profile hasn't loaded yet
- Bonus: caught an existing bug in the original brief-scheduler upsert — it was writing to a `text:` column but the schema is `brief_text`. Server-side upserts had been silently failing for weeks (only client-side path from `lib/brief-generator.ts` was actually writing rows). Fixed as part of the rewrite.
- Cost impact: 2× briefs for 2-adult families. At beta scale (~5 families, mostly single-adult) negligible. ~A$0.005/brief with caching.

**F. Bug 6 — Manual share iCal event toggle ⭐** (`CalSheetEventCard` in `app/(tabs)/index.tsx` + `lib/calendar-sync.ts` UPDATE-path fix).
- In CalSheetEventCard's expanded view for imported iCal events (`source === 'apple-ical'`): new mint pill "Share with family" with checkmark toggle
  - Default OFF (privacy_scope='personal', only importer sees)
  - Tap ON → UPDATE `events.privacy_scope='shared'` → whole family can see via RLS
  - Tap OFF → back to 'personal'
  - Optimistic UI + revert on error, `shareBusy` guards double-tap
  - Descriptive helper text below toggle explains state clearly
- `lib/calendar-sync.ts` UPDATE-path bug also fixed — was always resetting `privacy_scope='personal'` on subsequent syncs, which would have blown away the user's manual share. Fix: strip `privacy_scope` from the UPDATE row (only INSERT sets the default).

### Key decisions / learnings Session 36

- **Root cause > band-aid.** Every fix in Build 54 went one level below the visible symptom. Reminders bug wasn't "add another prompt rule" — it was 3 combined issues that all needed addressing. iCal flicker wasn't "add debounce" — it was "concurrent syncs racing through unsafe DELETE step". "?" avatar wasn't "hide the letter" — it was multiple avatar-render paths + a historical data repair.
- **Belt-and-braces on Sonnet tool inputs.** Prompt rules alone can drift. Defence-in-depth in `executeTool` (regex-strip prefaces, infer implicit-today, etc) means the same class of bug can't regress silently. This pattern should be applied to any tool where the shape of Sonnet's input matters (calendar events, shopping items, etc).
- **In-flight guards for any concurrent-safe operation.** iCal sync's `_syncInFlight` Map pattern should be reused for any Edge Function or client method that (a) can be triggered from multiple places and (b) has non-atomic write side effects. Applies to future push notification dispatch, memory extraction runs, etc.
- **Skip destructive ops when read was incomplete.** The `allReadsSucceeded` pattern (only DELETE stale rows if the READ pass fully succeeded) generalises: any sync-like operation that computes "what to remove" from an observed set MUST only act if the observation was complete. If a data source failed partial, treat it as unknown, don't delete.
- **Anthropic-cached prompt boundary + Sonnet mental model.** The stale line 2636 ("use add_todo instead") kept contaminating Sonnet's routing decisions even after tool_choice forced add_reminder. Removing the stale rule mattered as much as the new one. Any time a subsystem is added/replaced (reminders replaced add_todo self-remind path), audit the entire prompt for stale references.
- **`sendEmailInline` `force` flag pattern.** Dedup is right for normal grant/invite paths (accidental double-click = no double email). Manual "resend" actions must bypass dedup — that's the whole reason they exist. Any future email action button that supports "resend" should use `force:true`.
- **Per-user data isolation via RLS ≠ per-user data DELIVERY.** iCal sync respects RLS (Andy's personal events only visible to Andy in queries). But if a downstream consumer (the brief scheduler) queries as service_role and only groups by family, personal-scoped data is invisible even though it exists. Per-user briefs are the correct architecture — every future family-shared feature needs to think about whether the shared version is really what the user experiences.
- **Existing silent bugs surface as side-effects of new work.** Rewriting brief-scheduler for per-user surfaced the `text:` vs `brief_text:` bug that had been silently killing server-side upserts for weeks. Client-side path was silently the only real writer. Whenever rewriting an area, take a moment to audit the surrounding code for latent bugs — they often surface for free.
- **`lib/calendar-sync.ts` UPDATE must preserve user-mutable columns.** After Bug 6 shipped, if the next iCal sync had reset privacy_scope back to 'personal', the user's manual share would silently vanish. Any future user-toggleable column on synced entities needs the same strip-on-update treatment.

### Files touched Session 36

**NEW SQL:**
- `supabase-repair-stale-assignees.sql` — dry-run + apply pattern for stale assignee UUID cleanup
- `supabase-kara-email-backfill.sql` — one-shot fix for Kara's missing welcome email_log row
- `supabase-zaeli-briefs-per-user.sql` — user_id column + swap unique constraint for per-user brief cache

**MODIFIED (client):**
- `app/(tabs)/index.tsx` — 4 edits for reminders regression (Bug 1); userId param for generateBrief call; share-toggle handler + UI in CalSheetEventCard (Bug 6)
- `app/(tabs)/dashboard.tsx` — avatar render branches for iPhone icon / placeholder / letter (Bug 3)
- `app/_layout.tsx` — 5s AppState foreground sync debounce (Bug 2)
- `lib/calendar-sync.ts` — `_syncInFlight` guard, `allReadsSucceeded` skip-delete, preserve privacy_scope on UPDATE (Bugs 2 + 6)
- `lib/brief-generator.ts` — per-user cache (userId param, filter, upsert onConflict) (Bug 5)

**MODIFIED (server):**
- `supabase/functions/admin-actions/index.ts` — `force` param on `sendEmailInline`, rewired `resend_welcome`, NEW `resend_beta_list_confirmation` action (Bug 4)
- `supabase/functions/brief-scheduler/index.ts` — REWRITTEN for per-user briefs + fixed silent `text:`/`brief_text:` bug (Bug 5)

**MODIFIED (admin console):**
- `zaeli-app-links-template/public/admin/index.html` — split Resend welcome UI into two clearly-labelled buttons with descriptions + NEW beta-list handler

### Remaining / next up

1. **Rich runs 3 SQL migrations in order** — briefs-per-user first (safe), stale-assignees dry-run then apply (review first), Kara backfill (safe)
2. **Deploy 2 Edge Functions** — admin-actions + brief-scheduler
3. **Push admin console to Netlify** — auto-deploys
4. **EAS production build for Build 54 + submit**
5. **Verify with Andy** on TestFlight — reminders time-based, iCal no flicker, no "?" avatars, morning brief includes iPhone events
6. **Test resend_welcome** — on a test account, verify email_log row lands with `triggered_by='resend'`
7. **Build 55 — widgets + long-press icon shortcuts** — foundation packages already installed; fresh session tomorrow

### Prior Session 35 (16 August late, same-week predecessor) — kept as compact reference

Session 35 shipped six EAS production builds (47→48→49→50→51→52→53) and covered:
- **Full iCal two-way sync SHIPPED (Builds 49→52)** — SQL foundation + read-in (Build 49), write-back with ensureZaeliCalendar + mirrorZaeliEventsToEventKit (Build 50), timezone hotfix (Build 51), update/delete propagation + onboarding step + white-on-white DateTimePicker fix + MIRROR_SCHEMA_VERSION auto-remediation (Build 52)
- **api_logs INSERT RLS policy FIXED** — 3 days of silent logging failure since ~12 Aug when Session 33 admin console SQL enabled RLS without an INSERT policy. Same lesson as Session 21.
- **Home tiles instant-load (Build 53)** — `lib/home-cache.ts` cache-then-refresh pattern; per-family per-tile AsyncStorage cache; ~50ms paint vs 2-3s blank flash
- **family_members duplicate cleanup + name-match guard** — ensureOwnMembership was inserting duplicates
- **Website — iPhone Calendar sync section shipped** — new headline feature section on `zaeli.app/`
- **Build 54 scoped as widgets + long-press + share personal event** — foundation packages installed (`@bacons/apple-targets` + `react-native-shared-group-preferences`), but deferred to fresh session. Session 36 pivoted from widgets to bug bash based on Rich's real-user feedback.
## ══════════════════════════════════

### NEW THIS SESSION (Session 35 — 15 August late, 6 EAS builds shipped, multiple commits pending push)

**A. iCal read-in foundation (Build 49) ⭐⭐⭐.** Full per-user opt-in calendar sync foundation deployed.
- **NEW `supabase-calendar-sync.sql`** DEPLOYED — adds `events.source` (values: `zaeli`, `apple`, `google`), `events.external_id`, `events.external_calendar_id`, `events.imported_by_user_id`, `events.privacy_scope` (`personal` | `shared`), `events.synced_at`. NEW `calendar_sync_config` table (per-user JSONB tracking selected iOS calendars + last-sync timestamp + Zaeli-owned calendar id). RLS updated: SELECT enforces `privacy_scope = 'shared' OR imported_by_user_id = auth.uid()` — personal-scope events only visible to the person who imported them. Andy's work meetings only visible to Andy in Zaeli; Anna/Nadia never see them.
- **NEW `lib/calendar-sync.ts`** — `requestCalendarPermission()`, `listUserCalendars()` (returns iOS calendars with title/source/type/color), `loadSyncConfig()` / `saveSyncConfig()` / `setCalendarSyncEnabled()`, `syncNow()` (read-in with delta detection).
- **NEW Settings CalendarSyncView** — full opt-in-per-calendar picker (checkbox per iOS calendar), "Sync now" button, disconnect button.
- **`app.json` updated** — `expo-calendar` plugin added with `NSCalendarsFullAccessUsageDescription` entitlement.

**B. iCal write-back (Build 50) ⭐⭐⭐.** Zaeli events now mirror to a "Zaeli" EventKit calendar Andy can see alongside his Work + Home + School calendars on iPhone.
- **NEW `supabase-calendar-writeback.sql`** DEPLOYED — `events.mirrored_apple_id` text column + partial index (only rows with a mirror id are indexed).
- **NEW `ensureZaeliCalendar()`** — creates or fetches Zaeli's own EventKit calendar. First attempt used strict `s.type === CALDAV && s.name === 'iCloud'` matching — failed silently because iOS source names vary (`iCloud`, `iCloud (Personal)`, etc). Fixed with loose case-insensitive iCloud match. Also: `createCalendarAsync` params include Android-only fields that iOS rejects — fixed with iOS-only param subset.
- **NEW `mirrorZaeliEventsToEventKit()`** — full reconciliation. Reads all events where `source='zaeli'` for current user's family, compares with mirrored events in EventKit, INSERTs new, UPDATEs changed, DELETEs removed. Records `mirrored_apple_id` back to Supabase so next reconciliation is a diff, not a full re-mirror.
- **NEW `wipeAllMirroredEvents()`** — used by disconnect flow.

**C. Timezone HOTFIX (Build 51) ⭐⭐.** After Build 50 landed, Rich confirmed events were being mirrored — but at completely wrong times. 7am Brisbane event showed as 5pm on iPhone. Root cause: Postgres `timestamp with time zone` returns ISO with `+00` suffix; iOS `EventKit.saveEventAsync` treated the string as literal UTC and applied its own timezone conversion → +10 hours off. Fix in `parseIsoAsDate` (mirror of reminders' `parseLocalIsoAsDate` from Session 32 v2 Round A): strip any `+00`/`+10`/`Z` suffix, construct Date from local components. Rich verified — events now mirror at correct times.

**D. Update/delete propagation + onboarding + white-on-white fix + auto-remediation (Build 52).** The final polish pass.
- **Update propagation** — editing a Zaeli event updates the mirrored EventKit event via `updateEventAsync`. Delete propagation — deleting a Zaeli event removes the mirror. Both wired into `mirrorZaeliEventsToEventKit()` reconciliation.
- **Onboarding step** — NEW `CalendarSyncStep` added as step 7 in `app/onboarding/index.tsx` (informational only; the real per-calendar picker lives in Settings). Steps renumbered 1-10.
- **White-on-white DateTimePicker fix** — 3 places were missing `themeVariant="light"` prop on `<DateTimePicker>`: `app/onboarding/index.tsx:862`, `app/(tabs)/index.tsx:8958`, `app/(tabs)/our-budget.tsx:1602`. Text was invisible on modal in bright-mode iOS.
- **Auto-remediation via `MIRROR_SCHEMA_VERSION`** — constant in `lib/calendar-sync.ts`. `calendar_sync_config.mirror_schema_version` column (`supabase-calendar-mirror-v2.sql`) tracks last-completed version per user. On app open, if user's version < current constant → force one-time full re-mirror + bump version. Lets future schema changes auto-recover without user action.
- **Duplicate RLS cleanup** along the way: Session 21 events policies (`events_select_family` etc) + Build 48 additions (`events select`) were BOTH permissive → OR wins → Anna would have seen Rich's personal events. Dropped the 4 old policies.

**E. Home tiles instant-load — cache-then-refresh (Build 53) ⭐.** Rich reported "every app open, all Home tiles blank for 2-3 seconds before real data appears — feels broken". Fixed with per-family per-tile AsyncStorage cache.
- **NEW `lib/home-cache.ts`** — `loadTile<T>(familyId, tileKey)` / `saveTile<T>(familyId, tileKey, data)` / `clearFamilyCache(familyId)` / `clearAllHomeCache()` / `saveLastFamilyId(fid)` / `getLastFamilyId()`. `CACHE_KEYS` constants for event/shopping/reminder/radar/budget/meal tiles.
- **Dashboard hydrate flow**: on mount → `getLastFamilyId()` → parallel `loadTile()` for every tile → paint in ~50ms. Then fetch fresh from Supabase in background → `saveTile()` → silently update state if changed. `saveLastFamilyId(fid)` after every successful fetch (profile isn't loaded during Session 30 splash-latency fix flow — need a separate stash).
- **Sign-out invalidation** — `onAuthChange` in `_layout.tsx` now also calls `clearAllHomeCache()` (safer than needing to know which family was signed in).
- **Foreground sync** — new AppState listener in `_layout.tsx` on `background → active` triggers iCal sync + Home tile refresh. Returning from Safari/Notes/anywhere → fresh data without user interaction.

**F. api_logs RLS INSERT policy MISSING — 3 days of silent data loss FIXED ⭐⭐.** Session 33's admin console SQL enabled RLS on `api_logs` but only created a SELECT policy for admins. Since ~12 Aug, EVERY `logApiCall()` insert has been silently rejected — the api_logs table showed zero new rows for 3 days. Discovered when checking "did Andy use app today?" for engagement — admin console showed zero. Fix: `CREATE POLICY "api_logs insert authenticated" ON public.api_logs FOR INSERT WITH CHECK (family_id = public.current_family_id())`. Cost visibility restored. Lesson (same as Session 21): enabling RLS without policies = deny-everything.

**G. Family_members duplicate cleanup + name-match guard.** After Build 47's `ensureOwnMembership()` shipped, some accounts got DUPLICATE rows (Richard+Richard, Anna+Anna in the same roster). Root cause: `ensureOwnMembership` INSERTed a new row when it couldn't find one keyed by auth.uid — but a name-match row already existed from onboarding with a different id. Fix in `lib/family-roster.ts`: before INSERT, ALSO check for existing row with same name in family. If found → patch its id to auth.uid instead of inserting new. Idempotent, prevents future dupes. Rich manually cleaned existing duplicates via SQL.

**H. Website — iPhone Calendar sync section shipped.** `zaeli-app-links-template/public/index.html` — NEW Section 3.5 "iPhone Calendar sync" between Lockscreen and Reminders sections (mint bg, phone mockup showing calendar picker + Sync now button + "Zaeli events appear in iPhone. iPhone events appear in Zaeli." headline). Positioned as a headline feature. Privacy page (`privacy.html`) updated with iPhone Calendar events disclosure bullet. Deployed via Cloudflare Pages auto-deploy — Rich forgot last enter on cmd first push, second attempt pushed both files successfully.

**I. Build 54 SCOPED (deferred to fresh session).** Locked scope for tomorrow:
- **(1) Small Home widget** = mic launcher. Tap opens chat with mic active.
- **(2) Medium Home widget** = today's events + next reminder (from Shared App Group UserDefaults).
- **(3) Lock Screen circular** = Zaeli logo. Tap opens app.
- **(4) Lock Screen inline** = one-line today summary text.
- **(5) Long-press Zaeli icon** = 4-action menu (Start with voice / Today / Shopping / Quick add) via Info.plist `UIApplicationShortcutItems` + `zaeli://` URL scheme routing.
- **(6) Share personal event manually** = privacy_scope toggle in event edit sheet (pure JS, one Supabase UPDATE). Turns a personal event → shared visibility for the whole family.
- **Foundation packages installed tonight**: `@bacons/apple-targets` (Expo config plugin for native iOS widget target) + `react-native-shared-group-preferences` (writes to iOS Shared App Group UserDefaults — swapped from `expo-shared-preferences` which was Android-only). Rich chose to defer the actual widget implementation to a fresh session tomorrow to give the native Swift/SwiftUI/WidgetKit work proper focus.

**J. Andy engagement diagnosis.** Andy's last api_log was 14 Aug 21:14 (though api_logs was broken from ~12 Aug so real last use may be later). Never sent Nadia her invite. Silence for 24+ hours. Rich sent direct text with iPhone Calendar sync screenshot to spark re-engagement. If Andy comes back and uses iCal sync, that's the killer feature. Nadia invite pending 2 days unaccepted. Damien Android never installed the sideload APK.

**K. Doc sweep + commit + push.** This session updated CLAUDE.md, ZAELI-PRODUCT.md, new-chat-handover.md with Session 35 state. Full commit + push at end.

### Key decisions / learnings Session 35

- **Ship an incremental multi-build feature as an incremental multi-build feature.** iCal sync could have been one big drop of 15 files — instead Builds 49 → 50 → 51 (hotfix) → 52 each shipped a testable checkpoint. Build 51 caught the timezone bug in a small surface area; catching it in Build 52 with everything else would have been much harder to isolate.
- **Same timezone parsing pattern applies everywhere.** `parseIsoAsDate` in `lib/calendar-sync.ts` is a direct mirror of `parseLocalIsoAsDate` in `lib/reminders.ts` (Session 32 v2 Round A). Anywhere iOS reads a Postgres tz-aware timestamp for display or scheduling, apply the same strip-tz-suffix-then-local-construct pattern. It should probably become a shared helper in `lib/date-utils.ts` eventually.
- **Cache-then-refresh is the right pattern for any "list of things" tile.** Load from AsyncStorage → paint fast → fetch fresh → silently update. Applies beyond Home tiles: sheet lists (Shopping, Reminders, Meal Plan), calendar month grids, any surface where paint speed matters more than freshness on first frame.
- **Cache MUST be invalidated on sign-out.** `clearAllHomeCache()` is the safety net — don't rely on per-family clearing which requires knowing which family was signed in (which is tricky at sign-out time). Same principle applies to any future per-user cache.
- **RLS with no INSERT policy = silent data loss.** Second time this session's memory this has bit us (Session 21 was first). Any table with existing writes MUST have its INSERT policy defined BEFORE enabling RLS. Add a startup check to admin console that queries `api_logs` count over last 24h — if it's zero, alert.
- **`ensureOwnMembership` name-match guard** — auto-heal helpers must guard against creating duplicates. If the "person doesn't have a row" check has multiple ways to be true (auth.uid missing OR name-match with different id), the fix path must handle both.
- **iCal reconciliation is more complex than sync.** INSERT-only sync is trivial. UPDATE + DELETE propagation requires tracking `mirrored_apple_id` on Supabase side + comparing sets on every sync. Auto-remediation via `MIRROR_SCHEMA_VERSION` lets future schema changes recover without user action.
- **Anthropic API caching = don't mix user data into cached blocks** (Session 34 lesson still applies). Applied same principle to the Sonnet system prompt structure this session — nothing new hardcoded.
- **Native iOS work needs a fresh session.** Widget bring-up has 20-30 min EAS cycle time per iteration. Rich making the call to defer Build 54 was right — starting the Swift work at end-of-day when tired is a recipe for hard-to-diagnose bugs.

### Files touched Session 35

**NEW SQL:**
- `supabase-calendar-sync.sql` — Build 49 iCal foundation (events privacy_scope + external_id + calendar_sync_config table + RLS)
- `supabase-calendar-writeback.sql` — Build 50 events.mirrored_apple_id column
- `supabase-calendar-mirror-v2.sql` — Build 52 calendar_sync_config.mirror_schema_version column

**NEW libs:**
- `lib/calendar-sync.ts` — Full sync engine (permission, list calendars, config CRUD, syncNow with reconciliation, ensureZaeliCalendar, mirrorZaeliEventsToEventKit, wipeAllMirroredEvents, parseIsoAsDate)
- `lib/home-cache.ts` — Per-family per-tile AsyncStorage cache

**MODIFIED (client):**
- `app.json` — expo-calendar plugin, NSCalendarsFullAccessUsageDescription entitlement
- `app/_layout.tsx` — auto-sync-on-app-open (Build 50) + AppState foreground sync listener (Build 53) + clearAllHomeCache on sign-out + name-match guard in ensureOwnMembership flow
- `app/(tabs)/dashboard.tsx` — cache hydrate on mount using getLastFamilyId, saveTile after every fetch, saveLastFamilyId(fid)
- `app/(tabs)/settings.tsx` — NEW CalendarSyncView (permission gate + per-calendar picker + Sync now + disconnect), iPhone Calendar sync row in Preferences
- `app/onboarding/index.tsx` — NEW CalendarSyncStep (step 7 informational) + white-on-white DateTimePicker fix (themeVariant="light") + steps renumbered to 10
- `app/(tabs)/index.tsx` — white-on-white DateTimePicker fix at line 8958
- `app/(tabs)/our-budget.tsx` — white-on-white DateTimePicker fix at line 1602
- `lib/family-roster.ts` — name-match guard added to ensureOwnMembership (prevents duplicates)

**MODIFIED (website):**
- `zaeli-app-links-template/public/index.html` — NEW Section 3.5 "iPhone Calendar sync" between Lockscreen and Reminders
- `zaeli-app-links-template/public/privacy.html` — iPhone Calendar events disclosure bullet added

**DEPLOYED (external):**
- `supabase-calendar-sync.sql` run in Supabase Studio (Build 49)
- `supabase-calendar-writeback.sql` run in Supabase Studio (Build 50)
- `supabase-calendar-mirror-v2.sql` run in Supabase Studio (Build 52)
- `CREATE POLICY api_logs insert authenticated ...` run in Supabase Studio (fixes 3-day logging gap)
- 6 EAS production builds (47, 48, 49, 50, 51 hotfix, 52, 53) each `eas submit --platform ios --profile production --latest` to Public Beta
- Cloudflare Pages auto-deploy for website homepage + privacy page

**PACKAGES INSTALLED (Build 54 foundation, not yet used):**
- `@bacons/apple-targets` — Expo config plugin for native iOS widget target
- `react-native-shared-group-preferences` — writes to iOS Shared App Group UserDefaults (widget data pipeline)

### Remaining / next up

1. **Build 54 — fresh session tomorrow.** Widget foundation (`app.json` widget extension config via `@bacons/apple-targets` + Shared App Group entitlement), Swift widget entry point + TimelineProvider + SwiftUI views for 4 widget variants, JS-side data writing via `react-native-shared-group-preferences`, dashboard.tsx writes widget data on every fetch, `WidgetCenter.reloadAllTimelines()` triggering (may need small custom native bridge). PLUS long-press icon shortcuts (pure app.json config + `zaeli://` URL routing). PLUS share personal event toggle (privacy_scope UPDATE in event edit sheet).
2. **Monitor Andy engagement.** iCal sync is the bet. If he comes back and engages, great. If silent past 48h from re-engagement text (Sunday 17 Aug), personal SMS + honest question.
3. **Nadia invite** still pending 2 days. Could offer Andy a "resend invite" flow but not urgent.
4. **Sonnet 5 migration** — still queued for post-beta stability.
5. **Damien APK** — no action needed until he asks.
6. **Website testimonial section** once Andy or Nadia says something quotable.

### Prior Session 34 (same day, earlier) — kept as compact reference

Session 34 shipped Builds 47 + 48 earlier the same day. Headlines:
- **Build 47** — Andy's Add Event RLS bug (hardcoded DUMMY_FAMILY_ID literal at `index.tsx:2979` → NEW `awaitFamilyId()` async helper); Option C Our Family fully editable (`supabase-family-editing.sql` + EditNameSheet + ColorPickerSheet + real Remove-from-family + kid Budget toggle persisted); admin-announce SMTP false-failure fix (return 200 immediately, SMTP via EdgeRuntime.waitUntil); brief-scheduler fresh-signup exemption for dormant gate.
- **Build 48** — Owner missing from own family list (NEW `ensureOwnMembership()` + proactive `_layout.tsx` auto-heal + onboarding fix); cross-family name leak (moved FAMILY line from Anthropic-cached `staticSystem` to `dynamicSystem` — Anthropic caches at 90% discount, so hardcoded Rich family names were baked into every user's context); anti-snowball rules added to CALENDAR HONESTY block (NO META-COMMENTARY / call find_calendar_events FIRST / recurring defaults); recurring event code verified clean; iCal sync scoped as 3-build plan (Session 35 shipped the entire roadmap); Android path decided (fast-track APK for close family via new admin console templates).

---

## ══════════════════════════════════
## PRIOR STATE — SESSION 32 v2 ROUND A · MAJOR VISION PIVOT (kept for context)
## ══════════════════════════════════
