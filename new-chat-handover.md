...
**Session 34 (15 August) is the biggest real-user-feedback iteration since Anna's beta rounds.** Andy (second real beta user) hit six real bugs across a single test session that surfaced deep issues: hardcoded DUMMY_FAMILY_ID in one code path, entire owner-missing-from-family-list gap, cross-family name leak via cached Sonnet prompt, "vanishing act" prompt-drift meltdown. All fixed and bundled into Build 48 (currently uploading to TestFlight). Damien Pagnin signed up but has Android — decision made to fast-track via direct-install APK for close family/friends while proper Google Play Store path builds in background over 6-8 weeks. iCal two-way sync (Andy's ask) fully scoped as Build 49+ multi-build plan with SQL foundation ready but code deferred until Build 48 stability confirmed with real users.

**What Rich needs to do next (in order):**
1. Wait for Build 48 to finish uploading, then `eas submit --platform ios --profile production --latest`
2. App Store Connect → answer compliance ("None of the algorithms mentioned above") → add Build 48 to Public Beta group
3. Deploy the updated brief-scheduler Edge Function: `supabase functions deploy brief-scheduler` (fixes Andy's name leak in tomorrow's morning brief)
4. Message Andy: apologise for the multiple bugs, tell him update is landing shortly, ask him to test recurring events + verify he sees himself in Our Family
5. Push admin console update to Netlify if not already done (Android email templates)
6. Damien: send him the 🚪 Android back door email once you're ready to build his APK
7. Andrew Fraser (`fraggle@me.com`) still hasn't installed after 3+ days — personal SMS/DM if you know him

**Next session priorities** (order):
1. Verify Build 48 fixes work end-to-end with Andy on real device (family list, chat name leak, recurring events)
2. If Andy stable → resume Build 49+ iCal two-way sync work (foundation SQL exists, next step is client-side)
3. If more bugs from Andy → hotfix cycle
4. Damien APK build once he opts in
5. Sonnet 5 migration (still queued for post-beta stability)
6. Website v6 updates for iCal sync + Android messaging

---

## ══════════════════════════════════
## CURRENT STATE — SESSION 34 · REAL BETA + FOUNDATION HARDENED ✅
## ══════════════════════════════════

### NEW THIS SESSION (Session 34 — 15 August, multiple commits pending push)

**A. Admin-announce false-failure fix (deployed).** `HTTP 546 CPU Time exceeded` on Zoho SMTP path meant client saw "failed" while email actually delivered. Root cause: `await client.send()` burned enough CPU (TLS handshake + DATA transmission) to trip Supabase's ~200ms budget before response could ship. Fix in `supabase/functions/admin-announce/index.ts`: return 200 immediately, run SMTP in background via `EdgeRuntime.waitUntil`. Trade-off: rare Zoho SMTP failures now invisible to client (real audit trail belongs in `email_log` — deferred). Ship the code fix in this session, deployed via `supabase functions deploy admin-announce`.

**B. Brief-scheduler dormant-gate fresh-signup exemption (deployed).** Session 33's Batch 6 added "skip families with no api_logs in last 7 days" gate. But brand-new users have zero api_logs BY DEFINITION → their first-ever morning brief never fires → the exact re-engagement hook we wanted for new users was blocked. Fix in `supabase/functions/brief-scheduler/index.ts`: any family with a profile <7 days old is exempt from the dormant gate. Deployed via `supabase functions deploy brief-scheduler`. Andy will get his first morning brief tomorrow.

**C. Andy's Add Event RLS bug — hardcoded DUMMY_FAMILY_ID literal missed by Session 21 sweep.** Session 21's `FAMILY_ID` constant sweep replaced explicit references but MISSED an inline string literal at `app/(tabs)/index.tsx:2979` in the calendar sheet Save handler. Every sheet-added event has been trying to INSERT with `family_id: '00000000-0000-0000-0000-000000000001'` (DUMMY_FAMILY_ID). Rich's chat-tool adds worked (they use `getFamilyId()`) — nobody noticed. Andy hit it first Add Event → "new row violates row-level security policy for table 'events'". Fix: NEW `awaitFamilyId()` async helper in `lib/family.ts` that blocks briefly (polls every 60ms, max 4s) waiting for profile load — throws with friendly error instead of falling back to DUMMY. Sheet Save handler now `await awaitFamilyId()` before building payload.

**D. Option C — Our Family member profile fully editable ⭐⭐⭐.** Andy discovered "nothing tappable" on member profile screens. Full backend + client fix:
- **NEW `supabase-family-editing.sql`** — profiles.budget_access column (persists kid Our Budget toggle) + tighter family_members UPDATE RLS to (owner OR self) + tighter DELETE RLS to owner-only + `remove_family_member(target_id uuid)` SECURITY DEFINER RPC that verifies caller is owner, verifies target is in caller's family, refuses to remove owner, deletes family_members row + nulls profiles.family_id to revoke RLS access. Idempotent. SET search_path = public, auth (Session 21 lesson).
- **NEW helpers in `lib/family-roster.ts`**: `updateMemberName` (writes family_members.name + profiles.name), `updateMemberColor` (writes family_members.colour), `removeMember` (calls RPC), `getBudgetAccess` / `setBudgetAccess` (persists kid Budget toggle).
- **UI in `app/(tabs)/family.tsx`**: EditNameSheet (text input modal), ColorPickerSheet (8-swatch palette: Blue/Coral/Purple/Green/Amber/Magenta/Teal/Slate), tap-to-edit Name/Colour rows, real Remove-from-family button with confirmation. Kid Budget toggle now persists (was local-state-only).

**E. Owner missing from own Family list — HOTFIX ⭐⭐⭐.** Andy signed up, added Nadia/Neave/Louie in onboarding, opened Our Family — himself absent. Root cause at `app/onboarding/index.tsx:391`: `family.filter(m => m.id !== 'me')` explicitly excluded the owner from family_members insert. Comment justified as "family_members is for OTHER family members" — WRONG. Every screen (Our Family, Calendar avatars, Meal cook picker, event assignee resolution) treats family_members as THE authoritative roster. Fix in five parts:
- **NEW `ensureOwnMembership()` helper** in `lib/family-roster.ts` — checks if signed-in user has a family_members row keyed by auth.uid, INSERTs one from profile data if missing (id = auth.uid, name from profile, colour from profile.colour or palette default, role='parent' unless kid). Idempotent — no-op if row already exists.
- **`app/_layout.tsx` proactive heal** — fires on every app open right after `loadProfile()` completes. Runs in background before user navigates anywhere.
- **`app/(tabs)/family.tsx` safety net** — same check on Our Family load in case the proactive heal missed.
- **Onboarding fix** — removed the filter, owner inserted with `id = profile.id` matching auth.uid.
- **Invitee flow fix** — `app/invite/[token].tsx` `finishAdult` + `finishKid` both call `ensureOwnMembership()` right after `loadProfile()`.
- Invariant now enforced: `family_members.id === profiles.id === auth.uid()` for the person themselves.

**F. Cross-family name leak in Sonnet system prompt — CRITICAL FIX ⭐⭐⭐.** Andy's Zaeli asked "who's it for (Poppy? Anna?)" for his Netball event — RICH'S family names appeared in Andy's conversation. Root cause at `app/(tabs)/index.tsx:4676`: hardcoded `FAMILY: Rich (logged in), Anna, Poppy (Yr6, age 12), Gab (Yr4, age 10, boy), Duke (Yr1, age 8, boy).` line INSIDE the cached static system prompt. Anthropic caches static text at 90% discount — so Rich's family names were baked into every user's context. Fix:
- Moved FAMILY line from `staticSystem` (cached) to `dynamicSystem` (uncached), built at prompt-build time from `getRoster()` + `getProfile()`. Format: `FAMILY: <name> (logged in), <name>, <name> Yr<n>, <name> Yr<n>.` — includes real roster, owner tagged "(logged in)", kids show year level.
- Empty roster fallback: "FAMILY: just <first name> (logged in) for now — no other members added yet."
- Genericized hardcoded example names ("Poppy's pasta" → "The bolognese").
- Removed hardcoded "Rich" reference ("Never tell Rich to do things" → "Never tell the user...").
- Same fix in `lib/brief-generator.ts` — 6 hardcoded name references (Poppy/Gab/Duke in kid-parent-driver examples + chip examples + morning/evening body examples) replaced with placeholders + explicit "USE ONLY REAL FAMILY NAMES FROM PROVIDED FAMILY LIST" rule.
- Same in `supabase/functions/brief-scheduler/index.ts` — hardcoded "Duke's soccer" → "<kid>'s soccer" pattern.
- **LOCKED RULE**: never hardcode family-specific data in cached prompts. Ever.

**G. Anti-snowball rules added to CALENDAR HONESTY block.** Andy saw Zaeli say "If it still doesn't show up, that's the calendar doing its little vanishing act again" — Sonnet snowballing on past failed turns, inventing external causes to avoid responsibility. Three new rules in `app/(tabs)/index.tsx`:
- **NO META-COMMENTARY ON FAILURES** — explicit ban on "vanishing act", "sounds like it never landed", "the system's being flaky", any external-blame phrasing. Each turn starts CLEAN. If earlier turns went badly, DO NOT reference them.
- **WHEN USER ASKS "DID YOU ADD X?"** — call `find_calendar_events` FIRST. Don't assume it didn't save. Only claim missing after the search returns zero.
- **RECURRING DEFAULTS** — title + weekday + time is enough. Don't ask "who's it for" or "when should it end". Infer sensible defaults (assignee = family, open-ended = 12 months).

**H. Recurring event code verified clean.** Traced `generateRecurrenceDates()` + batch insert path in `app/(tabs)/index.tsx`. Enum-locked schema forces Sonnet to pick from `daily/weekdays/weekly/fortnightly/monthly`. Silent-RLS block detection via `.select('id')` returns TOOL_FAILED if 0 rows land. 12-month horizon, 400-row cap. Andy's Netball bug was prompt drift (Sonnet over-asking then meta-commenting) not code — the anti-snowball rules address it.

**I. iCal sync — full two-way roadmap SCOPED, foundation SQL READY, code deferred to Build 49+.** Andy asked for two-way sync between iPhone Calendar and Zaeli. Decisions locked: per-user external sync (Andy's iPhone events show only for Andy in Zaeli, Anna doesn't see his work meetings), opt-in per calendar (picker for his 8+ iOS calendars), full two-way not phased. Roadmap 3 builds:
- **Build 49**: SQL foundation deployed + expo-calendar native module installed + permission dance + per-calendar picker in Settings + read-only sync (iPhone events → Zaeli). ~4-5 days.
- **Build 50**: Zaeli creates its own "Zaeli" EventKit calendar + mirrors all Zaeli events to it + write-back working. ~2-3 days.
- **Build 51**: Delta detection + deletion propagation both ways + recurring event RRULE mapping + Android parity. ~3-4 days.
- **NEW `supabase-calendar-sync.sql` DRAFTED** (not yet run): events.source/external_id/external_calendar_id/imported_by_user_id/privacy_scope/synced_at columns, new calendar_sync_config table (per-user JSONB of selected iOS calendars + Zaeli-owned calendar id), updated events RLS to enforce privacy_scope=personal only visible to importing user.

**J. Android path SCOPED.** Damien Pagnin (Rich's cousin) signed up but has Android. Decision: fast-track close family/friends via direct-install APK (EAS build → download link → sideload with "install from unknown sources" warning), public Google Play Store internal testing track waits 6-8 weeks. Two new admin console email templates deployed:
- **🤖 Android waitlist** — generic 6-8 weeks holding line for public Android signups. Warm, honest, no ask.
- **🚪 Android back door — only if you're up for it** — fun conspiratorial invite for cousins/friends. Honest about scary "install from unknown sources" warning + no push notifications yet (Firebase FCM setup pending) + manual reinstall every few weeks for updates + guinea pig role. "Reply 'in'" opt-in flow.
- Both templates deployed to admin console (`zaeli-app-links-template/public/admin/index.html`) + pushed to zaeli-app-links repo → Netlify.

**K. Doc sweep + commit + push.** This session's largest single ship — updated CLAUDE.md, ZAELI-PRODUCT.md, new-chat-handover.md with Session 34 state. Full commit + push at end.

### Key decisions / learnings Session 34

- **Never hardcode family-specific data in cached prompts.** Anthropic caches static prompts at 90% discount, but that means every user gets the same content. Family/user-specific data MUST live in the dynamic/uncached block. Cost impact of moving small blocks (FAMILY line = ~50 tokens) to dynamic is negligible.
- **Trigger auto-heal on app open, not just on the target screen.** Downstream screens can't assume the roster is correct. Fire the heal in `_layout.tsx` after auth so ALL screens benefit — not just the one that surfaces the bug.
- **Invariant: `family_members.id === profiles.id === auth.uid()` for the same person.** Enforced by onboarding + invitee flow + auto-heal. Every future feature that needs to look up "this person's row" relies on this.
- **Silent-RLS detection is essential** — `.select('id').maybeSingle()` after insert exposes phantom writes. If we hit another RLS gap later, apply this pattern immediately.
- **Sonnet prompt drift needs explicit anti-snowball rules.** Long conversations with past failures snowball. Each turn needs to be told "start CLEAN, ignore past drama". Applies to any long-lived conversation surface.
- **iCal sync ship in incremental builds even if the FEATURE spec is "full two-way".** Each build is a testable checkpoint. Trying to land 2 weeks of sync logic in one drop is reckless — first bug = full rollback.
- **For close-family Android: direct-install APK is fine.** For public: waits for Google Play internal testing track. The two paths coexist.

### Files touched Session 34

**NEW:**
- `supabase-family-editing.sql` — Option C RLS + remove_family_member RPC + budget_access column
- `supabase-calendar-sync.sql` — iCal foundation (drafted, NOT deployed)

**MODIFIED (client):**
- `lib/family.ts` — NEW `awaitFamilyId()` async helper
- `lib/family-roster.ts` — 6 NEW helpers (updateMemberName / updateMemberColor / removeMember / getBudgetAccess / setBudgetAccess / ensureOwnMembership)
- `app/(tabs)/index.tsx` — cal-sheet-save DUMMY_FAMILY_ID → awaitFamilyId, FAMILY dynamic in prompt, anti-snowball rules, genericized example names
- `app/(tabs)/family.tsx` — EditNameSheet + ColorPickerSheet + tap-to-edit rows + persisted Budget toggle + real Remove + auto-heal safety net
- `app/onboarding/index.tsx` — removed 'me' filter, owner inserted with id = profile.id
- `app/invite/[token].tsx` — finishAdult + finishKid call ensureOwnMembership after signup
- `app/_layout.tsx` — proactive auto-heal fires on app open after loadProfile
- `lib/brief-generator.ts` — 6 hardcoded name references replaced with placeholders + explicit rule

**MODIFIED (server):**
- `supabase/functions/admin-announce/index.ts` — return-first-then-SMTP pattern (Batch 9 false-failure fix)
- `supabase/functions/brief-scheduler/index.ts` — fresh-signup exemption for dormant gate + genericized hardcoded name

**MODIFIED (admin console):**
- `zaeli-app-links-template/public/admin/index.html` — 2 new email templates (🤖 Android waitlist + 🚪 Android back door)

**DEPLOYED (external):**
- `supabase functions deploy admin-announce` — Batch 9 fix
- `supabase functions deploy brief-scheduler` — dormant-gate fix (Rich to also redeploy after Session 34 commits for the family-name genericization)
- `supabase-family-editing.sql` run in Supabase Studio — Option C ready
- Netlify auto-deploy for admin console templates

### Remaining / next up

1. **Wait for Build 48 EAS build to complete** → `eas submit --platform ios --profile production --latest` → App Store Connect compliance answer → add to Public Beta group
2. **Verify with Andy** on new build: (a) sees himself in Our Family, (b) no more Rich family names in chat, (c) recurring events work, (d) Name/Colour/Remove all editable
3. **If stable, resume Build 49+ iCal sync work** — deploy supabase-calendar-sync.sql, install expo-calendar, build sync engine
4. **Damien Android** — send back-door email → EAS Android build → send install link → he becomes first Android tester
5. **Andrew Fraser follow-up** — still hasn't installed after 3+ days, personal DM if Rich knows him
6. **Sonnet 5 migration** — queued for post-beta stability
