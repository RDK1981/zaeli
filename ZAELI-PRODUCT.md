...
*Last updated: 17 August 2026 late — Session 37 ✅ · **BRIEF ARCHITECTURE PIVOT — CHAT BRIEF KILLED · TIME-OF-DAY CHATTILE ON HOME · CHAT EMPTY-STATE GREETING · MARKETING KICK-OFF · COST INVESTIGATION → BUILD 60 BACKLOG.** Rich noticed a compound UX problem: evening brief still showing "clear day" at 10pm Monday when Tuesday was packed, plus the "morning brief still visible at 3:30pm" pattern inherent to putting a moment-in-time push on a persistent tile. Realisation: the brief was DESIGNED as a lockscreen push moment, not a widget. Chat is for INPUT, not for reading briefs. Full pivot in one session — kill client brief pipeline (4 `tryFireBrief` call sites + 5-min interval), replace Home BriefTile with a time-of-day ChatTile that taps into Chat, add chat empty-state greeting so users never land on blank screen · **⭐⭐⭐ Chat brief killed (Build 57)** — server-side brief-scheduler still fires lockscreen pushes at scheduled morning/evening times, unchanged. Client just no longer generates or displays briefs in chat. One less Supabase query per Home load, no more stale-cache/bucket-refresh/shadowing bugs to maintain. `zaeli_briefs` table still populated by server for possible future "what was in my morning brief?" chat retrieval · **⭐⭐⭐ Home ChatTile — 5 time-of-day windows (Build 58)** — replaces the earlier BriefTile experiment. Warm greeting card at top of Home shifts through the day (peach/sky/mint/lavender/slate) with matching greeting text ("Morning 👋 What's on your mind?" / "Hey — anything I can help with?" / "Afternoon 👋 What can I sort?" / "Evening — anything to knock off?" / "Still up? I'm here."). Tap → routes to Chat. Zero AI cost per render. Feels like Zaeli's persistent "I'm here" tap-in, not a static widget. Uses palette tokens already in the app · **⭐ Chat empty-state greeting (Build 59)** — Rich's call: "chat can't have a blank screen." When chat feed has no messages (fresh install, cleared history, post-brief-strip), a matching greeting card renders at top of feed. Same 5-window palette as Home ChatTile → visual continuity whether user tapped tile or swiped right. Instantly disappears on first message. Sub-line: "Type below or tap the mic to talk." · **⭐ Old brief messages stripped from persisted chat (Build 59)** — Rich installed Build 58 and briefs still visible. Root cause: AsyncStorage chat history file had briefs from BEFORE Session 37 kill still on disk (24h TTL only fires with 24h of no chat activity). Fix: `stripBriefs()` filter on hydrate + save (`m.isBrief || m.id?.startsWith('brief-')`). One-shot clears stale briefs on first open post-install · **⭐⭐ Cost investigation — Andy re-engagement uncovered imgCtx cache breakage** — Andy back one session 17 Aug 9-10pm Brisbane (3 messages with photos → 6 Sonnet calls → A$0.25 AUD). Diagnosed via api_logs token-level analysis: cache works within message pairs but breaks between messages when photos change, because `imgCtx` (image description ~2000 tokens per photo) lives INSIDE the cached prefix. New photo → new imgCtx → different bytes → cache miss → pays to write ~8700-token prefix again on every photo change. **Build 60 backlog (deferred by Rich — invisible to users, cost-only)**: (1) move imgCtx out of staticCached (~30% off photo sessions), (2) Sonnet 4.6 → Sonnet 5 intro pricing (~33% off always), (3) add cache_read + cache_creation columns to api_logs (observability blind spot right now), (4) trim CAPABILITY_RULES (~5000 tokens → target ~3000), (5) optional Sonnet follow-up → Haiku 4.5 for reflection turn (~25-30% overall, persona trade-off). Combined: A$5.86/mo Rich baseline → target A$2.10/mo (70% margin at A$6.99). Honest math correction from earlier claim of 10× reduction — real numbers are ~30% on photo sessions, 0% on text-only · **⭐ Marketing kick-off — Instagram + Facebook pages set up + 4-week content calendar drafted** — Rich set up @zaeli.app IG + Facebook Page. NEW browser-based FB cover PNG generator (tagline + explainer variants, 820×312 + @2x). NEW 4-week content calendar HTML: 12 posts × 3 platforms (IG/TT/FB Groups) × 4 weeks, **Mum POV throughout** (Rich course-corrected — mums are the acquisition wedge, not families), Australian-generic references (not Brisbane), mental-load themes. Weeks: Recognition → Curiosity → Reveal → Invite. Starts Aug 25 with Mon/Thu/Sun cadence. FB Groups strategy for stranger reach (bypass friends being weird about it). This is the first structured push to grow beyond warm network · **Andy engagement — briefly back, Nadia still not joined** — Andy came back for one photo-heavy session (school newsletter multi-event add). Server evening brief fired for him. Nadia invite from 14 Aug still pending 3 days later (invite named "Family" — probably fat-fingered). Rich needs personal nudge · **Prior Session 36 work still current** — Build 54 six-fix bug bash. **Pricing A$6.99/mo family plan tax-inclusive** (unchanged).*

*Legacy Session 36 header (kept for context): 17 August 2026 — Session 36 ✅ · **BUILD 54 BUG BASH — REAL BETA-USER-FACING QUALITY PASS.** Rich pivoted Build 54 from widgets to bug rectification after Session 35's iCal ship uncovered a cluster of real UX bugs. Six fixes bundled (all root-cause not band-aid): (1) reminders no longer route to To-dos with raw preface text; (2) iCal event flicker eliminated via in-flight sync guard + skip-delete-on-read-failure + AppState debounce; (3) "?" avatar replaced with iPhone icon (imported) or subtle placeholder (roster loading) — plus historical stale-assignee repair SQL; (4) admin console `resend_welcome` action rewired to actually send the welcome template (was silently sending the beta-list email — Kara received wrong template); (5) per-user briefs (Phase 85 promoted from queued) — Andy's brief now includes his iPhone Calendar events; (6) users can manually share imported iCal events with the family via one toggle · **⭐⭐⭐ Per-user brief architecture (the important one)** — server-side brief-scheduler rewritten from family-scoped to user-scoped. Each adult now gets a personalised brief that includes their own personal iCal events (via `imported_by_user_id` RLS) plus all family shared. Push goes only to that user's device. Cache keyed per-user via `supabase-zaeli-briefs-per-user.sql`. Same treatment on client-side `lib/brief-generator.ts` — each user sees THEIR brief on chat mount, not a shared one. Also caught an existing bug where the Edge Function had been silently upserting to a non-existent `text:` column instead of `brief_text` — server-side scheduler was writing nothing to cache for weeks; only client-side path actually populated rows. Cost impact: 2× briefs per 2-adult family; negligible at beta scale · **⭐⭐ iCal flicker root-cause fix** — the sync engine's DELETE-stale step was firing based on `allExternalIds` collected during the read loop. If any single calendar's `getEventsAsync` threw (transient EventKit failure common on recurring events), those events' ids never entered the set → DELETE wiped them from Supabase → next sync re-inserted → flicker. Fix skips the DELETE entirely if any read failed AND adds concurrent-sync coalescing (mount + AppState were both firing without debounce). Better to keep one stale row for one sync than flicker perpetually · **⭐ Widgets deferred to Build 55** — Rich's Session 35 widget scope (small Home mic, medium Home info, Lock Screen circular, Lock Screen inline + long-press icon shortcuts + share personal event) remains locked but deferred. Widget work involves proper native Swift/SwiftUI/WidgetKit with ~20-30 min per EAS iteration cycle — high risk of first-try issues that would delay this bug-fix ship. Foundation packages installed Session 35 (`@bacons/apple-targets` + `react-native-shared-group-preferences`). Only the "share personal event" piece landed in Build 54 since it was pure JS + one Supabase UPDATE · **Prior Session 35 work still current** — Full iCal two-way sync SHIPPED (Builds 49→52), Home tiles instant-load (Build 53), api_logs INSERT RLS unblocked, website homepage iPhone Calendar sync as headline feature. Session 34 shipped Builds 47 + 48 (Andy Add Event RLS fix, Option C member editing, cross-family name leak, ensureOwnMembership, anti-snowball CALENDAR HONESTY rules). Session 32 v2 vision pivot to family productivity trio + Reminders as 4th pillar. **Pricing A$6.99/mo family plan tax-inclusive** (unchanged).*

*Legacy Session 35 header (kept for context): 15 August 2026 late — Session 35 ✅ · **ICAL TWO-WAY SYNC LIVE + HOME INSTANT-LOAD + WEBSITE CALENDAR SYNC SECTION SHIPPED + WIDGETS SCOPED FOR BUILD 54.** iPhone Calendar sync is now a first-class product pillar. Andy's requested feature became the killer differentiator in one sprint day (Builds 49→52). Home tile UX polished to zero-flicker cold start (Build 53). Website's homepage now leads with the calendar sync story alongside the trio. Native widgets + long-press icon shortcuts + share personal event manually all scoped and locked for Build 54 — deferred to fresh session to give the native iOS work proper focus · **⭐⭐⭐ iCal two-way sync as a product pillar** — Zaeli now reads Andy's selected iPhone calendars into his personal view (respects per-calendar opt-in) AND writes Zaeli events back to a "Zaeli" EventKit calendar he can see alongside Work + Home + School calendars on iPhone. Full reconciliation on every sync: INSERT new, UPDATE changed, DELETE removed. Per-user external sync — Andy's iPhone work meetings are only visible to Andy in Zaeli, never leak to Anna/Nadia. Auto-syncs on app open + on foreground return. Onboarding step introduces the feature (informational only, deep-configure lives in Settings). This is now THE feature that keeps beta users engaged — Andy's dogfooding drove the entire scope · **⭐⭐ Home tiles feel instant** — 2-3 sec blank tile flash on cold-start was a genuine UX problem (felt broken, not slow). Fixed with cache-then-refresh: AsyncStorage per-family per-tile cache hydrates in ~50ms, fetch runs in background, silently updates state if fresh differs. Also: sign-out wipes cache (no leak between users), app foreground triggers refresh (returning from Safari/Notes → fresh data without user interaction) · **⭐ Website update: iPhone Calendar sync as headline feature** — `zaeli.app/` now shows dedicated Section 3.5 between Lockscreen briefs and Reminders (phone mockup + calendar picker + Sync now UI + "Zaeli events appear in iPhone. iPhone events appear in Zaeli." headline). Privacy page updated with iPhone Calendar events disclosure. This positions Zaeli against generic family calendar apps (Cozi, TimeTree) that either don't do two-way sync or bury it · **⭐ Build 54 scope — widgets + long-press shortcuts + share personal event (DEFERRED)** — locked scope for tomorrow's session: (1) Small Home widget = mic launcher (tap → chat with mic active). (2) Medium Home widget = today's events + next reminder. (3) Lock Screen circular = Zaeli logo (tap → app). (4) Lock Screen inline = one-line today summary. Also: long-press Zaeli icon → 4-action menu (Start with voice / Today / Shopping / Quick add) via Info.plist UIApplicationShortcutItems + zaeli:// URL scheme routing. Also: share personal event manually (privacy_scope toggle in event edit sheet). Foundation packages installed tonight (`@bacons/apple-targets` + `react-native-shared-group-preferences`). Native Swift/SwiftUI/WidgetKit work needs proper focus — Rich chose to defer over pushing through tired · **⭐ api_logs INSERT policy — 3 days of silent data loss FIXED** — RLS enabled on api_logs during Session 33 admin console prep, but no INSERT policy → every logApiCall silently rejected from ~12 Aug. Discovered when checking Andy's activity → zero rows visible. Fix: standard family_id INSERT policy. Cost visibility restored. Same lesson as Session 21: RLS-on + zero-policies = deny-everything · **Andy engagement watch** — dormant since 14 Aug 21:14 (last api_log, though real usage may be later given the 3-day logging gap). Never sent Nadia her invite. Rich sent direct re-engagement text with iPhone Calendar sync screenshot. Betting on the calendar sync feature to bring him back. Nadia invite has been pending 2 days unaccepted · **Damien Android** — never installed the sideload APK, never signed up. Two-week window to invite Nadia (Andy's wife) plus close network before we shift priority · **Prior Session 34 work still current** — Build 47 (Andy Add Event RLS fix, Option C member editing, cross-family name leak fix, ensureOwnMembership auto-heal, anti-snowball CALENDAR HONESTY rules). Session 32 v2 vision pivot (trio + Reminders as 4th pillar, Home is front door, Kids Hub/Tutor/Travel/My Space/Meals hidden, server-side brief scheduler on lockscreen). **Pricing A$6.99/mo family plan tax-inclusive** (unchanged).*

*Legacy Session 34 header (kept for context): 15 August 2026 — Session 34 ✅ · **BETA VALIDATION UNDERWAY WITH REAL USERS + FAMILY-MEMBER FOUNDATION HARDENED + ICAL SYNC + ANDROID PATH SCOPED.** Second real beta user (Andy) hitting real bugs, Damien Pagnin waitlisted for Android. Two-way iPhone Calendar sync locked as Andy's requested feature — full spec + SQL foundation ready, code deferred to Build 49+. Android decision: fast-track close family/friends via direct-install APK, public waits 6-8 weeks for proper Google Play Store internal testing track · **⭐⭐⭐ Cross-family data privacy validated + hardened** — Andy's Zaeli chat asked "who's it for (Poppy? Anna?)" for his Netball event, exposing Rich's family names to Andy's session. Root cause: hardcoded FAMILY line inside the cached Sonnet system prompt (Anthropic caches static text at 90% discount — so Rich's family names were baked into every user's context). Fix: moved FAMILY to dynamic block built from real roster at prompt-build time. Cost impact negligible (~50 tokens uncached per turn), privacy story restored. Same fix pattern applied to brief-generator + server-side brief scheduler prompts. **Locked rule: never hardcode family-specific data in cached prompts. Ever.** · **⭐⭐ Our Family module fully editable (Option C)** — real users can now tap Name → text input modal → save (writes family_members.name + profiles.name), tap Colour → 8-swatch palette picker → save, toggle kid Budget access (persisted to profiles.budget_access), Remove-from-family with confirmation (SECURITY DEFINER RPC deletes family_members row + nulls profiles.family_id to revoke RLS access). Backed by tighter family_members RLS (owner-or-self UPDATE, owner-only DELETE) so client permissions match server enforcement · **⭐⭐ family_members "invisible owner" bug FIXED** — Andy signed up, added Nadia/Neave/Louie in onboarding, opened Our Family — and wasn't in the list. Root cause: onboarding filtered `id === 'me'` from the family_members insert with reasoning "family_members is for OTHER family members" — but every downstream screen (Our Family, Calendar avatars, Meal cook picker, event assignee resolution) treats family_members as THE authoritative roster. Fix: NEW `ensureOwnMembership()` auto-heal helper fires on app open regardless of screen (_layout.tsx proactive + family.tsx safety net), onboarding now inserts owner with `id = profile.id`, invitee flow does same. Invariant enforced: `family_members.id === profiles.id === auth.uid()` for the person themselves · **⭐ Zaeli "meltdown" anti-snowball rules** — Andy saw Zaeli say "If it still doesn't show up, that's the calendar doing its little vanishing act again" — Sonnet had drifted into inventing external causes for past failures instead of executing cleanly. Three new rules to CALENDAR HONESTY block: (a) NO META-COMMENTARY on failures — each turn starts CLEAN, don't reference past-turn drama. (b) When user asks "did you add X?" call find_calendar_events FIRST — never guess. (c) RECURRING DEFAULTS — title + weekday + time is enough, don't over-ask clarifying questions · **⭐ Sheet-added event RLS bug fixed** — Session 21's family_id sweep missed one inline string literal at index.tsx:2979. Every sheet-added event was going to DUMMY_FAMILY_ID. Rich never saw it because his chat-tool adds worked. Andy hit it first attempt. Fix: NEW `awaitFamilyId()` async helper that blocks briefly (polls every 60ms, max 4s) waiting for profile load — throws with friendly "Profile still loading" error instead of falling back to DUMMY · **iCal two-way sync locked as Build 49+ priority** — Andy asked. Full two-way (not one-way subscribe), per-user external sync (Andy's iPhone events show only for Andy in Zaeli — Anna doesn't see his work meetings), opt-in per calendar (picker for his 8+ iOS calendars). 3-build plan: Build 49 foundation + read-in, Build 50 write-back to Zaeli-owned EventKit calendar, Build 51 reconciliation + Android parity. Foundation SQL ready (supabase-calendar-sync.sql — events privacy_scope + external_id + imported_by_user_id, new calendar_sync_config table, updated RLS to enforce privacy_scope=personal only visible to importing user). NOT yet deployed · **Android path scoped** — Damien Pagnin (Rich's cousin) waitlisted. Decision: direct-install APK for close family/friends now (EAS build → sideload link), public Google Play Store internal testing track in 6-8 weeks. Two new admin console email templates ready to send: 🤖 Android waitlist (holding line for public) + 🚪 Android back door (fun conspiratorial invite for cousins/friends — honest about scary "install from unknown sources" warning + no push notifications yet + manual reinstall for updates + guinea pig role). Both match existing lifecycle email branding · **Admin-announce false-failure fixed (Batch 9)** — email actually delivered but client saw HTTP 546 CPU-exceeded and reported "failed". Root cause: `await client.send()` SMTP handshake burned enough CPU to kill worker before response shipped. Fix: return 200 immediately, run SMTP in background via `EdgeRuntime.waitUntil`. Trade-off: rare Zoho failures now invisible to client (audit trail belongs in email_log — deferred) · **Brief-scheduler dormant-gate fresh-signup exemption** — Batch 6 gate skipped families with no api_logs in last 7 days to save Sonnet cycles. But fresh signups have zero api_logs BY DEFINITION → their first-ever morning brief never fired → the exact re-engagement hook we wanted for new users was blocked. Fix: any family with profile <7 days old is exempt from the dormant gate · **REMAINING**: verify Andy sees himself in family list post-Build-48 install, verify Andy no longer sees Rich's family names in chat, verify recurring events work now that anti-snowball rules landed. Sonnet 5 migration still queued for post-beta stability. Website v6 updates for iCal sync + Android messaging pending.*

*Legacy pre-Session-34 header (kept for context): 31 July 2026 — Session 32 v2 Round A ✅ · MAJOR VISION PIVOT — Zaeli's Phase 1 identity has shifted from "AI companion that also manages family life" (Philosophy B, Session 9) to **"family productivity trio: Calendar + Shopping + Budget"** with **Reminders as the 4th pillar**. Home is the front door (was Dashboard, renamed). Kids Hub / Tutor / Travel / My Space / Meals moved to `app/_hidden/` — off the map for v2 launch, not deleted, brought back when the trio is validated. Rich made this pivot after Anna beta round 2: her iPhone Notes was faster than Zaeli for shopping (6-10s vs 2-3s), meal planner uptake was zero, and the broad AI framing was diffuse. Narrower beachhead, sharper story · **Home tile design** — big-text headlines (26px 800): "2 events on today.", "4 items to grab.", "1 due at 3pm.", "On track for the month." Detail rows below at 13-15px. Per-tile mic icon inside each add pill routes to Chat with mic intent. "Full × →" hint top-right on every tile. Empty state = competence line, NEVER a nudge · **Universal chat bar identical Home ↔ Chat** — same styles, same position, zero-flicker swipe · **Reminders subsystem (4th pillar)** — 3 shapes: timed (push notification at instant to creator) / date-only (shows on day, no push) / undated (someday bucket). Recurring: 12-month generated instances via `repeat_group_id`. Gold Home tile + 92% sheet with unified input pill (mic/text/camera/send). Family-shared visibility, creator-only notifications · **Server-side brief scheduler** — Phase 07 shipped: Deno Edge Function + pg_cron every 15 min. Cron fires at each family's morning/evening time (widened window 0-45 min after target for cron drift), generates brief via Sonnet, pushes RICH content to lockscreen via Expo push. Local notifications KILLED — server is sole source now. First brief-on-lockscreen with real content · **Sheet-over-Home pattern** — tapping a tile no longer scrolls to Chat first. Sheets pop over Home directly (RN Modal is portal). Closing keeps user on Home · **Reminder timezone fix** — Hermes was parsing local ISO as UTC (10-hour Brisbane skew). New `parseLocalIsoAsDate` + CURRENT_TIME injected fresh into Sonnet prompt · **Pantry no longer pre-blocks shopping adds** — always adds first, mentions pantry status softly for user to remove if not needed. Rich's rule: pantry isn't 100% accurate. Speed wins · **Notify chip on calendar adds fixed** (was rendering in wrong branch — Session 30 regression) · **MoreSheet redesign** — 6 full-width shorter tiles stacked (was 3×2 grid), Reminders added, cleaner scan · **Budget: Categories → Expenses** — flat expenses model (fixed/variable badge + monthly amount), legacy nested categories still shown during transition · Prior Session 30 infrastructure (Phase 5 API keys server-side, Stripe LIVE, Family push, Privacy/Terms deployed) all still current — remaining: production EAS build + `eas submit --platform ios --latest` for Anna beta round 3, website v6 rewrite for v2 pivot (next), Round B stale-tile bug (Home tile doesn't refresh after sheet edits — fix queued), Sonnet 5 migration queued. **Pricing dropped 31 July: A$9.99 → A$6.99 family plan (tax-inclusive) with 14-day free trial.** Reflects narrower v2 product surface. Tutor add-on hidden with the rest until trio validates. "Less than a coffee. Whole family sorted." copy hook on the site. Stripe live products need updating (new Price + Payment Link + webhook map entry — Rich to execute).*
...
**Revenue (Session 32 v2 Round A — 31 July 2026):** **A$6.99/month family inc GST** with 14-day free trial · 100% web sales · Tutor add-on **hidden for v2**, returns when trio validates (likely A$7.99/child). "Less than a coffee. Whole family sorted." is the copy hook. Reduced from A$9.99 (Session 25) alongside the v2 pivot — narrower product surface (Kids/Tutor/Travel/MySpace/Meals hidden) warrants the reflection. Cost + ROI economics: per-family API ~A$3.10/mo + minimal fixed → per-family margin A$2.41-2.63/mo (38-41% at 100 families and up). At 1000 families = ~A$2,633/mo net profit. Full table in `[[project-pricing-decision]]` memory.

**Pricing history (kept for context):**
- Sub-A$5 tier considered (A$4.99) but rejected — margin knife-edge (13-18%), any API drift = breakeven. Also signals commodity — families don't recommend "the cheap app" the way they recommend "this thing that actually works for A$7/month".
- A$9.99 (Session 25, held ~5 months) — reduced from initial A$14.99 target. Worked for the fuller "AI companion" product surface but felt heavy once v2 narrowed to trio + Reminders.
- A$14.99 (Sessions 1-25 target) — never charged; superseded before public launch.
...
101. ✅ **Phase 5 — API keys server-side migration** ⭐ — Session 30 completion (17 July) — the pre-launch security migration that unblocks public App Store distribution. Anthropic + OpenAI + Whisper calls now route: **App → JWT → Supabase Edge Function → vendor API**. Keys never bundled in the app. Three new Edge Functions (`anthropic-proxy`, `openai-proxy`, `whisper-proxy`) each JWT-verified against `supabase.auth.getUser()`. Client code (chat send, tutor session, brief generator, receipt scan, pantry scan, memory extraction, Zaeli Noticed, Whisper transcription) all swapped from direct `fetch(anthropicUrl, {headers: {'x-api-key': KEY}})` to `fetch(proxyUrl, {headers: {'Authorization': 'Bearer <token>'}})`. `EXPO_PUBLIC_ANTHROPIC_API_KEY` + `EXPO_PUBLIC_OPENAI_API_KEY` scrubbed from the bundle. Previous state (keys extractable from compiled `.ipa`) was fine for trusted beta but not for anonymous users. **Cost implication:** every AI call now incurs a Supabase Edge Function invocation (marginal — well within the Pro plan quota) + ~50ms latency for the proxy hop. Worth it for security.

102. ✅ **Our Budget backend migration** ⭐ — Session 30 completion (17 July) — the deferred Session 17 backend pass finally done. Four Supabase tables (`income_streams`, `budget_categories`, `category_line_items`, `savings_goals`) each family-scoped with the Session 21 RLS pattern (SELECT/INSERT/UPDATE/DELETE gated by `current_family_id()`). `lib/budget.ts` exposes 8 CRUD helpers (`fetchIncome`, `saveIncomeStream`, `deleteIncomeStream`, `fetchCategoriesWithLineItems`, `saveCategory`, `saveLineItem`, `fetchSavingsGoals`, `saveGoal`). Budget now **persists across restarts + syncs cross-device** — Anna sees Rich's edits and vice versa. Previously all state lived in in-memory `SEED_` constants that wiped on every app restart. Massive credibility fix — a budget planner that forgets everything on relaunch fails the trust test in one session.

103. ✅ **Stripe LIVE-MODE ACTIVATED** ⭐ — Session 30 completion (17 July, external) — Rich completed KYC (Australian business identity verification). Live products created with tax-inclusive prices: Family Plan A$9.99/mo, Tutor Add-on A$7.99/mo per child. **Live Payment Link with 14-day free trial** (card required upfront, standard for reducing churn). Live webhook endpoint registered subscribed to same 6 events as sandbox. Live Customer Portal configured with `https://zaeli.app` return URL. `PRICE_TO_PLAN` map updated with live Price IDs. `STRIPE_SECRET_KEY` + `STRIPE_WEBHOOK_SECRET` swapped to live values. Subscribe button copy updated: **"Start free 14-day trial"** (was placeholder). Beta grants still bypass Stripe (Anna keeps beta access through her `beta_end_date`); new sign-ups after beta ends will flow through real live checkout. **Real money can now be charged.** Sandbox flow proven Session 28, live flow now equivalent.

104. ✅ **Privacy + Terms pages LIVE** ⭐ — Session 30 completion (17 July) — legal foundation for public launch. `https://zaeli.app/privacy.html` covers: Australian Privacy Act rights (access/correction/deletion/complaint), AI processor disclosure (Anthropic + OpenAI both process user data in the US, family data sent as prompts), Supabase (data stored in Sydney ap-southeast-2), Stripe (payment data never touches Zaeli servers), retention periods, subscription cancellation. `https://zaeli.app/terms.html` covers: subscription terms, 14-day trial, cancellation policy, AI content disclaimer (Zaeli is a companion not a professional advisor — no legal/medical/financial advice), IP + acceptable use, limitation of liability. Both pages served from the same Netlify site as the AASA + landing page. Contact throughout: `hello@zaeli.ai`. Settings → Privacy Policy / Terms links wired to open these URLs in WebBrowser. Non-negotiable pre-launch item — done.

105. ✅ **Supabase auth emails Zaeli-branded** — Session 30 completion (17 July) — Confirm signup + Reset password templates updated via Supabase Dashboard → Authentication → Email Templates. Both now feature: sky-blue `a+i` wordmark logo (inline SVG so no external asset dependency), warm Zaeli-voice copy ("Welcome in! Confirm your email so we can get started..." / "Password reset — tap the link below to pick a new one"), `hello@zaeli.ai` support link, "Less Chaos. More Family." tagline in footer. No more generic "Confirm your Supabase project email" — first-touch email now feels like Zaeli, not like a system notification. First impression matters more when the email is the user's very first interaction with the product.

106. ✅ **Anna beta round 2 fixes** — Session 30 completion (17 July) — four bugs surfaced during Anna's second beta round:
    - **Multi-event photo calendar adds** — Anna photographed a school term calendar with 8 dates. Zaeli added only the first + fabricated success responses for the rest ("Added Duke's excursion to Nov 5..." — nothing landed). Same shape as the Session 28 calendar phantom bug. Root cause: the Sonnet vision path processed all events in one call but the tool_use loop only iterated the FIRST tool_use block, silently dropping the rest. Fix: loop through all tool_use blocks in the response, execute each `add_calendar_event`, collect results, report accurate count in the follow-up message.
    - **Whisper Welsh hallucination** — Anna's voice notes were sometimes returning transcribed as Welsh nonsense text. Root cause: `whisper-1` auto-detects language and defaults poorly on short/ambiguous audio; 8 of 9 call sites in the codebase omitted the `language` parameter. Fix: all 9 call sites (chat mic, tutor mic, shopping mic, meals mic, etc) now force `language: 'en'`. Whisper stops guessing.
    - **`send_family_message` refusing to send** — leftover capability rule in the chat system prompt (from before the tool was added) said "I can't send messages on your behalf." When users said "text Anna to say home in 20", Zaeli would apologise and decline INSTEAD of calling the tool that Session 30 shipped. Fix: capability rule removed, prompt now explicitly documents the tool.
    - **`**markdown**` showing literally** — chat responses containing `**bold text**` were rendering as `**bold text**` in the feed instead of bold formatting (React Native Text doesn't parse markdown). Fix: strip `**...**` markers from Zaeli responses in the render layer, leaving plain text.

107. ✅ **Family push notifications fully LIVE** ⭐ — Session 30 completion (17 July) — Session 30's earlier missing-plugin fix (Phase 93) + verbose diagnostic (commit `dcb263b`) combined to fix push delivery end-to-end. On build #17: both Rich and Anna's devices registered push tokens successfully (verified via the "🔔 Register push token now" Developer row's structured Alert), family-notify Edge Function delivered messages successfully, Notify chip on calendar confirm cards works. The "Zaeli lives across every family member's phone" story is now real: Rich adds Duke's soccer carnival, taps Notify, Anna's phone lights up with a Zaeli-branded push. First cross-device shared surface — the moment where "Zaeli is Rich's AI" becomes "Zaeli is OUR AI."

108. ✅ **Dev row gate (email-allowlist)** — Session 30 completion (17 July) — Settings → Developer section now email-gated. `if (getProfile()?.email === 'richarddekretser@gmail.com') { /* render dev rows */ }`. Anna + any future testers no longer see scary buttons ("Reset to owner account", "Simulate invite accepted", "Force-open Stripe checkout", "Register push token now", etc). Pre-launch UX polish — regular users should never encounter developer surfaces. When more admins are needed post-launch, promote to a real `is_admin` flag on profiles.

### Session 30 strategic reflection

Session 30 is where the product crossed the line from beta-with-workarounds to genuinely public-launch-ready.

Family push notifications shipped — the shared-surface moment that makes Zaeli feel like "our app" rather than "Rich's app". Anna gets an OS push when Rich adds Duke's soccer carnival. First cross-device shared surface, and it works.

The Phase 5 API keys migration is the single most important pre-launch security change. The previous state (client-bundled keys) was fine while every user was someone Rich personally trusts. It was not okay for anonymous App Store distribution. That's now behind us — Anthropic + OpenAI + Whisper all route through JWT-authenticated Supabase Edge Functions. Marginal cost + ~50ms latency, worth every millisecond.

Our Budget backend migration closes the last "prototype pretending to be a product" gap. A planner that wipes on every app restart doesn't survive the first real week of use. Now it persists + syncs cross-device — Anna adds a subscription, Rich sees it. That's what makes it feel like a family surface, not a solo tool.

Stripe live-mode + Privacy + Terms + branded emails are the boring-but-essential legal/commercial foundations. None of them are exciting product wins. All of them were blocking the "sign up any Australian family and start charging them" moment. All done.

The Anna beta round 2 fixes (multi-event photo, Whisper Welsh, family message tool, markdown strip) are the ongoing shape of real-world usage — small quality bugs invisible in solo dev, obvious in real hands. Every one is a would-be trust erosion caught before it lands.

Backend pass is now ~99% complete. Only remaining items are optimisations (Sonnet 5 migration when Anna beta settles, per-user brief cache when scale demands it) — not blockers. **The app is ready for public sign-ups** as soon as Anna beta round 2 confirms nothing regressed.
...
- **All AI vendor calls route through Supabase Edge Functions with JWT auth** (Session 30 completion) — Phase 5 locked. Never reintroduce client-bundled `EXPO_PUBLIC_ANTHROPIC_API_KEY` / `EXPO_PUBLIC_OPENAI_API_KEY`. Every new vendor integration (ElevenLabs when we ship voice, any future model provider) must follow the same Edge Function proxy pattern.
- **Every family-scoped feature has real Supabase persistence** (Session 30 completion — Budget was the last hold-out). Never ship a family-scoped surface backed by in-memory `SEED_` constants again. If it's family data, it lives in Supabase with RLS.
- **Multi-tool-use responses MUST loop through ALL tool_use blocks** (Session 30 Anna round 2 fix) — never break after the first. Same discipline as the Session 28 "verify insert landed" pattern: silent partial success is a trust-killer.
- **Whisper `language: 'en'` on every call site** (Session 30 Anna round 2 fix) — auto-detect is unreliable on short/ambiguous audio and defaults poorly. AU English users don't want Welsh transcription. Any future speech-to-text integration follows the same explicit-language rule.
- **Dev rows email-gated to `richarddekretser@gmail.com`** (Session 30 completion) — regular users never see developer surfaces. Promote to a real `is_admin` flag when more than one admin is needed.
- **Auth email templates are a first-touch UX surface** (Session 30 completion) — treat them with the same brand discipline as onboarding. Never leave them as Supabase system defaults for anything user-facing.
...
33. ✅ **Backend pass — ~99% COMPLETE** (multi-session). Status as of 17 July:
    ...
    - ✅ **Phase 5 — API keys server-side migration Session 30 completion (17 July)** ⭐ — Anthropic + OpenAI + Whisper calls now proxy through Supabase Edge Functions with JWT auth. Keys never bundled in the app. Unblocks public App Store distribution. Marginal Edge Function invocation cost + ~50ms latency per call.
    - ✅ **Our Budget backend migration Session 30 completion (17 July)** ⭐ — 4 Supabase tables (`income_streams`, `budget_categories`, `category_line_items`, `savings_goals`) with family-scoped RLS. `lib/budget.ts` 8 CRUD helpers. Persists across restarts + syncs cross-device. Previously in-memory SEED_ constants that wiped on every restart.
    - ✅ **Stripe live-mode ACTIVATED Session 30 completion (17 July)** ⭐ — Rich completed KYC. Live products (A$9.99 Family + A$7.99 Tutor, tax-inclusive), live Payment Link with 14-day free trial (card required upfront), live webhook, live Customer Portal. `PRICE_TO_PLAN` map updated with live Price IDs. Subscribe button copy: "Start free 14-day trial". Real money can now be charged. Beta grants still bypass Stripe.
    - ✅ **Privacy + Terms pages LIVE Session 30 completion (17 July)** ⭐ — `zaeli.app/privacy.html` (Australian Privacy Act rights + AI processor disclosure + Sydney data storage + Stripe payment handling + retention). `zaeli.app/terms.html` (subscription terms + 14-day trial + cancellation + AI content disclaimer + IP + liability). Contact `hello@zaeli.ai`. Settings links wired.
    - ✅ **Supabase auth emails Zaeli-branded Session 30 completion (17 July)** — Confirm signup + Reset password templates with sky-blue `a+i` logo, warm Zaeli-voice copy, `hello@zaeli.ai` support link, "Less Chaos. More Family." tagline.
    - ✅ **Family push notifications fully LIVE Session 30 completion (17 July)** — verification on build #17 confirmed end-to-end delivery. Rich + Anna push tokens registered. `family-notify` Edge Function delivers. Notify chip on calendar confirms works cross-device. First real shared-surface moment.
    - ✅ **Anna beta round 2 fixes Session 30 completion (17 July)** — multi-event photo calendar adds (loop through all tool_use blocks), Whisper Welsh hallucination (force `language=en` on all 9 call sites), `send_family_message` capability rule fix (was refusing to send), `**markdown**` strip in render layer.
    - ✅ **Dev row gate Session 30 completion (17 July)** — Settings Developer section email-gated to Rich only. Testers no longer see admin-only surfaces.
    - 🔨 Phase 2e (NEXT): Cross-device verification on Anna's phone confirms nothing regressed after Session 30 completion round. Push + Budget + branded emails + live Stripe all need one full round of Anna-side smoke testing before public launch.
    - 🔨 **GIPHY_API_KEY in EAS env vars** — currently only in local `.env`, missing from EAS env config, so GIPHY celebrations won't render in TestFlight until added at expo.dev. Small chore, matches the Session 27 pattern for `EXPO_PUBLIC_*` vars.
    - 🔨 Phase 4b: final launch cleanup — remove Session 30 Developer row for force-opening Stripe Payment Link (now that live mode is real, no need to bypass beta), remove QR chip (Universal Links do the job), expo-document-picker for Budget CSV (EAS rebuild), share extension (EAS). GDPR export/delete flows deferred to Phase D but placeholder Settings rows can point to `hello@zaeli.ai` manual process for launch.
    ...
    - 🔨 **Product-side welcome email** — Supabase Edge Function on `auth.users` insert. Deferred until we have real onboarded users. Stripe handles transactional emails (receipts, cancellations) already; Supabase auth confirm-signup email is now Zaeli-branded (Session 30 completion) so the first-touch experience is covered.
    - 🅿️ **Sonnet 5 migration** — queued for post-Anna-beta-round-2. Intro pricing through Aug 2026 makes it cheaper than 4.6. Optimisation, not blocker.
    - 🅿️ **Per-user brief cache** — queued for scale past 100 families. Current family-scoped cache means each adult in a family triggers a cache miss on primaryUser signature. Optimisation, not blocker.
...
- [x] AI Brief system ✅ (Session 16 + v3 Session 19 — 2 windows, Option B render, 3-paragraph structured prose)
- [x] zaeli_briefs Supabase table ✅ (Session 16 migration + Session 26 backfill + RLS fix)
- [x] Real authentication ✅ (Session 21 — Phase 1 sign-up/sign-in via DB trigger; Session 22 — Phase 2d invitee signup)
- [x] EAS build · TestFlight ✅ (Session 25 first preview build; Session 30 build cycles #15-#17 for Anna beta)
- [x] LANDING_TEST_MODE = false ✅ (Session 25 Phase 4a)
- [x] **API keys server-side (Phase 5)** ✅ (Session 30 completion — Anthropic + OpenAI + Whisper via Supabase Edge Functions with JWT auth)
- [x] **Our Budget backend persistence** ✅ (Session 30 completion — 4 Supabase tables, RLS, `lib/budget.ts`, cross-device sync)
- [x] **Stripe live-mode ACTIVATED** ✅ (Session 30 completion — live products + Payment Link + webhook + Customer Portal + 14-day free trial)
- [x] **Privacy + Terms pages LIVE** ✅ (Session 30 completion — `zaeli.app/privacy.html` + `zaeli.app/terms.html`, Settings links wired)
- [x] **Supabase auth emails Zaeli-branded** ✅ (Session 30 completion — Confirm signup + Reset password templates)
- [x] **Family push notifications LIVE** ✅ (Session 30 completion — `family-notify` Edge Function verified end-to-end on build #17)
- [x] **Dev row gate** ✅ (Session 30 completion — Settings Developer section email-gated to Rich)
...
- [ ] Zaeli Voice (ElevenLabs) — deferred to post-launch
- [x] Push notifications — daily brief local (Session 23) + family push LIVE end-to-end (Session 30 completion)
- [ ] Gmail + Outlook Calendar integration — parked, revisit if Anna asks twice
- [ ] Spoonacular integration — parked to post-launch
- [x] Interactive onboarding ✅ (Session 19 — 13-step Zaeli-led flow + tour handoff)
- [x] Stripe integration ✅ (Session 30 completion — live-mode ACTIVATED, Payment Link + Customer Portal + webhook)
- [ ] Website marketing rewrite — parked, ~1 day of work
- [ ] HealthKit (Fitness sheet — EAS build)
- [ ] YouTube embedded player (Stretch + Zen — EAS build)
- [ ] Wordle cross-family challenges (post-launch)
- [ ] Live testing with 10 families
- [ ] Analytics
- [ ] GDPR — export data / delete account flows (manual `hello@zaeli.ai` process for launch, self-service post-launch)
- [ ] Multi-user real-time sync (currently write-through polling — real-time subscriptions post-launch if usage justifies)
- [ ] App Store submission — Anna beta round 2 confirmation, then submit
