...
33. 🔨 **Backend pass — IN PROGRESS, ~90% complete** (multi-session). Status as of 1 July:
    ...
    - 🔨 **Checkout flow build (NEW Session 28)**: currently users have no path to become a Stripe customer from within the app. Options: (a) Stripe Payment Link (no code, hosted page — fastest for TestFlight beta), (b) Stripe Checkout via WebBrowser (still hosted, more control), (c) custom Payment Sheet (most code, best UX). Recommend (a) for beta.
