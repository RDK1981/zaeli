...
async function executeTool(name: string, input: any): Promise<string> {
  try {
    ...
    if (name === 'add_shopping_item') {
      ...
      // ── DUPLICATE CHECK — fetch ALL items (active AND recently bought) ──
      const { data: existingAll } = await supabase.from('shopping_items').select('id, name, checked')
      ...
      const activeDupes: string[]   = [];
      const movedBack: string[]     = [];
      const newRows: any[]          = [];

      for (const i of input.items) {
        // Already on the active list — skip
        if (activeNames.some(e => fuzzy(e, i.name))) {
          activeDupes.push(i.name);
          continue;
        }
        // In Recently Bought — uncheck it and move it back to active
        const purchasedMatch = purchasedItems.find((p:any) => fuzzy(p.name.toLowerCase().trim(), i.name));
        if (purchasedMatch) {
          await supabase.from('shopping_items').update({ checked: false, completed: false }).eq('id', purchasedMatch.id);
          movedBack.push(purchasedMatch.name);
          continue;
        }
        // Brand new item — insert it
        newRows.push({
          family_id: DUMMY_FAMILY_ID,
      ...
      // Items moved back from Recently Bought
      if (movedBack.length > 0) {
        result += `✅ Moved back to your list from Recently Bought: ${movedBack.map(d=>`**${d}**`).join(', ')}.`;
      }
      ...
      if (newRows.length > 0) {
        ...
        try {
          ...
          const { data: pantryData } = await supabase.from('pantry_items').select('name, stock').eq('family_id', DUMMY_FAMILY_ID);
        ...
        } catch (e) { /* pantry check is best-effort */ }
      }

      // Items already on the active list — skip and note
      if (activeDupes.length > 0) {
        result += `${(movedBack.length > 0 || newRows.length > 0) ? ' ' : ''}⚠️ Already on the list (skipped): ${activeDupes.map(d=>`**${d}**`).join(', ')}.`;
      }
    ...
    if (name === 'add_recurring_event') {
      ...
      while (cur <= endOfYear) {
        ...
        rows.push({ family_id:DUMMY_FAMILY_ID, title:input.title, date:dateOnly, start_time:startISO, end_time:endISO, notes:input.notes||`Recurring ${frequency}`, timezone:'Australia/Brisbane' });
        if (stepDays) {
      ...
      return `✅ **${input.title}** booked ${frequency} — **${insertedCount} occurrences** added through ${endYearStr}.`;
    }
...
2. ALWAYS call the tool for every new item mentioned, even if it sounds similar to something discussed earlier.
3. The tool handles duplicates automatically. If it returns "Moved back to your list from Recently Bought" — relay that naturally, e.g. "Done — moved Nurofen back to your list from Recently Bought." No fuss, no questions.
4. If the tool returns "Already on the list (skipped)" — let the user know it's already there.
...
RECURRING EVENTS: If the user mentions anything weekly, fortnightly, or monthly — use add_recurring_event.
