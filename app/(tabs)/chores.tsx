...
type Mission = { emoji: string, title: string, points: number, member: string }
...
export default function MissionsOnboardingScreen() {
  ...
  const addCustom = () => {
    ...
    const newMissions: Mission[] = newMembers.map(m => ({
      emoji: newEmoji,
      title: newTitle,
      points: parseInt(newPoints) || 5,
      member: m,
    }))
    const newIndices = newMissions.map((_, i) => missions.length + i)
  ...
  return (
    <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
      ...
      <Text style={styles.title}>Quick mission{'\n'}setup</Text>
      <Text style={styles.sub}>AI-suggested for your kids' ages.</Text>

      {MEMBERS.map(member => {
        const memberMissions = missions.map((m, i) => ({ ...m, idx: i })).filter(m => m.member === member.name)
        return (
          <View key={member.name} style={styles.memberSection}>
            ...
            {memberMissions.map((mission) => {
              const isSelected = selected.includes(mission.idx)
              return (
                <TouchableOpacity
                  key={mission.idx}
                  style={[styles.mission, isSelected && styles.missionSelected]}
                  onPress={() => toggleSelected(mission.idx)}
                >
      ...
      {showAdd ? (
        <View style={styles.addForm}>
          <Text style={styles.addFormTitle}>ADD CUSTOM MISSION</Text>
          <TextInput
            style={styles.input}
          ...
          <View style={styles.row}>
            ...
            >
              <Text style={styles.saveBtnText}>Add mission</Text>
            </TouchableOpacity>
          </View>
        </View>
      ) : (
        <TouchableOpacity style={styles.addMissionBtn} onPress={() => setShowAdd(true)}>
          <Text style={styles.addMissionBtnText}>+ Add custom mission</Text>
        </TouchableOpacity>
      )}
...
const styles = StyleSheet.create({
  ...
  mission: { flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: '#141929', borderRadius: 12, padding: 14, marginBottom: 8, borderWidth: 1, borderColor: 'rgba(74,144,217,0.08)' },
  missionSelected: { borderColor: 'rgba(74,144,217,0.4)', backgroundColor: 'rgba(74,144,217,0.05)' },
  ...
  missionTitle: { flex: 1, fontSize: 13, color: '#FFFFFF', fontFamily: 'DMSans_400Regular' },
  ...
  addMissionBtn: { borderWidth: 1.5, borderColor: 'rgba(74,144,217,0.3)', borderRadius: 12, borderStyle: 'dashed', padding: 14, alignItems: 'center', marginBottom: 16 },
  addMissionBtnText: { color: '#4A90D9', fontSize: 14, fontFamily: 'DMSans_700Bold' },
