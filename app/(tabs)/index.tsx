...
import { getPendingChatContext, clearPendingChatContext, setPendingChatContext, consumeChatIntent, setChatIntent, bumpHomeRefresh, consumeTourResumePending, subscribeChatFocus, consumePersistedWidgetChatIntent } from '../../lib/navigation-store';
...
- Zaeli CANNOT make phone calls, send SMS/iMessage, or send email.
- Zaeli CAN send family push notifications inside the Zaeli app via the send_family_message tool — this is a real, delivered push that lands on the recipient's phone. NEVER tell the user you "can't send messages" — you can, via send_family_message. If a message is short and clearly meant for a family member ("text Anna to say I'll be home in 20"), invoke send_family_message directly. Only clarify the SMS-vs-in-app distinction if the user explicitly asks about SMS or if they want to reach someone outside their Zaeli family (e.g. a friend, plumber, colleague) — in that case, honestly say you can draft it and they'll need to paste into Messages themselves.
...
BACKGROUND KNOWLEDGE (likes, routines, patterns — NOT the calendar/todos). Never claim something is "booked" or "on the calendar" based on this. LIVE DATA above is the only source of truth for what's scheduled.${mem}`;
  ...
  //   3. The AsyncStorage poll useEffect below (Build 66 — cold-start
  //      widget race fix)
  //
            ...
            <View style={s.barFloat}>
              ...
              <View style={s.barPillV2}>
                ...
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                >
                  {/* Build 66 — mic stroke is now solid #0A0A0A (matches
                      Home bar's IcoMic T.ink). Was rgba(10,10,10,0.55)
                      which read as a lighter grey vs Home's black on
                      swipe. Rich's directive: Home bar is canonical,
                      Chat must match for seamless swipe. */}
                  <Svg width={24} height={24} viewBox="0 0 24 24" fill="none"
                    stroke={isRecording ? '#FF4545' : '#0A0A0A'} strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round">
                ...
                {/* Send — onTouchStart preserved. Allows photo-only sends (single or multi).
                    Build 66 — opacity gate REMOVED. Rich wants Home ↔ Chat swipe to look
                    identical. Home's send is always full coral because it's a nav trigger,
                    not a real send. Chat's disabled state now matches. Tap when empty
                    is a silent no-op (guarded by the trim/pendingImages check below). */}
                <View
                  style={s.barSendV2}
                  onTouchStart={() => {
                    const t = input;
                                ...
                                <View style={{ flex:1 }}>
                                  ...
                                  {(whenLabel || !isMe) && (
                                    <Text style={{ fontFamily:'Poppins_500Medium', fontSize:12, color:'rgba(0,0,0,0.5)', marginTop:2 }}>
                                      {r.remindAt ? '🔔 ' : ''}{whenLabel}{whenLabel && !isMe ? ' · ' : ''}{!isMe ? 'shared' : ''}
                                    </Text>
                                  )}
                                  {showChip && !isExpanded && (
                                    <TouchableOpacity
